...
**What to build:** 指向一个装着图片的目录，工具把每页 fit-inside 缩到目标尺寸、转成灰度、写出 PNG 到新目录，并给出一份逐页的报告。源目录保持只读。
...
**位深写死 8bit、颜色类型写死灰度**，06 号票逐页判定位深、编码器在灰度与调色板之间按体积取小者
（ADR 0004）之后一并退场——默认输出因此既不是 8bit，也不一定是灰度颜色类型，只剩「是 PNG」这一条；
