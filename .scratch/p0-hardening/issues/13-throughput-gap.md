...
**What to build:** 默认构建达到每秒八十页。

**「`fast` 模式」与「spec 定的」这两半都不成立，本条按事实写**：实现里没有 `fast` 这一档
——`src/request.rs` 的 `Mode` 只有 `Process` 与 `DryRun`，命令行上没有 `--mode`；
`CONTEXT.md` 的《模式》给 `fast` 描的那个配置（关掉 oxipng、仲裁只在边界带）
正是**默认构建**，因为 P0 的 Out of Scope 把 oxipng、SSIMULACRA2 仲裁与 `best` 整个摘了出去，
两档还没有分岔。那个「八十」也翻不到 spec 出处，`.scratch/` 下没有一份 spec 写着它
（见《落地记录》）。
...
- [ ] 默认构建在同一批素材上达到每秒八十页 —— **达不到，改判**，见下一条
