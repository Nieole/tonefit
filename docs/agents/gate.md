...
| 2 | `cargo test --no-default-features` | `target/gate/no-default-features` | **甩掉终端库**那一趟。会话里不碰终端的那几个模块（`session` 下的 `state`、`live`、`run`、`complete`、`viewport`、`columns`、`tone`，以及只在 `test` 里的场景夹具 `scene`）摆在特性外面，这一趟连它们自带的用例一起跑。 |
