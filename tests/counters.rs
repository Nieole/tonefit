...
//! **参照一张都不进缓存**（spec 的 P-B）。问「按输出页数存参照」的用例因此开着整卷统一灰阶跑。
...
/// 对一个目录卷跑一趟**预览**，点名 profile。
///
/// 这三个数与预览的交集有好几条（05 与 06 两票都落在这里），而 `fixtures` 那一侧
...
/// 写出环节要用的那一档因此已经定死，量化与编码分析环节当场做完。
///
/// 四条断言各钉票面的一条：
///
...
/// - **画质分曲线照旧求**——票面明写要保住的那一条。覆盖了判定也照求画质分值，
///   预览才说得出「你点的那一档画质分是多少」；候选只剩一个，那一个的值一格不少。
...
fn assert_the_pinned_volume_never_cached_a_reference(volume: &VolumeReport, candidate: Candidate) {
    ...
    for page in &volume.pages {
        assert_eq!(page.scores().len(), 1, "顶死之后画质分曲线不求了");
...
/// 整卷统一灰阶那条路上灰度卷的三个数说的是同一批页：解码按**源页**数，缩放与参照进缓存按
/// **输出页**数，而这一卷没有跨页，三者相等。
#[test]
fn every_page_of_a_gray_volume_is_resized_once_and_cached_once_under_the_envelope() {
    ...
    assert_eq!(
        ...
        "整卷统一灰阶那条路上每张灰度输出页存一份参照"
    );
}

/// **默认那条路（逐页）上参照一张都不进缓存**（spec 的 P-B；ADR 0018 之后窗口长度是 0）。
///
/// 一页的档只取决于它自己，画质分一出来就定了：量化与编码分析环节当场做完，缓存那一格从头
...
/// 一个消费者。**照做这一趟要编码，缩放因此照旧跑**——同一张页在预览里已经一次都不缩
...
/// **全彩卷上预览的缩放次数降到零**（05 号票的正题）。
///
/// 彩色分支的缩放结果只有编码一个消费者，而预览不编码——那一整趟三平面的预缩加卷积
...
fn a_dry_run_of_an_all_color_volume_resizes_nothing() {
    ...
    assert_eq!(volume_report.decodes, 2, "预览连解码都省了，那就认不出彩页");
    assert_eq!(volume_report.resizes, 0, "预览为它根本不会编码的彩页缩了图");
    assert_eq!(
        volume_report.cached_references, 0,
        "彩色分支不进灰度缓存（ADR 0005 决定第 4 条）"
    );
...
/// 混合卷上预览与照做只差**彩页那一次缩放**，另两个数逐个相同。
///
/// 05 与 06 两票动的都是「某个开关关掉之后分析环节该少做点什么」，这一条是它们的对照组：
/// 05 之后差的正好是彩页那一张，而灰度页照旧要缩——画质分要缩放结果，
/// 预览存在的理由正是预告那个判定。06 号票动的是第三个数。
#[test]
fn a_dry_run_skips_only_the_color_resize_of_a_mixed_volume() {
    ...
    // 开着整卷统一灰阶：默认那条路上照做不存参照、预览照旧存（预览没有写出环节要那些字节），
    ...
    assert_eq!(trial.resizes, 1, "预览该只剩灰度那一张要缩");
    assert_eq!(
...
/// **黑白面板上预览照旧缩每一张彩页。**那上面彩页转灰、走灰度路径，
/// 而灰度路径的缩放结果有画质分这个消费者，预览正是为了预告它。
///
/// 省掉的是彩色分支那一条路，不是「预览」这个开关——分流由面板定（ADR 0010 决定第 3 条）。
...
/// **没被顶死的那一趟一切照旧**（票面末一条）。
///
/// 只点名灰阶档位：其余页那一组的尺寸贴合检查开着，抖动那一维还有得判，画质分照旧说了算——
/// 整卷统一灰阶那条路上那一档要等整卷汇总，参照因此照旧攒到写出环节。
///
/// 它是上面三条的对照组，也是「谓词别放得太宽」那一面的钉子。开着整卷统一灰阶跑：
...
/// **预览不在其内。**
///
/// 那一趟没有写出环节，编出来的字节一个读者都没有；缓存也只记账、不留页
/// （`cache::Retention::Account`），没有块可换。顶死的预览因此照旧攒参照——
...
fn a_pinned_dry_run_still_caches_every_reference() {
    ...
    assert_eq!(report.resizes, 2, "预览的灰度路径照旧要缩放：画质分要它");
    assert_eq!(
        report.cached_references, 2,
...
fn both_counters_are_exact_when_the_first_pass_runs_on_every_core() {
    ...
    let space = Workspace::new();
    let volume = space.volume("volume-a");
    // 页窄而高：卷级那几条路径不看页上画着什么，窄到只够铺开两块画质分的分块就行。
