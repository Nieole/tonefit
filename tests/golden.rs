...
///
/// 它们考的是上包络、特例与迟滞，与几何无关，用的是小页（`fixtures::TINY`）——而小页
/// **只在 fit-inside 上还是小页**：以高为准会把每一页放大到面板高，一卷六十页的代价
/// 跟着涨两个数量级。
///
/// 这一组因此**只载着门不成立那一侧的卷级行为**：候选集里没有抖动那一维，基准档与
/// 迟滞升上去的那一档都在位深这一维上分胜负。门成立那一侧的同一批路径另有一个长卷
/// （`envelope-hysteresis-gate-holds`，08 号票），本组之外那些正常尺寸的卷
/// （`mixed`、`archive.cbz`、`damaged`）则只够得着上包络与特例，够不着迟滞。
...
/// 长卷因此只记卷级摘要——那一行里有基准档、定档页、其余页/特例/升档三个计数，
/// 本来就是卷级那几条路径的全部观测点。
struct Case {
    name: &'static str,
    build: Build,
...
/// 一卷几十页的代价压到可接受。小到哪一步由这一卷跑在哪条适配方式上定——
/// `SMALL_PAGES` 那三卷用 `fixtures::TINY`，默认那条路上的用 `fixtures::NARROW_PASSES_THROUGH`。
///
/// **`SMALL_PAGES` 那三卷的基线踩在一件尚未想清楚的事情上**：它们的页比面板小，
/// 几何门在**每一页**上都不成立（ADR 0007：抖动仅在目标尺寸未被下游缩放时启用），
/// 一页成立的都没有，那些页于是自己就当其余页、基准档必然不抖（ADR 0007 决定第 5 条）。
/// 而几何门不成立时该用哪个位深集合，是 `CONTEXT.md` 的《尚未确立》里明确挂着的一条——
/// 对齐消失，灰阶硬上界的依据随之失效，P0 只是暂且仍照 {1,2,4} 裁。那件事定下来的那一天，
/// 那三卷的位深与抖动会整片变动，而它与上包络、特例、迟滞一点关系都没有。**那不是回归。**
...
fn mono_cases() -> Vec<Case> {
    vec![
        ...
        // 彩页落在黑白面板上：转灰后走灰度路径，是特例页的主要来源（`CONTEXT.md`）。
        ...
        // 多页混排：卷内档位一致与定档页落在哪一页，只有多页才看得出来（ADR 0006）。
        // 三页线稿，另两页（渐变与纯色）在线稿过得去的那一档上远在界外，
        // 五页的卷因此摘得出特例页。**短卷上特例页那一层是唯一还挡着的防线**——上分位的秩在 20 页以内
        ...
        //
        // 页换成两种适配方式下都恒等通过的窄页，几何门因此**每一页都成立**，候选集是
        // 六个那一套——迟滞升的于是不再是位深，而是抖动那一维。这正是「两条路的行为不同」
        // 那句话的现场。
        //
        // 两个纯色取值与它们各自的判据读数出自 `fixtures` 的 `NEEDS_TWO_BITS` 与
        // `ONE_STEP_ABOVE_TWO_BITS`（那两处是出处，这里不复述）：一个判 `2bit` 定出基准档，
        // 一个基准档过不了它的界、又够不上特例，成段之后升到 `2bit+FS`。
...
fn render(
    ...
    let report = tonefit::run(&Request {
        ...
        // 纸白对齐这条路径在这一篇上因此没有覆盖，端到端那几条在 `tests/pipeline.rs`。
        // 逐卷的数与处置见停车场 Q550。
