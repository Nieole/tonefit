...
//! 15 号票那一句：防止调优在无人察觉时改变判定结果。画质分的低通核、细节放宽加权、分块的
//! 边长与尾巴宽度、整卷统一灰阶的分位、迟滞页数、特例画质分的立脚点分位与倍数、编码器在灰度与
//! 调色板之间的取舍，任何一处动一下都会在这里露出来。
//!
//! **两条路各记各的**（ADR 0018）：头一组是**不加参数的人拿到的那一套**——默认那条路，
//! 灰阶档位逐页各判各的、不做迟滞；其余几组开着 `--envelope`，那条路的形态一格没动
//! （ADR 0006 的决定），整卷统一灰阶的分位、迟滞页数、特例画质分那几个数只有在那条路上才露面。
//! 卷行的标签写着走的是哪条路，两趟的数因此混不到一起比。
//!
//! 非退化的上分位与迟滞升档要页数够多才走得到，因此有四个长卷专门喂它们——三个在
//! `--fit inside` 上，一个在默认缩放方式上（迟滞升档在两条路上升的不是同一档，见 `SMALL_PAGES`
//! 与 `envelope-hysteresis-gate-holds`）；这几卷只考整卷统一灰阶那一层，默认那条路上不跑。
//! 特例那一条不挑卷长，短卷夹具里就走得到（立脚点逐页各取一个，见 `envelope` 的 `outlying`）。
//! 画质分自己那一层的分块聚合则要**局部**损伤才走得到，`local-damage` 专喂它。
//! 归档卷单列一个，让写进容器的页字节数也进快照。
//!
//! 与 `tests/metric.rs` 分工相反：那边测的是画质分**该有的性质**，数值动了不算错；
...
#
# 卷行：`[型号 缩放方式 裁白边 路] 卷名 · 尺寸贴合检查 · 卷级判定`。
# 路是 `per-page`（默认：灰阶档位逐页各判各的，ADR 0018）或 `envelope`（`--envelope`：整卷统一灰阶加迟滞）。
...
enum Route {
    /// 默认：灰阶档位逐页各判各的，不做迟滞。
    PerPage,
    /// `--envelope`：整卷统一灰阶加迟滞，卷内其余页共用统一档位。
    Envelope,
}
...
/// **几何上真会分岔**的三个卷：两条缩放方式各跑一遍（页几何批 01 号票）。
///
...
///
/// 它们考的是整卷统一灰阶、特例与迟滞，与几何无关，用的是小页（`fixtures::TINY`）——而小页
/// **只在 fit-inside 上还是小页**：以高为准会把每一页放大到面板高，一卷六十页的代价
/// 跟着涨两个数量级。
///
/// 这一组因此**只载着未贴合屏幕那一侧的卷级行为**：候选集里没有抖动那一维，统一档位与
/// 迟滞升上去的那一档都在灰阶档位这一维上分胜负。门成立那一侧的同一批路径另有一个长卷
/// （`envelope-hysteresis-gate-holds`，08 号票），本组之外那些正常尺寸的卷
/// （`mixed`、`archive.cbz`、`damaged`）则只够得着整卷统一灰阶与特例，够不着迟滞。
...
/// 默认缩放方式（以高为准）上跑的夹具卷：除 [`SMALL_PAGES`] 那三卷之外的全部。
///
...
/// 之外的全部。
///
/// 摘掉的是只考整卷统一灰阶那一层的长卷（`Case::summary`）：那一层在默认那条路上根本不在场，
/// 卷级那一行只会写「逐页」，六十页跑下来一个观测点都没有。留下的每一卷逐页记行——
/// 这一组记的正是每一页拿到画质分说它要的那一档。
...
///
/// 默认换成以高为准之后，**尺寸未贴合屏幕那一支在默认路径上成了空集**——每一页的高都等于
/// 面板高。ADR 0007 认下的那笔代价并没有消失，它只是搬到了 `--fit inside` 上，
/// 而快照要是只跑默认，那条路连同它下面的一整套行为（抖动关闭、跟着统一档位的灰阶档位走、
/// 一页成立的都没有时未贴合屏幕的页就当其余页）会一行都不剩。
...
/// **裁白边会把它们考的东西拿走**的两个卷，因此在 `--no-crop` 上再跑一遍（页几何批 02 号票）。
///
...
///   那条路径在快照里一行都不剩。
/// - `local-damage` 是一页留白加一小块灰调补丁，考的是**画质分的分块聚合**（ADR 0002 决定第 3 条）。
...
///
/// 它们仍留在默认那一组里：裁白边对它们做了什么本身就值得钉住。这一组是**补**回被拿走的
/// 那两条路径，不是替换。
const KEPT_MARGINS: [&str; 2] = ["a-class-gradient", "local-damage"];

/// 裁白边关着那条路上跑的夹具卷（页几何批 02 号票）。
...
/// 特例那一条不挑卷长），而几十行页行会把快照撑爆，真正的判定变动反而淹在里面。
/// 长卷因此只记卷级摘要——那一行里有统一档位、代表页、其余页/特例/升档三个计数，
/// 本来就是卷级那几条路径的全部观测点。
struct Case {
    name: &'static str,
    build: Build,
...
/// 一卷几十页的代价压到可接受。小到哪一步由这一卷跑在哪条缩放方式上定——
/// `SMALL_PAGES` 那三卷用 `fixtures::TINY`，默认那条路上的用 `fixtures::NARROW_PASSES_THROUGH`。
///
/// **`SMALL_PAGES` 那三卷的基线踩在一件尚未想清楚的事情上**：它们的页比面板小，
/// 尺寸贴合检查在**每一页**上都不成立（ADR 0007：抖动仅在目标尺寸未被下游缩放时启用），
/// 一页成立的都没有，那些页于是自己就当其余页、统一档位必然不抖（ADR 0007 决定第 5 条）。
/// 而尺寸未贴合屏幕时该用哪个灰阶档位集合，是 `CONTEXT.md` 的《尚未确立》里明确挂着的一条——
/// 对齐消失，灰阶硬上界的依据随之失效，P0 只是暂且仍照 {1,2,4} 裁。那件事定下来的那一天，
/// 那三卷的灰阶档位与抖动会整片变动，而它与整卷统一灰阶、特例、迟滞一点关系都没有。**那不是回归。**
/// 门成立那一侧的长卷不在此列：它的候选集本来就是六个那一套。
...
fn mono_cases() -> Vec<Case> {
    vec![
        // B 类中位页：总缩放比 1.219 < 2，预缩退化为恒等，连续灰调直接考画质分。
        ...
        // 一页留白里一小块灰调崩坏，页与面板等大——**画质分这一侧的分块聚合只在这里走得到**。
        // 别的夹具页画的都是铺满整页的东西，块与块之间读数相近，尾巴取第几块都一样；
        // 只有这一页的损伤是局部的，尾巴宽一格窄一格当场换一档灰阶档位（02 号票，ADR 0002 决定第 3 条）。
        ...
        // 彩页落在黑白面板上：转灰后走灰度路径，是差异大的页的主要来源（`CONTEXT.md`）。
        ...
        // 一条空白列都挑不出来，中缝因此找不到——这一页够得上跨页候选却**不被切开**，
        // 退回以高为准、靠横向平移看。「切不切由中缝决定」在快照里就是这一行：
        // 它仍然是一页，而不是两页。
        Case::new("spread", |volume| {
            volume.page("001.png", &fixtures::gradient(fixtures::SPREAD));
        }),
        // **拆得开的跨页**（页几何批 04 号票）：正中偏左一条贯穿全高的中缝。
        //
        ...
        // 源比目标小：不放大，一条边都贴不住，尺寸贴合检查在这一页上不成立。
        // 全卷只此一页，一页成立的都没有——它自己就当其余页，抖动因此关着（ADR 0007 决定第 5 条）。
        ...
        // 它自己拿卷级统一档位的灰阶档位、抖动关掉。这一卷因此是快照里唯一两套候选集
        ...
        // 多页混排：卷内档位一致与代表页落在哪一页，只有多页才看得出来（ADR 0006）。
        // 三页线稿，另两页（渐变与纯色）在线稿过得去的那一档上远在界外，
        // 五页的卷因此摘得出差异大的页。**短卷上差异大的页那一层是唯一还挡着的防线**——上分位的秩在 20 页以内
        // 就是页数本身，统一档位那一层已经退化了（见 `envelope` 的 `envelope`）。
        ...
        // 以下三卷至少二十页。二十是**统一档位**那一层的下限，不是随手取的：
        // 上分位的秩是 ceil(0.95n)，n≤19 时它等于 n，秩落在排序末位——统一档位成了
        // 判定最高的那一页自己，上分位整个退化。n=20 时秩是 19，它第一次真的是个分位。
        // 差异大的页那一层不受这个下限约束：立脚点逐页各取一个、不含被判的这一页，
        // 短卷照样摘得出来（上面的 `mixed` 就是五页的卷，加固批 01 号票）。
        //
        // 填充页一律用 `solid`：卷级那三条路径只看画质分的分布，不看页画的是什么，
        // 而纯色页的生成、编码、解码、画质分全是这批夹具里最便宜的。
        //
        // 三卷都只记卷级摘要——那一行里的统一档位、代表页、其余页/特例/升档三个计数，
        // 就是这几条路径的全部观测点，二十行页行只会把真正的变动淹掉。

        // 非退化的上分位 + 一页特例：统一档位由分位定出，那一页另行摘走。
        ...
        // 特例占比 10%，越过上分位那 5% 的线。**这一卷正是加固批 01 号票的现场**：
        // 立脚点从前由全卷聚合而来，秩 19 落进那两页自己里，它们合力把立脚点抬上去，
        // 检验又在那一档上做，于是谁都不显著偏离——摘出 0 页，剩下那十八页被拖高到 4bit。
        // 立脚点改成逐页各取一个、不含被判的这一页之后，这两页摘了出去，其余页落回 1bit。
        // 快照里那一行记的就是修完之后的样子。
        ...
        // **迟滞升档在默认那条路上的那一卷（08 号票）。** 上面那三卷全在 `--fit inside` 上，
        // 于是 ADR 0006 决定第 4 条在默认路径上一条端到端用例都没有（停车场 Q6）。
        //
        // 页换成两种缩放方式下都恒等通过的窄页，尺寸贴合检查因此**每一页都成立**，候选集是
        // 六个那一套——迟滞升的于是不再是灰阶档位，而是抖动那一维。这正是「两条路的行为不同」
        // 那句话的现场。
        //
        // 两个纯色取值与它们各自的画质分读数出自 `fixtures` 的 `NEEDS_TWO_BITS` 与
        // `ONE_STEP_ABOVE_TWO_BITS`（那两处是出处，这里不复述）：一个判 `2bit` 定出统一档位，
        // 一个统一档位过不了它的界、又够不上特例，成段之后升到 `2bit+FS`。
        // 卷长 60 与那一段 3 页的由来同 `envelope-hysteresis`——那条不等式数的是页在序列上的
        // 名次，候选集换了它不跟着换。
        Case::summary("envelope-hysteresis-gate-holds", |volume| {
            long_volume(volume, 60, |index| {
                let level = if (8..=10).contains(&index) {
                    fixtures::ONE_STEP_ABOVE_TWO_BITS
                } else {
                    fixtures::NEEDS_TWO_BITS
                };
                fixtures::solid(fixtures::NARROW_PASSES_THROUGH, level)
            });
        }),
        ...
        // 三种页状态与透传文件各占一格（04 号票）：完好的那一页、残缺的那一页、
        // 失败的那两页——一张连尺寸都分配不下，一张尺寸有而一个像素都没救回来。
        // 这一卷整个进隔离目录（12 号票），空白占位页按卷内统一尺寸出；
        // 残缺的那一页按自己的尺寸出，也不进整卷统一灰阶。
...
fn render(
    ...
    let report = tonefit::run(&Request {
        inputs,
        // 各设备、各缩放方式、各条路一个输出目录：同一个工作区里几趟并列，互不覆盖。
        ...
        // **照默认值走**（05 号票起是 4，即默认开着）：这一份快照记的就是**不加参数的人
        // 拿到的那一套判定与体积**。
        //
        // **抬默认值那一趟它一个字节都没动，而那不是它拦住了什么。**这批夹具的纸白
        // 逐页量过：不是 255（离格量 0、本来就在格点上），就是 0/15/85/86/96/128/200
        // （离格量 55~255，远超上限 4）——**一页都落不进钳得动的 251~254 那一段**。
        // 纸色提白这条路径在这一篇上因此没有覆盖，端到端那几条在 `tests/pipeline.rs`。
        // 逐卷的数与处置见停车场 Q550。
        white_align_limit: tonefit::WhiteAlignLimit::default(),
        bit_depth: None,
        dither: None,
        // 走哪条路由这一组定（见 [`Route`]）：头一组是默认（逐页），其余几组开着整卷统一灰阶。
        envelope: route.on(),
...
/// 这一卷的尺寸贴合检查（ADR 0007）：判定范围有几页，其中几页不成立。
///
/// 门逐页判（06 号票），卷级这一行因此记的是两个计数，不是一页的名字。
/// 是**哪几页**不成立，逐页那几行自己说得出来：它们的理由写着「尺寸未贴合屏幕」。
...
fn page_line(volume: &VolumeReport, page: &PageReport, sizes: &BTreeMap<String, u64>) -> String {
    let (candidate, mut reason) = match page.branch() {
        ...
        None => ("failed".to_owned(), "坏页 · 空白占位页".to_owned()),
    };
    ...
    // 救回了多少进快照（04 号票）：它决定这一页算不算数——救回到 0 就是一张坏页，
    // 而这个比例还决定了它进不进整卷统一灰阶。夹具固定，它因此也是个定值。
...
///
/// 它排在目标尺寸**之前**：裁白边发生在适配之前，目标尺寸就是从这一列算出来的。
/// 两列都进快照，是因为它们会各自变动——裁法改了，头一列先变；缩放方式改了，后一列才变。
