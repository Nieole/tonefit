...
/// （坏页、卷转换失败、结束）各留下它带的那几样，因为有用例要拿它们与报告比。
...
impl Recorder {
    ...
    /// 结束那一条报的是什么。一条都没收到时是 `None`——「报过开工就一定报得到结束」
    /// 正是靠这个分得开（停车场 Q39）。
...
/// 各段自己可能不在，报的就少一遍：`--no-metadata` 没有幂等那一道，dry-run 没有写出环节。
///
...
fn the_decision_point_carries_the_volume_report_only_for_an_observer_that_reads_it() {
    // 两棵各自的工作区：同一个输出目录跑第二趟会命中幂等而整卷跳过，
    // 那一趟连确认点都到不了。
    ...
    assert_eq!(
        ...
        "读它的那一位在确认点上没拿到这一卷的报告，会话就画不出「拿什么主意」"
    );
    assert_eq!(
        skimmer.seen(),
        vec![
...
fn a_failed_page_is_reported_the_moment_it_fails() {
    ...
    assert_eq!(failures.len(), 1, "坏页没有报出来：{failures:?}");
...
/// 观察者在第 2 卷之后答**做完再停**：当前卷跑完就停，盘上只有完整的卷（ADR 0013 决定第 1 条）。
///
...
fn finishing_up_after_the_second_volume_leaves_two_whole_volumes() {
    ...
    assert!(!report.any_isolated(), "按停止不该让任何卷进隔离目录");
    assert_eq!(
        recorder
            .shape()
            .iter()
...
/// 立即停止在**卷边界上**也停得下来：力度更强的那个字不该比做完再停那一级停得更晚。
///
/// 它按在一卷跑完那条事件上，而那正是做完再停生效的地方——两级停止在这一道上给出同一个答案。
/// 立即停止**更早**停的那一道在页边界上，见
...
/// 观察者在**页边界**上答立即停止：那一卷当场停下，它那格 `partial` 被丢掉
/// （04 号票，ADR 0013 决定第 2 条）。
///
/// 三件事在这一条里连成一串，缺一件断言就落空：
///
/// 一是按下那个字的**当口**——写出环节已经写出一张页，那格 `partial` 真的在盘上。
/// 不看这一眼，「丢弃」测的就可能只是「它压根没建出来过」。
///
/// 二是立即停止之后**最终位置上没有它这一趟的任何痕迹**：既没有 `volume-a`，
...
/// 页边界那个检查点在**每一遍**上都在：分析环节答立即停止，写出环节连开工都没有。
///
/// 「立刻停」不能等到写出那一遍才生效——一个几千页的卷，分析环节是解码、缩放、算画质分，
/// 那是这一趟最贵的一段。停在分析环节的页边界上，写出环节那格 `partial` 因此连建都没建过。
///
/// 判别式是**点名那个卷走过哪几遍**：写出环节不在那张单子上，就说明立即停止没有等到写出才停。
/// 光看盘上没东西是不够的——立即停止在写出环节写完第一张之后才停，盘上同样什么都不剩。
#[test]
...
/// **整卷统一灰阶那条路上分析环节答立即停止，输出目录同样纹丝不动**（12 号票第 6 条前半）。
///
/// 上面那一条测的是默认那条路（逐页）：那条路上分析环节多了一道工序——一页判完当场就
/// 量化、编码了。**编好不等于写出**——字节进的是缓存，落盘仍旧只在写出环节。
/// 整卷统一灰阶那条路上参照攒一整卷、写出环节才编，两条路各钉一遍，「写出环节开始之前一个字节
/// 都没写」才在两条路上都站得住。
///
/// 断言与上面那条同形：写出环节不在走过的那张单子上，输出目录下一个名字都没有。
#[test]
...
/// 页边界那个检查点在**幂等这一道**上也在：那一遍答立即停止，它后面两遍一遍都不开工。
///
/// 两个数一起断言，各钉一头：
///
/// - **走过哪几遍**只有幂等那一道——立即停止没有等到解码开始才生效；
...
fn aborting_in_the_fingerprint_pass_never_lets_the_passes_after_it_start() {
    ...
    assert_eq!(
        ...
        "立即停止在幂等这一道上按下，后面的环节却照样开了工"
    );
    assert_eq!(
        stop.steps(),
        AFTER_THE_FIRST_PAGE,
        "幂等这一道在立即停止之后把余下的成员也读完了"
    );
    assert!(report.volumes.is_empty(), "被立即停止的那一卷进了报告");
    assert_eq!(
        fixtures::names_in(&space.out()),
...
/// 透传文件也在页边界那个检查点的管辖里：页写完了才答立即停止，那个透传成员就不再写。
///
/// 写出环节写出的是**输出成员**，页与透传文件都算（`CONTEXT.md` 的《进度》：写出环节写全部
/// 输出成员）。页写完之后还有一截，那一截同样要停得下来。
///
/// 判别式只能是**步数**：这一卷横竖要被丢掉，写没写那个透传文件在盘上留不下痕迹。
/// 停在最后一张页上，写出环节就该正好走 [`PAGES`] 步；透传那个循环头拦不住的话是 `PAGES + 1`。
#[test]
...
/// 立即停止只丢**它自己建的那一格**，别的一样都不碰（ADR 0013 的《后果》与《不要做的「简化」》）。
///
/// 场上摆了三样上一趟留下的东西，立即停止之后三样都要原封不动：
///
/// - 这一趟先做完并**已经改名**到位的那个卷（`volume-a`）——立即停止发生在它之后；
/// - 被立即停止的那个卷在最终位置上**上一趟的成品**（`volume-b`）——逐字节比，
...
/// 归档卷那一格 `partial` 是个**文件**，立即停止照样把它丢掉。
///
...
///
/// - **做完再停**问的是卷边界——当前卷照样写完并改名，盘上留下一整卷，下一趟幂等接着走；
/// - **立即停止**问的是页边界——那一卷当场停下、`partial` 丢掉，等于没做。
///
/// 一个检查点做两件事、或者做完再停误落到页边界上，这一条当场红：那时两支给出同一个答案。
///
/// 做完再停那一支按了**两处**页边界：写出环节上一次、分析环节上一次。页边界那个检查点在每一遍上
/// 都在（见上面那三条），因此「做完再停不抢它的活」也要在每一遍上都成立——
/// 只在写出环节上对照的话，分析环节那个循环头误认了做完再停，这一条看不出来。
#[test]
fn the_two_checkpoints_stop_at_two_different_boundaries() {
    ...
    .expect("按停止不是失败");
    assert_eq!(report.volumes.len(), 1, "做完再停没让当前卷跑完");
    assert_eq!(
    ...
    // 同一个字按在分析环节的页边界上，答案该一模一样：做完再停不在页边界上生效。
    ...
    .expect("按停止不是失败");
    assert!(report.volumes.is_empty(), "立即停止把当前卷跑完了");
    assert_eq!(
...
/// [`three_page_volume`] 有几页。按停止那几条按下去的时机按它数出来，因此只此一个出处。
const PAGES: usize = 3;

/// 写出环节写完**头一张**页就按下去：那一刻 `partial` 里正好装着一张页，
/// 后面还有页没写——「当场停下」因此看得出来。幂等那一道上它是「头一个成员刚喂进哈希」。
const AFTER_THE_FIRST_PAGE: usize = 1;

/// 写出环节写完**最后一张**页才按下去：页都写完了，下一个要写的是透传文件。
/// 拿它问的是透传那个循环头拦不拦得住。
const AFTER_THE_LAST_PAGE: usize = PAGES;

/// 一个 [`PAGES`] 页的卷。立即停止那几条要它有页可写——写出环节写完第一张才有一格装着东西的 `partial`。
...
/// 在点名那个卷**某一遍的页边界**上按下一个字，并在按下的那一刻看一眼输出目录。
///
...
/// 的 `WatchDuringRun`）。有东西可看的只有写出环节——写出去的页都在那格 `partial` 里。
#[derive(Clone)]
...
struct Pressed {
    /// 当前这一卷是点名的那个吗。
    named: bool,
    /// 点名那个卷走过哪几遍，按到达顺序。「立即停止之后写出环节还开不开工」问的就是它。
    passes: Vec<Pass>,
    /// 正在走的是点名那一遍吗。
    walking: bool,
...
impl StopsAtAPageBoundary {
    ...
    /// 按下那个字的一刻输出目录下有哪些名字。一趟里只看一眼——第一眼。
...
///
/// 这是清点存在的理由：一个卷要走多少步得先枚举它的成员才知道，而枚举原先发生在处理那一卷时——
...
/// 该一次改完，不该被逐个叫回来三趟。
///
/// 坏的那两个是**损坏的归档**（`CONTEXT.md` 的《失败》：卷转换失败列的第一批之一）：
/// 路径不在、扩展名不对那两种更早就被开工前那道去处校验挡下了——它算去处也要看路径
/// （见 `source::planned_output`）。到得了清点的是「路径像个卷、打开才知道不是」这一类。
#[test]
...
/// 这件事怎么测得出来：清点排在开工那条事件**之前**，因此在收到开工事件的那一刻
/// 往源卷里塞一张新页——卷还留在清点手上的话那张页不会被做掉，重开一次就会。
///
...
fn the_pipeline_opens_the_volume_again_by_path() {
    ...
    assert_eq!(
        ...
        3,
        "开工之后才出现的页没被做掉：这一卷没有按路径重开，成员表还是清点那一份"
    );
    assert!(
...
/// 清点**只列成员**：它连页的字节都没读过，更没解过一个像素（会话批 03 号票）。
///
/// 判别式与上一条同一个：清点排在开工那条事件之前。在收到开工事件的那一刻把一张好页
/// 改写成坏字节——清点要是读过（或解过）它，手上就是**改写之前**那份好像素，这一页会照常出来；
/// 而它其实一个字节都没读，管线随后才去读，读到的是坏字节，于是这一页失败。
/// **那一页失败正是证据。**
///
/// 这一条与「按路径再开一次」是一对：那一条问清点**留**下的是不是只有数与路径，
/// 这一条问它有没有顺手多做一步。两条都测得出来，靠的都是「清点在开工事件之前」这个时序。
#[test]
...
///
/// 会话要在确认点上等人拿主意（ADR 0012 决定第 3 条），因此「很久不返回」是观察者
/// **正常**的样子。库这一侧要是在持锁处调它，等人的那一刻别的线程就一起停住了——
/// 而那种停住不会报错，只会看起来像卡死。
///
/// 这条用例买的是那一侧的下限：真在持锁处调它，分析环节那几条计算线程会互相等到超时，
...
fn a_volume_that_never_got_done_is_reported_the_moment_it_fails() {
    let space = Workspace::new();
    // 归档结构完好、中央目录列得出这个透传成员，坏的是它的字节——清点打得开，
    // 真去读才看得出来。那正是「清点之后才做不成」。
    ...
    assert_eq!(failures.len(), 1, "卷转换失败没有报出来：{failures:?}");
...
///
/// 立即停止掉的那一卷既不进报告、也不报「一卷跑完」——它等于没做（ADR 0013 决定第 2 条）。
/// 从前那件事在库的出口上**没有形式**：立即停止掉的卷与「压根没点名它」在 `Report` 上
/// 一模一样，事件流那一侧也只剩「一条没有配对的开卷」要调用方自己去认。这一条把那个形式
/// 钉住：结束那一条带着它，`Report` 上也带着同一个值。
///
/// 三种一起断言，因为分得开才有意义：走到头的那一趟不许说自己停过，
/// 做完再停与立即停止不许混成同一个字——两者停下来的现场不同（前者盘上留着完整的卷，
/// 后者连当前那一卷都丢了）。
#[test]
fn the_last_event_says_how_the_run_ended() {
    ...
    let report = run(&winding_up, space.out_named("做完再停"));
    assert_eq!(
        winding_up.outcome(),
...
fn a_refusal_after_the_run_started_still_says_the_run_is_over() {
    ...
    .expect_err("尺寸未贴合屏幕时点名抖动应当整趟被拒");

    assert!(format!("{error:#}").contains("尺寸贴合检查"), "{error:#}");
    assert_eq!(
        recorder.outcome(),
