...
impl<M: ProcessMode> Internal<M> {
    extern "C" fn callback(
        ...
        match msg {
            native::UCM_CHANGEVOLUMEW => {
                // Scan for the nul terminator instead of assuming a fixed
                // buffer length: `from_ptr_truncate(p, 2048)` builds a 2048
                // *element* slice up front (8 KiB where wchar_t is 32-bit),
                // which is out of bounds whenever unrar's buffer is smaller.
