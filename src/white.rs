//! 纸色提白：量出这一页的纸白，把 `[纸白, 255]` 这一段钳到 255，低于纸白的取值一个都不动。
//!
...
//! 像素，画质分两侧因此同源，量化仍然是唯一被隔离出来的变量（ADR 0002 决定第 1 条）。
//! 取对齐前的参照等于把自己的修正当成误差收费。
//! 彩色分支不经这里（ADR 0010：那条路径既不量化也不抖动，对齐对它没有意义）。
//!
//! **尺寸未贴合屏幕的页照样对齐**：那种页只是没有抖动那一维，纸白该在格点上还是要在。
//!
//! # 手段是钳制近白，不是拉伸也不是平移
//!
...
///
/// **它与《提白上限》不是同一档待遇**：上限是用户调得动的口味取舍，
/// 这个数是那条定义自带的一部分，眼下只有一个调用方（停车场 Q519）。
const MIN_FLAT: u64 = 5000;

/// 纸色提白的**上限**：这一趟最多愿意为对齐压平多宽的色调，单位是灰度级。
///
/// **上限即开关**：取 `0` 时只有「本来就在格点上」的页满足条件，即**一页都不会被改动**，
/// 产物与不带这个功能时逐字节相同。不另做一个布尔开关——那个开关的两个取值这个数都已经
/// 说得出来，多一个就是同一件事的第二个出处，而两处对不上时谁赢又要再定一条规矩。
///
/// **取值只有这一处**（[`Default`]）。别处凡是说到那个数的都是在**描述**它——`--help`、
/// [`Request`](crate::Request) 那一格的文档、`CONTEXT.md` 的《尚未确立》——
/// 抬默认值那一趟改的就是这一处加上那几处措辞，不是又一个取值。
/// **默认值从 05 号票起是 4**（默认开着），取 0 仍是关闭，那是用户点名的事。
...
impl Default for WhiteAlignLimit {
    /// **默认取 4 级，即默认开着**（spec 的《Implementation Decisions》第 11 条第二步）。
    ///
    /// **4 是分布上的一个断口，不是拍出来的数**：全语料普查里钳制宽度**在 4 与 8 之间
    /// 一页都没有**，超出的那几页是「另一批扫描」混进来的。
    /// **那几个百分比不抄在这里**：出处是 measurements 的《全语料普查：四成三的页纸白
    /// 不落在格点上》——抄下来，普查重做那一趟就要多改一处，而没有东西会红
    /// （`CONTEXT.md` 对《细节放宽》立的正是这条规矩）。
    ///
    /// **断口不是人眼**：钳掉几级之后最淡的网点还在不在要在真机上问，
    /// 因此它照旧是**未标定占位值**，与 `K`、`MASKING_FLOOR`、`MASKING_KNEE` 同一档待遇，
    /// 记在 `CONTEXT.md` 的《尚未确立》里。
    ///
    /// 它进参数哈希（见 `crate::metadata`），所以**抬默认值这一趟让上一套输出整卷过期重做**
    /// ——不加参数的用户升级后第一趟要整库重跑一次，那是有意的（spec 第 11 条：
    /// 「当作一次全库重跑处理」），也是幂等这一次该拦住的东西。
    fn default() -> Self {
        Self::new(4)
    }
}
...
/// 短路挡的是**白花的工夫**，守卫挡的是**像素**。**预览那一趟正是拆掉它跑的**——
...
///
/// # 预览那一趟为什么要它（纸色提白批 02 号票第 3 条）
///
/// 逐页那一层是给**还没决定上限取多少**的用户看的，而那个用户按定义上限就是 0——
/// 走 [`align_white`] 的话，那道短路让他每一页都读到「没开」，一个数都拿不到。
/// 预览因此绕开短路另判一遍：`--dry-run` 一个字节都不写，
/// 判一遍的代价是每页一遍平坦掩码，摆在同一页那六档画质分旁边不算什么。
///
