//! 彩页识别：一页有没有彩色内容，分析环节就要问出来（ADR 0005 决定第 1 条）。
//!
...
/// 一页有没有彩色内容。分析环节识别出来（`CONTEXT.md`：彩页）。
///
...
///
/// 判定依据除了幅度还要一道覆盖率：一页灰度扫描上零星几个带色像素，来路是压缩伪影或一枚彩色印章，
/// 不是「这一页要保留颜色」。
///
/// 《B 类素材普查》的「97% 近灰度，3% 彩色」是**分布**，不是判定依据的形态——那一节没有给出
/// 它是怎么数出来的，因此这两个数都不能说成是从它标定来的。
const COLORED_FRACTION: f64 = 0.005;

/// 这一页是彩页还是灰度页。
///
/// 判定依据有两维：单个像素的**色度幅度**要过 [`CHROMA_FLOOR`]，这样的像素要占到
...
mod tests {
    ...
    /// 一枚小面积彩色印章不足以让整页成为彩页：判定依据除了幅度还有一道覆盖率。
    ///
