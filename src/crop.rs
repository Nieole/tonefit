//! 裁白边：按行列墨量占比裁掉页面白边，**在适配之前**。
//!
//! 这件事的要点不是省白边，是让用户**关得掉阅读器那一侧的裁切**。6 寸屏上自动裁白边是刚需，
//! 但阅读器裁掉白边就改变了页尺寸，随后的适配于是不再是 1.0 倍——抖动连同 1 像素周期的
//! 结构一起被抹平，字节照付（见 measurements 的《真机像素完整性》，两台设备都实测过）。
//! 裁白边收进 tonefit 之后好处烤进产物，阅读器那个开关变成空操作，1:1 恢复。
//!
...
//!
//! **裁白边不碰尺寸贴合检查，也没有把 ADR 0003 的硬上界静默关掉。**ADR 0007《后果》要求
//! 「任何改动目标尺寸的改动，都要同时对 ADR 0003 的硬上界负责——两条约束的破裂条件
//! 是同一个」。裁白边改的是**送进适配的那个尺寸**，不是目标尺寸的来源：目标尺寸仍只由
...
///
/// 拆分跨页那一侧也问它（`crate::spread`）：中缝检测与裁白边是同一类扫描，
/// 「什么算墨」不该有第二套答案。
...
///
/// 与 [`is_ink`] 同一个用意：占比那条线只有一个出处。拆分跨页拿它判「这一列是不是空白」
...
/// 这条守卫也接不住它。
///
/// 顺带得到的是一条界：留下的宽高各不少于原页的一半，裁白边因此**至多把目标宽放大 2 倍**。
/// 越过兜底上界的必定是源页本身的形状，不会是裁白边推过去的。
const MIN_KEPT: f64 = 0.5;

/// 裁法那两个数，报告抬头原样印出。
///
/// 与画质分那一侧的 [`aggregation`](crate::aggregation) 同一条规矩：数摆出来，来源跟着摆出来，
/// 读的人自己判断它对手上这批素材成不成立。
...
impl std::fmt::Display for InkRule {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "按每行每列的墨量裁 ⋅ 灰度低于 {} 算墨 ⋅ 墨点占 {:.1}% 以上的行列算有内容",
            self.ink,
            self.fraction * 100.0
        )
    }
...
/// 裁白边关着的那一趟是一个原样通过的 [`Crop::keeping_all`]，
/// 「这一趟没裁」与「这一页没什么可裁」因此在报告里长得一样，
/// 分辨它们靠抬头那一行（裁白边开着还是关着）。
...
impl Crop {
    ...
    ///
    /// **裁白边那一侧一个都不用**——白边窗口只由 [`Crop::of_gray`] 与 [`Crop::of_color`]
    /// 量出来。生产路径上用它的只有拆分跨页（`crate::spread`）：切出来的那一半
    ...
    ///
    /// - 裁白边**关着**（`--no-crop`）。
    /// - **残缺页**。它缺的那一段留成纸白（`CONTEXT.md` 的《失败》），
    ...
    /// 与尺寸贴合检查那一条同一个理由（`GeometryGate::of`：本仓库唯一一处判定尺寸贴合检查的地方）。
    ///
    ...
    /// 出来的仍是一个长在原页上的窗口。
    ///
    /// 拆分跨页要它（页几何批 04 号票）：整页裁一次、切成两半、每半再裁一次，
...
impl std::fmt::Display for Crop {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "裁白边 {} ⟶ {}", self.before, self.after)
    }
}
...
/// 中间的空档原样留着：页当中一片浅色（天空、留白的格子）不是白边，裁白边只从外面往里收。
...
mod tests {
    ...
    /// 页当中的浅色不是白边：裁白边只从外面往里收，中间的空档原样留着。
    #[test]
