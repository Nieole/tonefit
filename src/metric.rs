//! 画质分：**两项之和**，分块聚合取上分位与「第 K 差的那一块」之间更严的那个（ADR 0002）。
//!
//! - **整体走样项**：参照与候选各做低通之后的局部均值误差。量的是灰调塌陷与 banding。
//! - **抖动颗粒项**：候选比参照多出来的高频起伏里，**超出颗粒可见下限**的那一部分。
//!   量的是抖动颗粒自身有多显眼。
//!
//! 只有整体走样项时画质分在同一页内把序排反：误差扩散的定义就是把量化误差摊进邻域、保住局部均值，
//! 而整体走样项量的正是那个量，于是结构性地偏袒抖动、看不见颗粒（见 measurements 的《位深盲测》）。
//! 补上抖动颗粒项不是把低通拆掉——低通仍是对的，逐像素度量在「该不该抖」这一维上符号仍然是反的
//! （《抖动》）。**任何逐像素度量都不得单独作为候选之间的选择依据。**
//!
//! 画质分是量，画质门槛是界。这里只出量：界在 `profile`，拿量去和界比在 `decide`。
...
///
/// 低通核由面板 PPI 推出，**画质分数值不可跨面板比较**（ADR 0002）：换面板即换核，
/// 同一个数在两块面板上不是同一件事。
...
/// 参照：缩放到目标尺寸后、未经目标灰阶档位量化的图，工作精度 8 位灰度（`CONTEXT.md`）。
///
/// 建的时候要给面板，因为低通核由面板 PPI 推出。一张参照要与好几个候选各比一遍，
/// 参照这一侧的低通、细节放宽加权与高频起伏因此都在这里算一次就存下——
/// ADR 0002 认下的代价就是画质分贵。
...
impl Reference {
    ...
    pub fn new(panel: Panel, image: GrayImage) -> Self {
        ...
        // 细节放宽的细节度量在**块这个尺度**上（见 [`masking_weight`]），与低通核那一层不同，
...
/// 抖动颗粒项：候选比参照多出来的高频起伏，减去颗粒可见下限，负的算零。
///
/// 减参照那一份，是因为线稿与网点自带高频——候选把它照搬过来不是新长出来的颗粒。
/// 减地板，是因为高频起伏低到一定程度就真的看不见：抖动把误差摊到眼睛分不开的尺度上，
/// **那一段是它该得的便宜**，画质分不收。收的是超出去的那一截。
///
/// 地板由调用方按灰阶档位算出（[`Composition::floor`]）：它是格点间距的一个比例，
/// 而这里一块一块地算，灰阶档位在整页上只有一个。
...
/// 把加减并成一句 `sum += add - subtract` 就少了一次舍入，画质分会静默地漂。
...
/// 那种错既不越界也不报错，只让画质分整体偏一点点。`debug_assert` 把这个契约摆出来。
///
...
/// 画质分由哪几项构成（ADR 0002 决定第 5 条）。报告要说得出逐页那一行的数是怎么来的。
///
/// 与 [`Aggregation`] 分工：那个说的是**块的读数怎么收成一个数**，这个说的是
/// **一块的读数本身由什么组成**。两者都是画质分的形状，但不是同一层。
#[derive(Debug, Clone, Copy, PartialEq)]
pub struct Composition {
    /// 抖动颗粒项那道颗粒可见下限**占格点间距的比例**。地板本身由它乘间距算出，
    /// 见 [`Composition::floor`]。
    pub grain_ratio: f32,
}

impl Composition {
    /// 这一档灰阶档位上的颗粒可见下限，8 位灰度级。低于它的高频起伏当作看不见。
    ///
...
impl std::fmt::Display for Composition {
    /// 地板按**比例**说，连同它的来源一并说出——它与画质门槛同一批盲测标定，读的人要判断得了
    /// 这个数对手上那块面板成不成立（与 [`Threshold`](crate::Threshold) 同一个做法）。
    ///
    /// 说比例而不说某一档的那个绝对值：画质分一页要排开好几档灰阶档位，
    /// 说死其中一档的数，另几档那几个数就没有出处了。
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "整体走样 ＋ 抖动颗粒（起伏不到相邻两级灰度差的 {:.1}% 不算；这个比例在 boox-poke6 上实测，其他屏幕未验证）",
            self.grain_ratio * 100.0,
        )
    }
}

/// 这一档灰阶档位的**格点间距**，8 位灰度级：`255 / (2^n − 1)`。
///
...
/// 分块取分（ADR 0002 决定第 3 条）：块边长绝对，尾巴按比例走但永不宽于 K 块。
///
...
/// 本次画质分用的聚合。三个数眼下对所有 profile 都一样。
...
impl std::fmt::Display for Aggregation {
    /// 形状连同「K 还没标定」一并说出——画质分那一栏的每一个数都是这个形状算出来的，
    /// 不说，读的人无从判断该信到什么程度（与 [`Threshold`](crate::Threshold) 同一个做法）。
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "切成 {}x{} 的小块分别打分 ⋅ 一页的分数看最差的 {}% 小块，但最多只看最差的 {} 块（{} 块这个数是暂定值）",
            self.tile,
            self.tile,
            100.0 - (self.quantile * 100.0).round(),
            self.tail_tiles,
            self.tail_tiles,
        )
    }
...
/// 颗粒颗粒可见下限占**格点间距**的比例。高频起伏低于地板就当作看不见。
///
/// **地板是间距的一个份额，不是一个绝对值**（ADR 0002 决定第 5 条）。一个绝对的 55
/// 卡在 1bit 与 2bit 之间，抖动颗粒项在 2bit 及以上恒读零、画质分在那些档上退回只剩整体走样项
...
///
/// 它跟着面板走，与画质分、画质门槛同一条（ADR 0002）：换面板即换低通核，
/// 「哪一段算高频」跟着变，这道地板也就不是同一件事。
const GRAIN_RATIO: f32 = 0.215_686_27;

/// 细节放宽加权的地板：结构再密也不至于完全不看。**未标定占位值**。
///
/// 细节度量法换到块尺度之后细节度整体变大，这个数跟着一起重定
/// （窗口见 measurements 的《位深盲测》）。
const MASKING_FLOOR: f32 = 0.5;

/// 细节放宽加权的中点细节度，8 位灰度级。块内细节度到这里，加权正好落在不打折与地板的中点。
/// **未标定占位值**。
const MASKING_KNEE: f32 = 8.0;

/// 细节放宽（ADR 0002 决定第 4 条）：一块的读数**怎么加权**。
///
/// 与 [`Composition`]、[`Aggregation`] **平级**，三者是画质分形状的三件事，不是同一层：
/// 那个说**一块的读数由什么组成**，这个说**同一个读数落在有结构的块上该打几折**，
/// [`Aggregation`] 说**块的读数怎么收成一个数**。
///
/// 两个数摆在一处，与 [`Aggregation`] 同一个理由：读它们的两端要的是同一件事——
/// 报告要把它们标成未标定占位值，本模块算一块的细节放宽加权要按它们算。
...
pub struct Masking {
    ...
    pub floor: f32,
    /// 中点细节度，8 位灰度级：块内细节度到这里，加权正好落在不打折与地板的中点。**未标定占位值**。
    pub knee: f32,
}

/// 本次画质分用的细节放宽。两个数眼下对所有 profile 都一样。
...
impl std::fmt::Display for Masking {
    /// 形状连同「这两个数都还没标定」一并说出——画质分那一栏的每一个数都经过这道加权，
    /// 不说，读的人无从判断该信到什么程度（与 [`Aggregation`] 同一个做法）。
    ///
    /// 说的是加权曲线上**说得出名字的那几处**（平坦处不打折、地板压不下去、中点细节度在两者中点），
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "大片平涂处扣分不打折 ⋅ 细节越多扣分打折越多，但总保留 {:.0}% 以上 ⋅ 细节度 {:.1} 时折扣走到一半（两个数都是暂定值，还没用真机校准）",
            self.floor * 100.0,
            self.knee,
        )
    }
...
/// 细节放宽细节度量在多大的尺度上——**与分块边长同一个数**。
///
/// 细节放宽要防的是「参照本身有结构的地方，同样的偏移看不出来」，而画质分的读数按块出，
/// 那个「有没有结构」问的就该是块内的事。核尺度（2~4 像素）只看得见网点残留那一层，
/// 看不见线稿与画面构成——真机盲测正是在这里出的岔：画集 040 的坏块贴着画上的边，
/// 056 的坏块泡在一片没有内容的平滑渐变里，核尺度的细节度把两者读成一样的
/// （见 measurements 的《位深盲测》里 4bit 不抖那一条）。
///
/// 取局部均值而不是块内均值，是为了让**平缓斜坡透明**：box 均值不改变线性斜坡，
/// 斜坡因此不贡献细节度——而斜坡恰恰是 banding 最显眼的地方，它不该给自己买到细节放宽。
const STRUCTURE_KERNEL: u32 = TILE;

/// 一块的细节放宽加权：平坦低对比区不打折，有结构的区域放宽（ADR 0002）。
/// 崩掉的从来是灰调，不是线稿。
///
/// 细节度取块内「原值离**块尺度**局部均值有多远」的均值（见 [`STRUCTURE_KERNEL`]）。
/// 加权只由参照定、与候选无关：否则抖动候选会拿自己的高频噪声给自己放宽，
/// 而画质分恰恰是要在「该不该抖」上说话的。
///
/// 加权是**相对**的：平坦区取 1.0 作基准、不打折，有结构的区域才打折。整体乘一个常数会被
/// 画质门槛标定原样吸收，能改变判定的只有两类区域之间的比。
///
/// 地板与中点细节度**两个都是未标定占位值**（`CONTEXT.md` 的《尚未确立》，measurements 的
/// 《位深盲测》）：中点细节度从来没标定过；地板的现值是量法换到块尺度那次跟着重定的，
/// 那一次夹的是窗口、不是一次标定（见 [`MASKING_FLOOR`]）。打折太狠时线稿密的块会被压到
/// 看不见，而灰调真崩在那种块上时画质分就读不出来了（1bit 不抖动在网点页上正是这种块）。
///
...
/// 一块，连同它从参照上取到的细节放宽加权与参照自己的高频起伏。
struct WeightedTile {
    tile: Tile,
    weight: f32,
    /// 参照在这一块上的高频起伏。抖动颗粒项减掉的就是它。
    grain: f32,
}

/// 画质分的聚合单位。边上不足一块的按实际像素数算。
struct Tile {
    x: u32,
    y: u32,
    width: u32,
...
impl Tile {
    ...
    /// 块内的细节放宽细节度：像素离**块尺度**局部均值有多远（见 [`STRUCTURE_KERNEL`]）。
...
fn aggregate(values: &mut [f32]) -> f32 {
    ...
    // 升序排开，秩越靠后取到的块越差、画质分越严：两个秩取靠后的那个，就是取更严的那个。
...
///
/// 画质分的分块聚合、整卷统一灰阶与差异大的页画质分的立脚点（后两者见 `crate::envelope`）共用它。
...
mod tests {
    ...
    /// 抖动颗粒项减的是**参照**那一份，不是候选那一份：两个同型参数换个位置，
    /// 画质分会安静地把「候选新长出来的颗粒」读成「候选抹掉的颗粒」，方向恰好反过来。
    #[test]
    ...
    /// 而窗口的加减一旦错开一格，画质分整体只是**略微**偏一点——性质测试全绿，
    ...
    /// 分带扫列与逐列扫**逐位相同**。
    ///
    /// 画质分的读数最终要与画质门槛比大小，而 `f64` 的加法不结合：把一列的加减并成一句、
    /// 或者换一个折叠次序，画质分都只是**略微**偏一点——性质测试全绿，黄金快照整片挪几个
    ...
    /// 不该动折叠次序——单精度的连加换个次序就漂，而漂出来的画质分只是**略微**偏一点：
    ...
    /// 颗粒可见下限的下界是**算出来的**，不是调出来的：低于它，「同一档上抖动优于不抖动」
    /// 这条性质会在某个灰调上翻掉——而那正是 ADR 0002 立画质分时守的那一条。
    ///
    /// 推导写在 [`GRAIN_RATIO`] 的文档里。这里把它当成一条算术不变量钉住，
    /// **三档各钉一次**：谁把比例调到 0.2071 以下，三档里任何一档当场红。
    /// 地板还是绝对值时这一条只管得着 1bit——另两档上抖动颗粒项恒读零，钉什么都是白钉。
    ///
    /// 断言只管抖动颗粒项这一项。整个画质分上抖动那一侧还要背自己的整体走样项——
    /// 平坦的中浅灰上两者因此仍会交叉，代价写在 ADR 0002 的《后果》里。
    #[test]
    fn the_grain_floor_stays_above_what_an_undithered_flat_tone_pays() {
        // 与最近格点差 u 的一块平坦灰调（`u ≤ s/2`）：不抖动的整体走样项读 u，
    ...
    /// 细节放宽那两个数由 [`masking`] 一处出：报告读的那一份，就是加权真正用的那一份。
    ///
    ...
    fn the_masking_weights_come_from_the_one_place_the_report_reads() {
        let Masking { floor, knee } = masking();
        // 单精度上比不得逐位相等：加权那一步是 `(1−地板)·中点细节度/(中点细节度+细节度)`，
        // `(a·k)/k` 只在 k 是 2 的幂时恰好还原 a，而中点细节度标定之后未必还是。
