...
pub(crate) enum Stage {
    ...
    /// 整卷的解压加整卷的写盘，单独摘出来必然改变前三大开销的名单（本类型开头那条判据）。
    ...
    /// 喂源哈希。读字节那一截不在里面。
    ///
    /// 两遍都喂、两个作用域都喂：幂等那一道喂**卷级**那一份（整卷一个数），并趁字节在手上给
    /// 每个源页喂一份**页级**的拿去按页比（two-pass-rework/14）；第一遍重做的那些页再喂一次
    /// 页级的写进记录（two-pass-rework/13）。同一件事、两个作用域，记在同一格——
    /// 它答的是「哈希花了多少」，不分给谁。
    Hash,
    /// 解码到像素缓冲。
    Decode,
    ...
    ///
    /// 两种装法都记在这一格：上包络那条路上第一遍参照进缓存，默认路径与覆盖顶死那一趟
    /// 第一遍就装编好的字节（`PageCache::insert_encoded`）。后一种不压——那已经是一页 PNG。
    CacheIn,
    /// 一页出缓存：取回 + 解压。
    ///
    /// 第二遍取回来写出。默认路径上取回的是编好的字节，那一次不解压。
    CacheOut,
    /// 定下来那一档的量化。判据里那六次不在这里。
    Quantize,
    /// PNG 编码。彩色分支上的那一次也算在这里。
    Encode,
    /// 写进输出容器。留下的页从上一趟的输出里读回、改写记录那一项再写进去，那一趟往返也算在这里
