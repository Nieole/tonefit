...
//!
//! **它不是《卷级计时》。**卷级计时按**段**分（幂等这一道 / 分析环节 / 写出环节），是互不重叠的
//! 墙钟，进 [`crate::Report`]；本模块按**阶段**分——解码、转灰、裁白边、缩放、画质分、量化、
//! 编码这几步，只印到 stderr。两者一个是产品输出，一个是量具，不许合流
//! （`CONTEXT.md` 的《管线》，加固批 13 号票明禁把阶段计时塞进报告）。
//!
//! # 这个数是什么
//!
//! **各线程墙钟之和**，不是 CPU 时间。分析环节是满核并行的，一条计算线程在某个阶段里
//! 被调度出去的那一截也算在内。因此：
//!
...
pub(crate) enum Stage {
    ...
    ///
    /// 一卷走一次，而且只有摊开那一档的卷（`.rar` / `.7z`）走——排一格是因为它一次就吃掉
    /// 整卷的解压加整卷的写盘，单独摘出来必然改变前三大开销的名单（本类型开头那条判定依据）。
    /// 摊开之后那几步与目录卷一模一样，[读字节](Self::Read)因此照旧只记读，不记这一笔。
    Extract,
    /// 取源字节。读取端是一个目录时两路都可能并发（目录卷，以及摊开了的卷）；
    /// 是一个归档句柄时两遍是一条顺序扫，幂等那一道各开各的句柄
    /// （见 `crate::medium::IoPlan` 的《为什么是两路》）。
    Read,
    /// 喂源哈希。读字节那一截不在里面。
    ///
    /// 只有幂等那一道喂，按这一趟的作用域喂一种（two-pass-rework/15）：`--envelope` 那条路喂
    /// **卷级**那一份（整卷一个数），默认路径趁字节在手上给每个成员各喂一份**页级**的——
    /// 拿去按页比，也是分析环节盖进记录的那一份（分析环节不再喂第二遍，停车场 Q686）。
    Hash,
    /// 解码到像素缓冲。
    Decode,
    /// 彩页识别。
    ...
    ToColor,
    /// 裁白边：量墨 + 取窗口。切开之后每半再裁的那一趟也算在这里。
    Crop,
    /// 判跨页与切开。
    Split,
    /// 缩放到目标尺寸（含整数倍预缩）。
    Resize,
    /// 画质分：建参照（低通、细节放宽加权、参照自己的高频起伏），
    /// 再在六个候选上各量化一遍、各求一次画质分。
    Metric,
    /// 一页进缓存：压缩 + 记账（溢写的话还有那一次写盘）。
    ///
    /// 两种装法都记在这一格：整卷统一灰阶那条路上分析环节参照进缓存，默认路径与覆盖顶死那一趟
    /// 分析环节就装编好的字节（`PageCache::insert_encoded`）。后一种不压——那已经是一页 PNG。
    CacheIn,
    /// 一页出缓存：取回 + 解压。
    ///
    /// 写出环节取回来写出。默认路径上取回的是编好的字节，那一次不解压。
    CacheOut,
    /// 定下来那一档的量化。画质分里那六次不在这里。
    Quantize,
    /// PNG 编码。彩色分支上的那一次也算在这里。
    Encode,
    /// 写进输出容器。留下的页从上一趟的输出里读回、原样写进去，那一趟往返也算在这里
    /// （two-pass-rework/14）：它是「写出」的一种，不是编码。
    Write,
    /// 汇总：整卷统一灰阶、迟滞、特例。
    Summarize,
...
///
/// 走到这里的只有跑完与被按停止两种结束。**拒绝开始那一路不印**——那一趟一页都没做
...
mod tally {
    ...
    /// 一格一个阶段，装纳秒。原子加：分析环节是满核并行的，每条计算线程都往里写。
    ///
...
mod tests {
    ...
    /// 一格都没碰过就**一个字都不印**。
    ///
    /// 拒绝开始那一路走到的正是这一种（`crate::run` 一页都没做）：那时印一张全是零的表，
