...
/// 有卷**没做成**、或有地方**走不进去**时的退出码
/// （05 号票：卷级失败；`p4-parking-lot/11`：走不进去的地方）。
///
...
///
/// **走不进去的地方也走这个数**，而不是新开第五个（`p4-parking-lot/11`）：
...
///
/// 它**不进** `ISOLATED_EXIT`：那一个说的是「卷交出来了，只是带着占位页」，
/// 而走不进去的那一块连交都没交。
...
/// 这一趟的退出码：全部成功是 [`SUCCESS_EXIT`]，有卷没做成**或有地方走不进去**是
...
fn exit_code(report: &Report) -> u8 {
    // **走不进去的地方与卷级失败并排问**，两者交出同一个数：为什么不新开第五个，
    // 见 [`FAILED_VOLUME_EXIT`]。它排在隔离前面，理由与卷级失败那一条同一句——
