...
struct Cli {
    ...
    /// 输出目录。产物按源的结构镜像到它下面，处理路径自己的名字打头；
    ...
    /// `--envelope`／`--no-envelope`），预设把裁白边关掉之后，这一趟用 `--crop` 把它开回来。
    ///
    /// 预设**不装处理范围与输出目录**——那两样每趟都不同，混进去会让人套用预设时误写到上一次的
    ...
    /// 每个预设两节：`[preset."名字".device]` 装型号、可见灰阶数与画质门槛，
    ...
    ///
    /// 点了 `--preset` 而那份预设的设备设置供了型号时，这一项可以不填。
    #[arg(short, long, required_unless_present = "preset", value_name = "型号")]
    profile: Option<String>,

    /// 覆盖屏幕灰阶数。内置表没收录的设备、或在真机上数出的实际可分辨级数走这里。
    #[arg(long, value_name = "级数")]
    gray_levels: Option<u32>,

    /// 覆盖判定用的画质门槛。内置值由一块面板上的真机盲测定出，**没有逐面板复核**——
    /// 画质分跟着面板走、不可跨面板比较（ADR 0002）。在自己那台设备上盲测出来的界填这里：
    /// 拿 `--dry-run` 逐页读出各档画质分，把同一页的各档拷进设备目视，
    /// 记下最低的那个**不可接受**档的画质分，界取在它之下。
    ...
    ///
    /// 它与别的开关咬在一起时会怎样，见 `--help` 末尾的《选项冲突》。
    #[arg(long, value_name = "方式")]
    fit: Option<String>,

    /// 不裁掉页面白边。**默认是裁的**：tonefit 自己按行列墨量占比逐页裁白边。
    ///
    /// 裁白边的要点不是省白边，是让你**关得掉阅读器那一侧的裁切**：
    /// tonefit 裁完之后好处已经烤进产物，阅读器那个开关变成空操作，1:1 恢复。
    /// 留着它要付什么，见下面那句指路。
    ///
    /// 裁法按**行列墨量占比**，不是内容外接框：白边里的孤立噪点不算内容，
    /// 边缘一个墨点不会让裁白边整个失效。逐页各裁各的，**页与页的字号因此会跳动**——
    /// 那是要更大实际利用面积的代价，不是缺陷。整页空白的页原样通过。
    ///
    /// 它与别的开关咬在一起时会怎样，见 `--help` 末尾的《选项冲突》。
    ///
    /// **反面是 `--crop`**：预设把裁白边关掉之后，这一趟用它开回来。
    ...
    /// 不把跨页拆成两页。**默认是拆的**：tonefit 在中缝上把跨页切成两半，每半用满面板高。
    ///
    /// 拆开之后每半基本不必横向翻动（实测三卷双页片源的屏占比中位从 1.88–1.91 落到 0.88–0.92），
    /// 而**缩放系数完全相同**——拆分不是拿分辨率换的。
    ///
    /// 判定分两级，两级都只看几何与墨量，不看画面语义：先按宽高比与面板比挑出跨页候选
    /// （判定宽度走 `--split-threshold`），再在跨页候选里找中缝定切点。**切点跟着中缝走，
    /// 不切正中**——实测沟中心在 0.401–0.538 之间，按正中盲切最偏的一页会切进画面近一成页宽
    /// （8K 跨页上是 599 像素）。
    ///
    /// **找不到中缝的是连续跨页，不切**：那是为视觉效果画满两页的一幅整画，切开就毁了。
    /// 不切的页照这一趟的缩放方式出——默认 height 下页面比屏幕宽，靠阅读器横向平移看。
    /// 关掉本项，所有跨页都走那条路。
    ///
    /// 它与别的开关咬在一起时会怎样，见 `--help` 末尾的《选项冲突》。
    ///
    ...
    /// 判定宽度与先后仍走 `--split-threshold` 与 `--reading-order`，本项不碰它们。
    #[arg(long)]
    split: bool,

    /// 跨页判定宽度：页宽高比要有面板宽高比的这么多倍才算跨页候选，默认 1.5。
    ///
    ...
    /// 可选 area（= box）、bilinear、hamming、bicubic、lanczos3，默认 lanczos3。
    /// 要缩小一半以上时先按整数倍缩一次（那一步恒为 box），本项只管之后剩下的那一步。
    #[arg(long, value_name = "缩放算法")]
    filter: Option<String>,

    /// 纸色提白的上限：最多把多宽的一段近白色调压平到纯白，**默认 4 级，即默认开着**。
    ///
    /// **取 0 就是关闭**：那时只有纸白本来就落在 255 上的页满足条件，即一页都不改，
    /// 产物与不带这个功能时逐字节相同。不另设一个开关——上限即开关。
    ///
    /// **这个数改了，上一趟的输出整卷过期重做**：它进参数哈希，改了不会被当成「之前转换过」悄悄跳过。
    ///
    ...
    /// 覆盖自动判定的灰阶档位：1、2、4、8。屏幕灰阶数那道上界仍在，越界的覆盖会被拒绝。
    #[arg(long, value_name = "灰阶档位")]
    bit_depth: Option<u32>,

    /// 覆盖自动选择的抖动模式：off（= none）、fs（= floyd-steinberg）。
    /// 抖动只在输出不被下游缩放时才谈得上，而这是**每一页**各自的事实——
    /// 点名 fs 因此会撞上尺寸贴合检查，那条冲突见 `--help` 末尾的《选项冲突》。
    /// 错误指得出是哪一页，不会静默照抖。
...
/// 与命令行上的 `--no-crop` 是同一件事，而预设把裁白边关掉之后这一趟用 `--crop` 开得回来——
/// 覆盖因此是对称的。
///
...
impl Cli {
    ...
    /// 本次开不开整卷统一灰阶（ADR 0018 决定第 5 条）。**默认不开**，`--envelope` 打开它、
    ...
    ///
    /// 不点名就是默认那一套：拆、判定宽度 1.5、右开。三项各自的默认在库那一侧
    ...
    /// 本次残差段用哪个缩放算法。不点名就是默认的 lanczos3（ADR 0001）。
    ...
    /// 本次纸色提白的上限（纸色提白批 01 号票）。命令行点了名就是那个数，没点就是预设说的，
    ...
    /// 本次要不要点名抖动模式。不点名就由画质分在尺寸贴合检查放行的那几种里选。
...
/// 两者都带着「选项冲突」四个字，而短帮助里该有的只有路标。
const INTERLOCK_HEADING: &str = "选项冲突（几项凑在一起会互相削弱的那几种组合）:";

/// 输出目录走到这里就一定有值：`required = true` 挡在 clap 那一层。
///
/// 字段类型仍是 `Option`——`subcommand_negates_reqs` 让子命令那一路不必交出它
/// （`calibrate` 根本不收输出目录与卷），字段就得容得下「没有」。
...
enum Command {
    /// 生成灰阶测试图：一次上机答两件事——像素有没有原样贴上、还分得开几级灰。数出的是可见灰阶数，不等于屏幕的物理灰阶数。
    ///
    ...
    /// 四角压在第 0 行列与末行列上的直角标记。下半是并排的各候选灰阶档位阶梯，每一级标着自己的号。
    ///
    ...
    /// 它在画质分之前裁掉候选灰阶档位）。
    ///
    /// **数出来的是可见灰阶数，不等于屏幕的物理灰阶数。** 两者不必相等——
    /// 显示固件的处理、环境光、观看距离都会改变你数得出几级，而 `--gray-levels`
    /// 填的正是前者：判定要贴合你**实际看到**的效果，不是贴合规格表。
    ///
    /// 灰阶测试图本身不经过灰阶档位判定：它是量具，不是被处理的页。像素以 8 位工作精度画出，
    /// 无损写出，不带自描述元数据。
    Calibrate {
        ...
        /// 覆盖屏幕灰阶数。上一趟数出来的级数填这里，图跟着只排这台设备真会用到的那几档。
        #[arg(long, value_name = "级数")]
        gray_levels: Option<u32>,

        /// 灰阶测试图写到哪个文件。父目录不在就建出来。
...
/// 有卷被隔离时的退出码。
///
/// 与「拒绝开始」分成两个数（12 号票：退出码要分得开「全部成功」与「有卷被隔离」）：
...
/// 有卷**没做成**、或有地方**无法访问**时的退出码
/// （05 号票：卷转换失败；`p4-parking-lot/11`：无法访问的地方）。
///
/// 与 [`ISOLATED_EXIT`] 分成两个数，理由与当初把它从「拒绝开始」里分出来的是同一条：
/// 脚本要分得开。隔离过的那一卷**交出来了**——输出齐着、页数齐着，只是其中几页是空白占位页，
...
///
/// **无法访问的地方也走这个数**，而不是新开第五个（`p4-parking-lot/11`）：
/// 上面那一段问的是「用户下一步该去查什么」，而两者的答案逐字相同——文件还在不在、
/// 盘还挂着没有、权限变没变。交出的东西也一样：那一块**一个字节都没交出来**，
/// 而其余卷照常跑完。分成两个数买不到任何一个脚本分得开的决定，
/// 却让「四个码各说一件事」变成五个（`CONTEXT.md` 的《失败》）。
///
/// 它**不进** `ISOLATED_EXIT`：那一个说的是「卷交出来了，只是带着空白占位页」，
/// 而无法访问的那一块连交都没交。
const FAILED_VOLUME_EXIT: u8 = 3;

/// **这一趟没做成**时的退出码：拒绝开始，以及别的任何一种当场返回 `Err` 的结束
/// （`CONTEXT.md` 的《失败》）。
///
/// 它就是 `ExitCode::FAILURE` 那个数，写成常量是为了让会话那一路取得到它——
/// 会话在拒绝开始之后**不退出**（把话画出来当场改，见 spec 的《卷转换失败与退出码》），
...
/// 这一趟的退出码：全部成功是 [`SUCCESS_EXIT`]，有卷没做成**或有地方无法访问**是
/// [`FAILED_VOLUME_EXIT`]，有卷被隔离是 [`ISOLATED_EXIT`]。
///
/// 拒绝开始那一种走不到这里——它在 `run` 里就返回了 `Err`，退出码是 `ExitCode::FAILURE`（1）。
/// 四个数因此各说一件事：做完了、做完了但有卷带着坏页、做完了但有东西根本没做成
/// （有卷没做成，或有一块没看过）、这一趟没做成（`CONTEXT.md` 的《失败》）。
///
/// **次序就是取舍**：一个进程只交得出一个数，两件事同时成立时报更重的那一件
/// （05 号票的验收）。为什么那两件是两个不同的决定，见 [`FAILED_VOLUME_EXIT`]。
///
/// 按停止停下来的那一趟**不在这里露面**：它是用户自己的决定，不是失败，退出码照旧
/// （`Report::outcome` 说得出它，见 `tonefit::RunOutcome`）。命令行这一路**按得出来**了
/// （`Ctrl-C` 一次做完再停、两次立即停止，见 [`install_the_stop_key`]），而这个 `match` 一格没变——
/// 本决定「不新开第五个数」，按停止停下来的那一趟交出的仍是它做到的那一步该交的数。
///
/// 出的是 `u8` 而不是 `ExitCode`：后者不可比较，这条规则也就测不了，
/// 而「退出码分得开这几种」正是本票要钉住的那一条。
fn exit_code(report: &Report) -> u8 {
    // **无法访问的地方与卷转换失败并排问**，两者交出同一个数：为什么不新开第五个，
    // 见 [`FAILED_VOLUME_EXIT`]。它排在隔离前面，理由与卷转换失败那一条同一句——
    // 「有一棵子树根本没看过」比「有卷带着坏页」更该让人停下来看。
...
fn execute() -> Result<u8> {
    ...
    // **两级停止的那个键**（ADR 0013 决定第 3 条）。装在这里，两头各有一条理由：
    // 会话那一岔早在函数头上就让开了，而**清点在 `run` 里面**——装晚一步，
...
///
/// 这一趟不读源、不写输出目录、不判定任何东西：灰阶测试图是量具，管线一整套都不在场。
...
/// 命令行这一侧的**闩**：用户按停止时按到过的最强那一级（ADR 0013）。
///
/// 「闩」这个说法出自 `CONTEXT.md` 的《进度》（「按停止是个闩」）。**这是第三份**，
...
/// 退出走立即停止，见 `session::run::Running::leave`），而命令行这一路根本没有「退出」这件事
/// ——按停止就是这一头唯一要按的东西。会话那一岔在 [`execute`] 函数头上就让开了，
/// 这个函数在那之后才被叫到，那一路一个字都动不到。
///
/// **按下之后屏上那一句由这里说**（本票的验收第 4 条），不由 [`Bar::observe`] 说：
/// 那一头要等**下一条事件**才被调到，而清点那一段一条事件都没有
/// （见 [`Bar::survey`]：那时进度这一层收不到任何事件，转轮是自己转的）。
/// 几十个归档卷列归档头正是最长的那一段，在那里按下去而屏上一声不吭，
/// 用户必然当它没收到、再按一下——而再按一下就是立即停止。
...
///
/// **装不上就照旧硬杀**，这一趟不因此拒绝开始：那时的行为与本票落地之前逐字相同，
...
///
/// 做完再停那一句把**下一下是什么**也说了：做完再停要等当前卷跑完，那可能是几分钟，
...
fn pressed_note(pressed: Instruction) -> &'static str {
    match pressed {
        ...
        Instruction::Finish => {
            "按停止（做完再停）：当前卷跑完就停，剩下的卷不开工。再按一次是立即停止——当前卷丢掉，立刻停。"
        }
        Instruction::Abort => {
            "按停止（立即停止）：当前卷丢掉，立刻停。那一卷等于没做，最终位置上一个字节都没动过。"
        }
    }
}
...
///
/// 屏上最多三行，按这一趟走到哪儿依次登场：**清点**那一条转轮、**整趟**那一条、
/// **当前卷**那一条。一卷走完抹掉那一卷的，整趟走完全抹掉——几十卷跑下来，
/// 屏幕上留下的只有报告，**加上按停止时按到过的那一两行**（[`install_the_stop_key`] 印的，
/// 走的是本类型手上这个 [`MultiProgress`]）。留着一排走完的进度条是另一种噪声，
/// 而「这一趟做了什么」报告说得更清楚；按停止那一两行反过来非留不可——
/// 报告那一份此刻说不出这一趟是被按停止的（停车场 Q260）。
struct Bar {
    ...
    /// 开工之前那一段的转轮（03 号票）。
    ///
    /// 它在 `run` **之前**就起来了，`RunStarted` 一到就收掉：清点排在那条事件之前
    /// （见 `tonefit::Event::RunStarted`），因此库这一侧报不出「清点开始了」——
    /// 那一段的交代只能由这一层自己给。几十个归档卷在慢盘上要列一阵归档头，
    /// 而静默的空白与卡死在屏幕上分不开。
    ///
    /// 它盖住的**不止清点**：开工前那几道检查（**拒绝开始**里**清点之外、要摸文件系统**
    /// 的那几道，单子在 `CONTEXT.md` 的《失败》）也在这条事件之前。措辞因此把两件事都说了
    /// ——只说「清点」的话，被那几道检查拒掉的那一趟屏上就写着一件根本没发生的事。
...
impl Bar {
    ...
    /// 每一条都收掉，一行不留。
    ///
    /// 也挂在 [`Drop`] 上：**开工之前**被拒的那一趟（清点发现坏路径就是其中一种）
    /// 一条事件都不发，结束那一条因此根本不来，而屏上那条转轮还转着。
    ///
    /// 开工**之后**才撞上的那一种拒绝照发结束（见 `tonefit::Event::RunFinished`），
...
impl Progress for Bar {
    ...
    /// （ADR 0011 的《后果》）。确认点那一条落在这一支上，而它带的那份卷报告因此一个字节都没人读
...
";
    ...
    /// **预设点的画质门槛与命令行点的印出同一句话**（`p2-loose-ends/10` 的验收：
    /// 三个入口下印出来的话都对）。
    ///
    ...
    fn a_threshold_from_a_preset_says_what_the_command_line_says() {
        ...
        assert!(
            from_preset.threshold().to_string().contains("由你指定"),
            "{}",
    ...
    /// 预设把裁白边关掉之后这一趟就没有再开回来的写法了（停车场 Q55）。
    ...
    /// **整卷统一灰阶默认关着**（ADR 0018 决定第 2、5 条）：一个 flag 都不加是逐页，
    ...
    ///
    /// 钉的是「那一段不是静默的空白」：清点在慢盘上要花时间，而它排在开工事件之前，
    ...
    ///
    /// 处理卷要的那一整排参数它一个都不收——灰阶测试图不读源、不落进输出目录，
    /// 缩放、灰阶档位、抖动、缓存、并发在它身上一件都不发生。
    #[test]
    fn the_calibrate_subcommand_names_a_device_and_where_to_write_the_chart() {
        ...
        assert_eq!(out, &PathBuf::from("灰阶测试图.png"));
    ...
    /// 数出来的级数回填给 `--gray-levels`，灰阶测试图跟着只排这台设备真会用到的那几档
    ...
    /// 短帮助必须说得出**这张图现在回答两件事**（灰阶测试图批 01 号票），
    /// 以及数出的是**可见灰阶数**、不等于屏幕的物理灰阶数（14 号票的最后一条；
    /// ADR 0003 的《后果》）。
    ///
    ...
    fn the_calibrate_help_says_the_chart_answers_two_things_and_what_the_count_means() {
        ...
        for help in [
            ...
            assert!(help.contains("可见灰阶数"), "{help}");
            assert!(help.contains("不等于屏幕的物理灰阶数"), "{help}");
        }
    ...
    /// 型号名解成了哪块面板；覆盖了屏幕灰阶数的那一趟排的阶梯条数不同，字节跟着不同。
    #[test]
    fn calibrate_folds_the_device_and_the_override_into_the_profile_it_hands_the_library() {
        ...
        let code = calibrate("Kobo Libra 2", None, &out).expect("写灰阶测试图");
        calibrate("Kobo Libra 2", Some(2), &two_levels).expect("写灰阶测试图");
        ...
        let panel = Profile::resolve("kobo-libra-2")
            ...
            .panel()
            .resolution;
        let written = std::fs::read(&out).expect("读回灰阶测试图");
        ...
        assert_ne!(
            written,
            std::fs::read(&two_levels).expect("读回灰阶测试图"),
    ...
    /// `--no-crop` 关得掉裁白边，**不点名就是裁**（页几何批 02 号票：默认打开）。
    ///
    ...
    /// `--help` 末尾的《选项冲突》里，由 [`the_help_lists_every_interlock_in_one_place`]
    ...
    fn the_crop_help_says_it_is_on_by_default_and_how_the_margins_come_off() {
        ...
        // 指路要在：互锁那一节是关掉裁白边这件事唯一说得到代价的地方。
        assert!(help.contains("选项冲突"), "{help}");
    }
    ...
    /// 那正是「只写一处」要防的漂移。
    ///
    /// **处置不筛。** 一趟不吭声的那一条（裁白边关着）与当场拒绝的那一条
    ...
    /// `--no-split` 关得掉拆分，判定宽度与阅读方向点得动，**不点名就是拆、1.5、右开**
    /// （页几何批 04 号票：默认打开，判定宽度可调）。
    ///
    ...
    /// （页几何批 04 号票的票面）。
    ///
    /// 少了「默认是拆的」，升级的人只会看到页数突然翻倍；少了「中缝定切点」，
    ...
    /// `--white-align-limit` 在命令行上认得，不点名就是默认的 4（纸色提白批 05 号票）。
    ///
    ...
    fn the_white_align_help_says_what_it_is_what_it_defaults_to_and_what_zero_means() {
        ...
        assert!(help.contains("纸色提白"), "{help}");
    ...
    fn no_metadata_says_in_the_help_text_that_it_also_turns_idempotency_off() {
        ...
        assert!(help.contains("重跑时就认不出转换过的卷"), "{help}");
    ...
    /// **按一次是做完再停、按两次是立即停止**，再按不动（ADR 0013）。
    ///
    /// 这是两级停止在命令行这一头的全部形状：`Ctrl-C` 每按一下走的就是 [`Latch::press`]
    /// （见 [`install_the_stop_key`]），而它答的那一级既是屏上那一句说的，
    /// 也是观察者交给库的那个字。
    /// **第三下与第二下一个待遇**——两级停止就是两级，立即停止之上没有更强的一级，
    /// 命令行这一头也没有第三种停法。
    ///
    ...
    ///
    /// 两级各说各的，一个字都不重样；做完再停那一句还得说出**下一下是什么**——不说的话，
    ...
    fn each_level_says_which_one_it_is_on_screen() {
        ...
        assert!(finish.contains("做完再停"), "{finish}");
        assert!(
            finish.contains("立即停止"),
            "做完再停那一句没说下一下是什么：{finish}"
        );
        assert!(abort.contains("立即停止"), "{abort}");
