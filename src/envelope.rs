//! 汇总：把逐页判定收成卷级的一个统一档位（ADR 0006：位深按卷取上包络并加迟滞）。
//!
//! 夹在两遍之间的那一步——要看完整卷才做得了。逐页判定在 `decide`，这里只重定它给出的档。
//! **只在 `--envelope` 那条路上在场**：默认路径上灰阶档位逐页各判各的，不做迟滞，
//! 这一层整个不跑（ADR 0018 决定第 2、5 条）。
//!
//! **这不是「整卷一个档」。** 差异大的页单独定档、迟滞升档，两者都会在卷内造成档位差
//! （ADR 0006 认下的代价），[`Envelope`] 因此把这两处各出了多少页原样摆出来——
//! 报告不许把整卷统一灰阶说成绝对一致。
//!
//! 四个数——整卷统一灰阶的分位、迟滞页数、差异大的页画质分的立脚点分位与倍数——全部**未标定**
...
/// 差异大的页画质分的倍数：画质分要超过画质门槛的这么多倍，才算「显著偏离卷内分布」。**未标定占位值**。
const OUTLIER_FACTOR: f32 = 3.0;

/// 迟滞页数：「一页说了不算」要连续多少页高于统一档位才升档（ADR 0006 决定第 4 条）。
/// **未标定占位值**，`Display` 把它连同「未标定」一起印出来。只有这一条路用它（ADR 0018 决定第 3 条）。
const PAGES: usize = 3;

/// 差异大的页画质分的立脚点所在的分位：偏离量在这一档上量。**未标定占位值**。
///
/// 它不等于 [`ENVELOPE_QUANTILE`]，两者问的不是同一件事。整卷统一灰阶问「其余页要哪一档」，
/// 答案该贴着分布的上沿走；立脚点问「其余页过得去的是哪一档」，答案要**经得住差异大的页自己的拉抬**。
/// p95 当不了立脚点：越过它的页至多 5%，差异大的页一旦多于此就把立脚点抬进自己那一档，
/// 检验又在那一档上做，于是人人达标、一个都摘不出。而 ADR 0006 决定第 5 条举的例子——
/// 黑白 profile 下的彩页转灰——在一卷里占到一成是常态。
///
/// 再往下取也不行。取中位数的话，一卷里过半页要更高一档时（同一位画师换了种网点，
/// 后半卷整片抬一档），那些页会被当成差异大的页整片摘走，而这一卷的档本该由它们定——
/// 摘完剩下的那一组反倒成了少数派，卷级统一档位跟着塌下去。
///
/// p75 把两侧的余量摆在一处。摘得出来的占比有个上界，越过它立脚点就站进了特例那一组——
/// 那时报出来的档说的是这一卷本身长什么样，不再是「几页偏离」
/// （与 [`summarize`] 里「一页不剩地落到特例侧」同一个道理）。
///
/// **这个上界随卷长变，长卷上才趋近四分之一**：留一之后剩 `n-1` 页，
/// 立脚点站在第 `⌈0.75(n-1)⌉` 名上，差异大的页要占到 `n+1-⌈0.75(n-1)⌉` 页才够把它抬进自己那一组。
/// 60 页要 16 页（26.7%），20 页要 6 页（30%），5 页要 3 页（60%），2 页要 2 页。
/// 短卷更宽不是漏洞，是同一件事的另一面：几页的卷本来就没有「分布」，
/// 而那时差异大的页那一层是唯一还挡着的防线（见 [`envelope`] 里上分位在短卷上的退化）。
const ANCHOR_QUANTILE: f64 = 0.75;

/// 汇总要看的那一页：逐页判定的结果，加上它是从哪条画质分曲线来的。
pub(crate) struct Page<'a> {
    /// 这一页各候选的画质分值，由小到大。候选集由屏幕灰阶数与尺寸贴合检查裁出，
...
/// 一个卷的整卷统一灰阶：统一档位、定出它的那一页，以及卷内档位差各出在哪。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct Envelope {
    /// 卷内其余页的统一档位。抖动那一维在这里跟着灰阶档位一起定下（ADR 0007：整卷统一灰阶取的是这个组合）。
    pub base: Candidate,
    /// 定出统一档位的那一页：其余页按逐页判定排开后，站在上分位秩上的那一页。
    ...
    /// 报告的尺寸贴合检查那一行说得出来（[`crate::VolumeReport::outside_the_gate`]）。
...
impl Envelope {
    /// 差异大的页占卷内进得了整卷统一灰阶的那些灰度页的比例，0 到 1。
    ///
    /// 分母是其余页那一组加差异大的页，**不是卷内全部灰度页**：彩页与坏页本来就不进整卷统一灰阶，
    /// 尺寸未贴合屏幕的页与残缺页则是进这一层之前被摘走的（见
    /// [`body_pages`](Self::body_pages)）。
    ///
    /// 差异大的页机制是卷级分位聚合的安全网，而这个数是**这张网有没有张开**的唯一外部观测点。
    /// 只看 [`outlier_pages`](Self::outlier_pages)，「特例 0 页」读起来像「这一卷本来就没有
    /// 差异大的页」，而它同样可能是这张网整个失灵。占比把它摆到其余页数旁边，「零」由此有了刻度。
    ///
    ...
    pub fn outlier_share(&self) -> f64 {
        // 其余页加特例就是**进得了这一层**的全部（摘走的两类见 `body_pages`）；
...
/// 把逐页判定收成一个卷级的统一档位。空卷没有整卷统一灰阶。
///
/// 两步，次序不能反：
/// 1. 摘出差异大的页（[`outlying`]），剩下的是其余页；
/// 2. 在其余页上取整卷统一灰阶定出统一档位，再叠加迟滞（[`raise`]）。
///
/// **这里不先取一次全卷的整卷统一灰阶。**特例检验要的立脚点整个在 [`outlying`] 以内逐页取，
/// 卷级聚合当不了它——那一档由全卷算出，被判的页自己也在里面
/// （`CONTEXT.md` 的《画质分》：立脚点不含被判的这一页）。
pub(crate) fn summarize(pages: &[Page], threshold: Threshold) -> Option<Summary> {
    ...
    // 一页不剩地落到特例侧，说明偏离的是这一卷本身，不是其中某几页：一页都不摘。
    // 候选上界都过不去的卷（`Reason::NoneWithinThreshold`，如 `--gray-levels 4` 撞上整卷灰调）
    // 就是这个局面——那时「远在界外」不再说明谁偏离了谁，而其余页不能空着。
    ...
    let raised_pages = raise(&body, pages, base, threshold, &mut verdicts);
...
/// 整卷统一灰阶：把这些页的逐页判定排一遍，站在上分位秩上的那一页定出档位。返回 (候选, 那一页)。
///
/// 分位与画质分的分块聚合共用最近秩取法（[`nearest_rank`]），不插值；
/// 排开的次序见 [`by_demand`]，同一卷跑两遍因此指出同一个代表页。
///
/// **页数少到取不出分位时退化成判定最高的那一页**：p95 的秩在 20 页以内就是页数本身。
/// 这与画质分的分块聚合是同一个取舍（见 `metric` 的 `aggregate`）——宁可严格，
/// 也不要把仅有的几页平均掉。代价是 ADR 0006 决定第 3 条要挡的「极端内容独自定全卷的档」
/// 在短卷上只剩差异大的页那一层挡着；短卷本来也没有「分布」可言。
///
/// `indices` 不得为空。
...
///
/// 整卷统一灰阶与特例检验的立脚点都站在这条序列的某个秩上（[`envelope`]、[`outlying`]），
/// 两者取的分位不同、秩不同，**序列必须是同一条**：一个说「其余页要哪一档」、
...
/// 差异大的页：画质分显著偏离卷内分布的页（`CONTEXT.md`）。不参与整卷统一灰阶，单独定档
/// （ADR 0006 决定第 5 条）。
///
/// 偏离量在**立脚点**那一档上量（`CONTEXT.md` 的《画质分》）：把被判的这一页从
/// [`by_demand`] 排出的序列里抽掉，站在 [`ANCHOR_QUANTILE`] 秩上的那一页就是它。
/// 卷内其余页过得去的那一档，差异大的页远远过不去。
///
...
///
/// **一页的卷没有差异大的页**：一页构不成分布，也就没有谁偏离谁——抽掉它就什么都不剩了。
///
/// 判定依据是**幅度**，判定依据的形态里没有「连着几页」这一维：卷首连着几页的彩页转灰后仍是差异大的页，
/// 那正是 ADR 0006 决定第 5 条举的例子。成段与否只在迟滞那一层说话
/// （见 [`raise`]），那一层管的是升不升档，不是摘不摘页。
fn outlying(pages: &[Page], threshold: Threshold) -> Vec<bool> {
    ...
    // 立脚点从这条序列上取，与整卷统一灰阶同一条（[`by_demand`]）。
    let all: Vec<usize> = (0..pages.len()).collect();
...
/// 迟滞升档：其余页里连续够了 [`PAGES`] 页**统一档位不够用**的，整段一起升。
/// 返回升上去的页数。
///
/// 一页说了不算——升档要有持续的证据，否则翻页跳变的密度就退回逐页可变
/// （ADR 0006 决定第 4 条）。段不够长的那些页留在统一档位上，也就得不到它们各自要的那一档：
/// 那是上分位明知故犯的另一半。
///
/// 「统一档位不够用」判的是**统一档位过不过得了这一页的界**，不是「判定排在统一档位之后」。
/// 后者在灰阶档位那一维上与前者等价：判定是这一页最低的合格候选，格点套嵌保证更高的档也合格。
/// 抖动那一维上不等价——`1bit+FS` 排在 `2bit` 之前，画质分却可能好过它，
/// 于是有页判定排在统一档位**之前**、统一档位却过不了它的界。照排序判会把这样的页
/// 静默写成越界档，而 ADR 0006 决定第 4 条说的本来就是「要求高于统一档位」。
///
/// 整段升到**满足整段的最低一档**：与逐页判定同一条规则（界以内最低的一档）抬到段上，
/// 只是「整段」把界的检验摊到段内每一页上（见 [`lowest_for`]）。
/// 段内不会有差异大的页——那些已经摘走了，所以这一档抬不到极端内容上去。
///
/// 「连续」数的是**其余页**的序列，差异大的页整个不在其中。否则一页特例就能把一段持续的要求
/// 切成两截，而差异大的页恰恰爱出现在段的边上（彩页常在章节交界）。
///
/// 整卷统一灰阶关着时这一层整个不在场：默认路径上一页的档只取决于它自己，没有统一档位可比，
/// 也不问邻居（ADR 0018）。
fn raise(
    body: &[usize],
    pages: &[Page],
    ...
    for run in runs(body.len(), |position| {
        ...
        // 兜底那一路可能给回统一档位本身：整卷在每一档上都过不去时就是这个局面
        // （见 [`summarize`] 里「一页不剩地落到特例侧」那一段）。那时一页都没升上去，
        // 段留在统一档位上，理由仍是整卷统一灰阶，`raised_pages` 也不该把它算进来。
        if candidate == base {
            continue;
        }
...
/// 满足整段的最低一档：统一档位之上、段内每一页的画质分都在界以内的那个最低候选。
///
/// 不能拿段内各页判定的最大值代替。灰阶档位那一维上两者相等——格点套嵌（见 `quantize`），
/// 灰阶档位升高只会让误差更小；抖动那一维上不成立：`2bit` 排在 `1bit+FS` 之后，
/// 却不因此就在每一页上都比它更贴近参照。规则本来就是「满足整段」，那就照着检验。
///
/// **只在统一档位之上找。**迟滞是升档规则（ADR 0006 决定第 4 条），不是给一段页
/// 另配一档的口子：段内某一页要的那一档排在统一档位之前时，照样得抬到统一档位之上，
/// 否则卷内就多出一次向下的跳变，而 `raised_pages` 也不再是实话。
///
/// 一档都满足不了整段时退回统一档位与段内判定的较高者——那是逐页那一层的兜底
...
impl std::fmt::Display for Envelope {
    /// 四个数一并说出，并标明都还没标定——报告不许把整卷统一灰阶说成绝对一致（ADR 0006）。
    ///
    /// 差异大的页数后面跟着占比（[`Envelope::outlier_share`]）：
    /// 「特例 0 页」要读得出它是相对多少页的零。
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "统一档位 {} ⋅ 其余 {} 页 ⋅ 差异大 {} 页（{:.1}%）⋅ 为少换档而升档 {} 页\
             （按 p{} 取统一档位 ⋅ 至少 {} 页才换档 ⋅ 差异大按 p{} 为基准、超过 {:.1} 倍画质门槛算，四个数都是暂定值）",
            self.base,
            self.body_pages,
            self.outlier_pages,
...
mod tests {
    ...
    /// e-ink 面板 + 尺寸未贴合屏幕时的候选集：{1,2,4} 三档，全不抖动。
    /// 卷级的多数用例只在灰阶档位这一维上分胜负，抖动那一维单开两条用例。
    fn candidates() -> Vec<Candidate> {
        Candidate::all(16, GeometryGate::Broken)
    }

    /// 一卷合成的逐页判定。画质分曲线要按页存下来，`Page` 借的就是它。
    struct Volume {
    ...
    impl Volume {
        /// 按「逐页判定 + 越界量」造一卷。
        ///
        /// 越界量是判定那一档**以下**各档的画质分值：那些档全部越界，判定那一档起全部在界内——
        ...
        /// 同上，但点名候选集——抖动那一维要的是尺寸贴合屏幕时那一套六个候选。
        ...
        /// （`Reason::NoneWithinThreshold`），画质分在每一档上都远在界外。
    ...
    /// 一页的画质分曲线：`decided` 以下各档都是 `over`，`decided` 起全部零误差。
    ...
    /// 其余页共用一个统一档位：判定低于统一档位的页也照统一档位定，翻页时不逐页变动。
    /// 体积不再最优是 ADR 0006 明知故犯的交换。
    #[test]
    fn the_body_pages_all_land_on_one_base_depth() {
        // 十页只要 1bit、十页要 2bit：上分位站在 2bit 上，全卷其余页跟着走 2bit。
    ...
    /// 整卷统一灰阶取的是 (灰阶档位, 抖动模式) 这个组合，不是灰阶档位单独一维（ADR 0007：
    /// 「整卷统一灰阶取的是这个组合，不设页级抖动开关」）：
    /// 抖动跟着灰阶档位一起按卷定下，全卷共用同一个抖动模式，页级没有开关。
    #[test]
    fn the_envelope_is_taken_over_the_whole_candidate_including_the_dither_mode() {
        ...
        // 十页不抖动就够，十页要抖：上分位站在抖的那一档上，全卷其余页跟着抖。
        let mut pages = vec![(Candidate::plain(BitDepth::One), 0.0); 10];
        ...
        // 升的是抖动那一维，灰阶档位一档没动——组合排序里 1bit+FS 就排在 2bit 之前。
    ...
    /// 代表页是站在上分位秩上的那一页：它的判定就是统一档位，而且它必是其余页。
    #[test]
    ...
    fn a_run_long_enough_to_be_sustained_raises_that_stretch() {
        // 六十页里让三页连续要求更高：上分位的秩落在 57，这三页因此仍在统一档位之上。
    ...
    /// ——抖动那一维上没有灰阶档位那种格点套嵌的单调性。满足整段的最低一档因此是 `2bit+FS`。
    #[test]
    ...
    /// 其余页跟着统一档位走，前提是统一档位**过得了这一页的界**。判定排在统一档位之前的页
    /// 不等于统一档位够用它——抖动那一维上没有灰阶档位那种单调性，`1bit+FS` 排在 `2bit` 之前，
    /// 画质分却可能好过它。这样的页要照样进迟滞那一段，否则会被静默写成越界档。
    #[test]
    fn a_page_the_base_cannot_admit_is_raised_even_when_it_sorts_below_the_base() {
        let all = Candidate::all(16, GeometryGate::Holds);
        let dithered = Candidate::new(BitDepth::One, Dither::FloydSteinberg);
        // 五十七页只要 2bit，统一档位落在它上面。
        let mut volume = Volume::over(
            all.clone(),
            &vec![(Candidate::plain(BitDepth::Two), just_over()); 60],
        );
        // 另三页连着：不抖动的两档都越界，`1bit+FS` 起在界内——判定因此排在统一档位**之前**。
        for index in 30..33 {
        ...
        for index in 30..33 {
            assert_eq!(
                summary.verdicts[index].candidate,
                Candidate::new(BitDepth::Two, Dither::FloydSteinberg),
                "第 {index} 页被写成了统一档位，而统一档位过不了它的界"
            );
            assert_eq!(summary.verdicts[index].reason, Reason::Hysteresis);
        }
    }

    /// 差一页就不算持续：同样的内容少一页，全卷留在统一档位上。
    #[test]
    ...
    /// 差异大的页：远在界外的那些页。不参与整卷统一灰阶，单独定到它自己要的那一档。
    #[test]
    fn a_page_far_outside_the_threshold_is_taken_out_and_decided_on_its_own() {
        ...
        assert_ne!(summary.envelope.driver, 4, "差异大的页不该定出统一档位");
        assert_eq!(
    ...
    /// 特例看的是幅度，不是「连着几页」：卷首连着几页的彩页仍然一页一页地摘出去。
    /// ADR 0006 决定第 5 条举的正是这个例子，它不能因为彩页成段就落回其余页。
    #[test]
    fn a_stretch_of_pages_far_outside_the_threshold_is_a_stretch_of_outliers() {
        ...
        // 60 页的卷要占到 16 页，立脚点才站进这一组，那时它们不再是差异大的页，
    ...
    /// 差异大的页整个被摘出去：其余页的连续性是**去掉差异大的页之后**的那条序列。
    /// 否则一页特例就能把一段持续的要求切成两截，迟滞跟着失灵。
    #[test]
    ...
    /// 六十页里六页远在界外，占一成：六页一页不少地摘出去，其余页那一档不被它们抬高。
    ///
    ...
    fn a_tenth_of_the_volume_falling_far_outside_is_a_tenth_of_the_volume_taken_out() {
        ...
        // 其余页那一档不被这六页抬高：剩下那五十四页要的仍然是 1bit。
        assert_eq!(summary.envelope.base, Candidate::plain(BitDepth::One));
        for index in 0..6 {
            assert_eq!(
                summary.verdicts[index].candidate,
    ...
    /// 卷页数从 1 到 20 逐个走一遍：除了一页的卷，末页那张差异大的页每一档卷长都摘得出来。
    ///
    ...
    /// 特例判定不许跟着这条坎翻面——它站的是自己那个立脚点，不是整卷统一灰阶那一档。
    #[test]
    ...
    /// 差异大的页占到 `n+1-⌈0.75(n-1)⌉` 页时，立脚点就站进了它们那一组，一页也摘不出来。
    ///
    ...
    /// 一页不剩地落到特例侧时一页都不摘：偏离的是这一卷本身，其余页不能空着。
    #[test]
    ...
    /// 差异大的页占比进报告：「一页都没摘出来」得在报告里看得见。
    ///
    /// 这个数是这张安全网**有没有张开**的唯一外部观测点。只报计数的话，
    /// 「特例 0 页」读起来像「这一卷本来就没有差异大的页」，而它同样可能是判定整个失灵——
    /// 本票修的就是后一种。占比把它摆在其余页数旁边，两个读数一起说话。
    #[test]
