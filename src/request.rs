...
pub struct Request {
    ...
    /// （`CONTEXT.md`）。
    ///
    /// 它改的是**目标尺寸**，而目标尺寸是尺寸贴合检查、总缩放比与画质分参照三样东西的来源：
    /// 换一个缩放方式，这一卷的判定要重算，幂等因此收着它（见 `crate::metadata`）。
    pub fit: FitMode,
    ...
    /// 《真机像素完整性》）。裁白边收进来之后好处烤进产物，那个开关变成空操作。
    ///
    /// 裁法按行列墨量占比、逐页各裁各的，详见 `crate::crop`。它改的是**适配之前**的页尺寸，
    /// 目标尺寸、尺寸贴合检查、画质分参照与判定因此全跟着变，幂等收着它（见 `crate::metadata`）。
    pub crop: bool,
    ...
    ///
    /// 它改的是**这一卷有几页、每一页是哪一块**：一个源页有中缝就切成两张输出页，
    /// 没有就是连续跨页、原样出一张。成员名、页尺寸与判定因此全跟着变，
    /// 幂等收着这三项（见 `crate::metadata`）。
    pub split: SplitRule,
    /// 残差段的重采样缩放算法（`--filter`）。整数倍预缩那一级不受它影响（ADR 0001）。
    pub filter: Filter,
    /// 纸色提白的上限（`--white-align-limit`，纸色提白批 01、05 号票）。
    /// **默认 4 级，即默认开着**（取值出处只有 [`WhiteAlignLimit::default`]）；取 0 是关闭。
    ///
    /// 它是**处理选项**的一项：这一趟愿意为对齐付多少色调，不是面板的物理事实。
    ...
    /// 灰阶档位覆盖（`--bit-depth`）。特殊卷靠它手工兜底（spec 的 story 23）。
    ///
    /// 覆盖项裁的是**候选集**：点名一档灰阶档位，候选就只剩那一档的。裁到只剩一个候选时判定
    /// 整个被顶掉；只点灰阶档位而其余页那一组的尺寸贴合检查开着时，抖动那一维还有得判，画质分照旧说了算。
    ///
    /// 裁得掉的只有候选。屏幕灰阶数那道硬上界仍在，越界的覆盖当场被拒（ADR 0003）。
    pub bit_depth: Option<BitDepth>,
    /// 抖动模式覆盖（`--dither`）。与 [`bit_depth`](Self::bit_depth) 同一个作用方式：
    /// 裁候选集的另一维。
    ///
    /// 裁得掉的同样只有候选。尺寸贴合检查是**页的**几何事实、不是自动选择，点名抖动顶不掉它——
    /// 卷里有一页未贴合屏幕，`--dither fs` 就当场被拒（错误指得出是哪一页），不静默照抖。
    /// 拒绝仍是整趟的：覆盖项是用户的显式指令，不是可以按页悄悄放弃的东西
    /// （ADR 0007 的《后果》）。
    pub dither: Option<Dither>,
    /// 打开整卷统一灰阶（`--envelope`）：按卷取 p95 定出一个统一档位并加迟滞，卷内其余页都用这一档
    /// （ADR 0006）。**默认关着**——默认路径上灰阶档位逐页各判各的、不做迟滞
    /// （ADR 0018 决定第 2、5 条）。两条路各买到什么、各付什么，见那两篇 ADR 的《后果》。
    pub envelope: bool,
    /// 内存上限（`--cache-budget`）：分析与写出之间的缓存最多在内存里留这么多字节，
    /// 超出的页溢写临时文件（ADR 0005）。限的是峰值内存，不是卷的大小上限。
    pub cache_budget: CacheBudget,
    /// 做到哪一步。
    pub mode: Mode,
    /// 读盘方式（`--io-mode`），覆盖按路径的硬盘类型探测。
    ///
