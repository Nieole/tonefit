...
//! # 字形约定那两条在库里，不在这里
//!
//! **不许断的那个空格**（[`HARD_SPACE`]）与**这个字形宽度稳不稳**
//! （[`tonefit::width_is_stable`]）从此是**库的公共 API**，理由写在 `tonefit::glyph`
//! 那个模块上：库自己会把字直接送进一条对齐的列里，规矩因此得跟着字一起递过去——
//! 靠这一头事后改字形，是把界面层的约束漏进库里却不把规矩递过去。
//!
//! **折法一条都没跟着搬**：折到多宽、断在哪儿、[禁则](NEVER_STARTS_A_ROW)、行首缩进
//! 全在这个模块里。那两条量的是「写下去的是哪个字符」，折行量的是「这一格真有多宽」——
//! **两个去处量的不是同一块地方**，一处当场问终端、一处当场问那一格（见下面那张表）。
//!
//! # 为什么非要自己一套
//!
...
//! | 会话屏上那几格 | 那一格真有多宽 | `crate::session::shell` |
//!
...
// **两条字形约定不在这里，在库里**（`tonefit::glyph`，理由写在那个模块上）：
// 库自己会把字直接送进一条对齐的列里，规矩因此得跟着字一起递过去。折行这一头照旧读它。
...
/// 断处的判定依据（[`breakable`]）只看「挨着空格还是挨着宽字符」，而
/// `……默认（按屏幕规格）` 断得开的地方里恰好有一处把 `）` 顶到下一行的行首
/// （停车场 Q103）。
///
...
/// **不走折行的那一处自己过**——拒绝开始那句话直接落在 stderr 上
...
/// 把这一行上仅有的那几处全挡住了。
///
/// **记号宽过一整行时因此仍旧硬断**：那时一行一格都不剩，与一个长过这一格的西文词
/// 同一个待遇——切掉尾巴更坏。断在标注上时**行尾那个空格照旧去掉**（[`printed`] 已经把它
/// 换回一个普通空格，行尾的空白跟着[折行那一步](fold_line)去掉）：走到那一步说明这一行
/// 连整个记号都摆不下，标注已经不成立，留一个吊在行尾的空格只是噪声。
...
mod tests {
    ...
    fn a_chinese_sentence_without_a_single_space_still_folds() {
        let sentence = "跑起来之前必填的两项是型号与输出目录";
