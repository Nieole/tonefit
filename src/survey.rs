//! 清点：开工之前**发现**这一趟有哪些卷，再把它们全枚举一遍，算出这一趟的
...
//! 扫两遍买的不是省事，是**不攥着几千个句柄**：卷留在清点手上的那一版里，N 个归档卷
...
//! （ADR 0009 的《不要做的「简化」》）——路径与输出错了可能写到别人的目录里，
//! 而那一趟已经写出去的卷收不回来。
//! **发现出来的**那一种点不开不走这条路：归档记进非漫画文件、目录记进无法访问的地方，
//! 其余照做（ADR 0014 决定第 5 条）——对推测出来的东西不用最重的处置，
//! 一个坏 zip 不该把几百卷挡在门外。
//! 清点**之后**才出的卷转换失败也不走这条路：那时其余卷照做，报告照出
//! （`CONTEXT.md` 的《失败》：卷转换失败）。
//!
//! **一页都没有的东西不是卷**（ADR 0014 决定第 3 条）：开出来一页都没有的候选在这里
//! 就被丢掉，此后的每一层都不知道它存在过——输出里因此一个字节都没有。
//! 字体包、源码包、空目录、只装着别的卷的目录都落在这一支上。
//!
//! # 另两份产出：非漫画文件，与无法访问的地方
//!
...
//! **不做**与**说得出**是同一条决定的两半。
//!
//! **不落盘。** 清点的作用域是这一趟点名的卷，活在一次运行之内。把成员表存下来下趟再用
...
/// 这一趟清点出来的东西：**发现出来的**每一个卷一份，外加它们步数之和，
/// 再外加没被任何卷收下的那些[非漫画文件](NonVolumeFile)，
/// 与[无法访问的那些地方](UnreachablePlace)。
pub(crate) struct Survey {
    ...
    /// 发现的时候**无法访问**的那些地方，按发现顺序（`p4-parking-lot/11`）。
    ///
...
///
/// 它是那个卷的一份摘要，不是那个卷：清点数完就把卷放掉，处理那一卷时按
...
pub(crate) struct Surveyed {
    ...
    ///
    /// 它算在**清点这一遍**数出来的成员上。重开之后成员数可能与它对不上（两遍之间源变了），
    ...
    /// 「从打开卷到这份卷报告成型」，而清点这一遍也是打开卷。不带上它，慢盘上那一截会
...
impl Surveyed {
    ...
    /// （输出目录）、隔离目录里那个去处（输出目录 + `_isolated`）、以及开工前那道撞名校验
...
impl Survey {
    ...
    pub(crate) fn of(request: &Request) -> Result<Self> {
        ...
        // 枚举一遍就产出一份步数、一份非漫画文件、一份无法访问的地方——折在枚举后面的话，
        ...
        for candidate in found.into_candidates() {
            ...
            match source::enumerate(&candidate.root) {
                ...
                Err(error) => match (candidate.provenance, candidate.container) {
                    ...
                    // 决定第 5 条同样只说归档）。它整棵子树跳过，进的是**无法访问的
...
///
/// 只有点名的进得来。发现出来的点不开的归档不在这张清单上——它进的是非漫画文件那一张
...
fn refuse(refused: &[(PathBuf, anyhow::Error)], named: usize) -> anyhow::Error {
    ...
    said.push_str(
        "整趟拒绝的理由与「处理范围为空是错误」同一条：路径与输出错了可能写到别人的目录里\
         （ADR 0009）。因此一页都没做，输出目录下一个文件都没有——把点名的路径改对再重跑。",
    );
    anyhow!(said)
}
...
mod tests {
    ...
    ///
    /// 两句在对方的平台上都恒成立，因此谁也不会误报；而把卷留在清点手上的那一版，
    /// 两句各在自己的平台上当场红。
    #[test]
    fn a_survey_keeps_no_archive_open() {
        ...
        if let Some(held) = open_files_under(&library) {
            assert_eq!(held, 0, "清点开完还攥着 {held} 个句柄");
        }
        std::fs::rename(&library, space.path().join("改过名"))
            .expect("装着这些归档的目录改不动名：清点还攥着其中某个文件");
    ...
    /// **两张表分得开**：读不动的**归档**进非漫画文件，读不动的**目录**进无法访问的地方
    /// （`p4-parking-lot/11`）。
    ///
    /// 这一条钉的是本票那条边界：非漫画文件那三类列的是**文件**，而一个目录不是文件——
