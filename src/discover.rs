...
//! 「里面到底有没有页」由清点开卷时才答得出（见 `crate::survey`）。
//! 一页都没有的候选在那里被丢掉——**一页都没有的东西不是卷**，输出里一个字节都没有，
//! 而它没能收下的那些进**非漫画文件**那张表（见 `crate::survey` 的《另一半产出》）。
//!
...
/// 发现出来的一个**卷候选**：一个路径，加上它将要写到输出目录下的哪里。
pub(crate) struct Candidate {
    ...
    ///
    /// 列目录时就知道了，不必再 stat 一次（见 [`Child`]）。清点按它分岔**点不开时**
    /// 的处置：点不开的**归档**进非漫画文件那张表，点不开的**目录**进无法访问的地方那一张
...
///
/// [`of`] 一次只看一个处理路径。点名的路径互相嵌套（`库` 与 `库/作品`）、或者同一个
...
/// 跨几个处理路径的卷序因此仍是「用户点名的次序，各自内部按[阅读顺序](source::reading_order)」
/// ——`tonefit 库/作品 库` 与 `tonefit 库 库/作品` 得到同一棵输出树、同样多的卷、
/// 同一份非漫画文件清单，而报告里的**卷序**不同（停车场 Q243）。
///
/// **折的是卷根，不是处理路径之间的前缀关系。** 点名 `库` 与 `库/#recycle` 是嵌套的两条
/// 路径，而发现根本走不进后者（[`push_children`] 把自动跳过的目录挡在外面），
...
/// 清点开它的时候同一个 `read_dir` 再失败一次，那一处把它记进**无法访问的地方**
/// （见 `crate::survey`，`p4-parking-lot/11` 收的停车场 Q117）。这里因此不必再报一遍——
/// 报了就是同一个目录两条。
///
/// 三样东西被挡在外面：[本来就自动跳过的目录](source::is_ignored_directory)、
/// 符号链接与 junction（`file_type` 问的是链接自己，因此它既不是目录也不是文件，
/// **环进不来**，深度不必设上界）、以及既不是目录也不是认得的归档的文件——
/// 后者要么是某个卷的透传成员，要么是**非漫画文件**——分界是它躺的那一层有没有页，
/// 而那要开卷才答得出，因此不在这里定（见 `crate::survey` 的《另一半产出》）。
///
/// 头一样是在**这里**挡的，不是攒完候选再滤：自动跳过的目录连同它底下的一切**根本不成为候选**，
/// 清点因此看不见它、无法访问的地方那一栏收不到它、退出码一格不动、报告上一个字都没有。
...
/// 一个卷根**镜像**到输出目录之下的相对去处。
///
/// 基准点是**处理路径的父目录**——处理路径自己的名字因此恒出现在输出目录下，
...
/// ```
///
/// 取「处理路径自己」当基准点的话，点名一个**卷**时它的页会直接撒进输出目录，
/// 于是实际要两套规则。
///
...
mod tests {
    ...
    /// 点名一个卷：它的名字恒出现在输出目录下，而不是把页撒进输出目录。
    #[test]
    ...
    /// 点名一个装着卷的目录：处理路径的名字打头，其下按源的结构镜像。
    #[test]
    ...
    /// 自动跳过的目录整棵子树不进去——回收站里躺着的正是用户删掉的那些卷。
    ///
    /// 打包环境留下的与操作系统留下的**同一份名单**：一个 `#recycle` 与一个
    /// `System Volume Information` 在这一层受同一条判定依据，谁都不成为候选。
    #[test]
