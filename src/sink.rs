...
impl Sink {
    ...
    ///
    /// **输出目录本身写不写得进不由这里答**：那是这一趟的参数错了，开工前探一次就整趟拒掉了
    /// （见 `crate::ensure_the_output_root_takes_a_write`，06 号票）。到得了这里的失败说的是
    /// **这一卷**这一次建临时容器没建成——这一卷的权限与别处不同、输出目录在探过之后才坏、
    /// 盘满——那些是真的「这一卷做不成」，走卷转换失败、其余卷照做。
    ///
    /// **最终位置被别的东西占着不在这一句上**：临时容器另占一个名字，建它照样成，
    /// 撞上是在 [`finish`](Sink::finish) 把它改名到位那一步。两处都是卷转换失败，
    /// 只是报出来的那句话不同。
    ///
...
/// （见 `crate::survey::Surveyed::lodgers`）。它相对的是**这一卷的去处**而不是输出目录，
...
/// 同一趟里它们随后照写，末了的盘是对的；而**那一趟没走完**（按了做完再停／立即停止、
...
/// 而最终位置**仍旧只在收尾这一步被碰到**，中途失败与立即停止照旧一个字节都动不到它。
...
impl DirectorySink {
    fn create(path: &Path, lodgers: Lodgers) -> Result<Self> {
        ...
        // 全枚举一遍、比一遍去处，那是清点要做的事（ADR 0011），不在这一层。
