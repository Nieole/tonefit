...
//!
//! - 整解成功：完好页。
//! - 整解失败、救回到像素：残缺页，缺的那一段留成纸白。它的几何一点没缺。
//! - 整解失败、一个像素都没救回，或尺寸解不出来，或尺寸解得出来而缓冲分配不下：坏页。
...
/// 变成一个坏页，而不是让进程被 OOM 收走。
///
/// **救回那条路上峰值是这个数的两倍**：量救回了多少要第二份同样大的缓冲（见 [`measure`]）。
/// 拒绝的门槛仍是这一个数，不为救回另设一个更小的——那就成了「整解收得下，救回嫌大」，
/// 正是上一段要消灭的东西。翻倍只落在整解已经失败的那条路上，正常页一份都不多占。
///
/// **目标那一侧的兜底上界由它折算而来**（`crate::geometry` 的 `MAX_TARGET_PIXELS`，07 号票）：
/// 解得进来的页，缩出去的那块缓冲也得分配得下——两侧各定一个数，就会出现
/// 「解码收得下、目标分配不下」，而后者中止的是整趟。它因此是 `pub(crate)`：折算的那一头
/// 要引到这一个数上，不许再抄一份。
...
impl Salvage {
    /// 救回的那一份占整页的比例，0 到 1。
    ///
    /// 一个像素都没有的页不会走到调用方手上——那是一张坏页（见 [`Decoder::decode`]），
    /// 因此从报告里读到的这个数恒大于 0。
    pub fn share(self) -> f64 {
        self.0
    }

    /// 直接造一个比例。生产路径上一个都不用——救回了多少只由 [`measure`] 量出来。
    ///
...
impl Decoder {
    ...
    ///
    /// 失败在这里只是一个 `Err`——把它变成一个坏页、把卷送进隔离目录，是 `crate::run` 那一层的事。
...
/// 它因此有自己的目标尺寸、照常缩放与判定，缺的那一段留成纸白；把它算作坏页反而更糟——
/// 那会用卷内统一尺寸顶掉一个本来就正确的尺寸。一个像素都没救回来的页则相反，
/// 「解得出尺寸」在它身上什么都没买到，与那张分配不下缓冲的页同一个结局。
///
/// 尺寸解不出来、或尺寸大到缓冲分配不下，就没有第二趟可言：`None` 回到调用方，那是一个坏页。
///
...
/// 那样数出来，一张只救回了页眉留白的页会被判成一个像素都没救回。**它恰恰是本票的判定依据**，
/// 判错的方向还是把一张救回来的页打成坏页。
///
