...
//! 回调因此是**双向**的——取消不必另开一条信号通路，而「写出环节之前停下来问一句」
//! （ADR 0012：接着写出）与「按停止」（ADR 0013：两级停止）走的是同一条回路。
//!
//! 库这一侧只报到，**印在哪、印不印、长什么样、要不要在确认点上等人由调用方定**。
...
//!
//! **观察者可能很久不返回**：会话要在确认点上等人拿主意（ADR 0012 的《后果》）。
...
pub enum Event<'a> {
    ...
    PassStarted {
        ...
        ///
        /// 对**要读它**的那一位非带不可：确认点问的是「这一卷的写出环节还做不做」，而答得上这一问的东西
        /// ——卷级判定、逐页结果、缓存用量、解码计数——要到
        /// [`VolumeFinished`](Self::VolumeFinished) 才交出去，而那一条排在确认点**之后**。
        /// 不带的话，要在这里等人拿主意的调用方手上只有逐步事件，屏上画不出任何
        /// 可供拿主意的东西。
        ///
        /// 带的是**那一刻为真**的一份，不是预告：写出环节一步没走，因此
        /// `timing.second_pass` 是零，`output` 指的是这一卷**会**落到的那个位置
        /// （此刻盘上还没有它）。答做完再停之后 `VolumeFinished` 交出来的那一份与它的差
        /// 也就只有计时那一格——这一卷等于走了一次预览（`CONTEXT.md` 的《会话》：确认点）。
    ...
    /// 一页失败了，附上给人读的那句原因。
    ///
    /// 它不等整卷跑完：坏页要在**出现的当场**就说得出口（09 号票的会话主区）。
    ...
    ///
    /// **被[立即停止](Instruction::Abort)掉的那一卷不报这一条**：它那格 `partial` 已经丢掉，
    /// 那一卷等于没做，没有报告可带。它也不报 [`VolumeFailed`](Self::VolumeFailed)——
    /// 被停下来不是失败。流上因此看得见一条开卷、后面两条一条都没有，
    /// 那正是「这一卷被立即停止了」在事件流上的样子；结束那一条会把它再说一次
    /// （[`RunOutcome::Stopped`]）。
    #[non_exhaustive]
    VolumeFinished {
    ...
    ///
    /// **报过开工，就一定报得到这一条**，拒绝开始的那一趟也不例外——那时 `outcome` 是
...
impl Event<'_> {
    ...
    ///
    /// [确认点](Events::ask_before_the_second_pass)那一处要在**造出事件之前**就问哨兵
...
pub enum Pass {
    ...
    First,
    /// 写出环节：量化、编码、写页、搬透传文件、收尾改名。
    ///
    /// 这一条报出去的那一刻**汇总已经做完、写出环节还没开始**，而那正是接着写出的确认点
    /// （ADR 0012 决定第 2 条）：答继续就往下做，答做完再停就停在这儿——那就是 dry-run 的效果。
    /// 观察者答的那个字在这一条上**当场作数**，不进闩就算数，见库内的
    /// `Events::ask_before_the_second_pass`。
    ///
    /// 它因此是唯一一条**答复会改变库下一步做什么**的事件：答做完再停的话，这一卷的写出环节
    /// 一步都不走，报告照出（`CONTEXT.md` 的《会话》：确认点）。
    Second,
}

/// 观察者收下一条事件之后回给库的那一个字（ADR 0011 决定第 1 条）。
///
/// 三个，不是四个：进度、两级停止、预览→执行的确认点共用这一条回路
/// （`CONTEXT.md` 的《进度》）。它**不非穷尽**——三级是 ADR 0013 拍死的形状，
/// 而它是观察者要**造**出来的东西，非穷尽会让库外根本造不出任何一个。
///
/// 序即**力度**：`Continue < Finish < Abort`。库把收到过的最强的那一个记下来
/// （见 [`Standing`]），因此指令**只升不降**——按停止是个闩，不是一个可以反悔的开关。
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Default)]
pub enum Instruction {
    /// 继续。
    #[default]
    Continue,
    /// 做完再停：**当前卷跑完就停**（ADR 0013 决定第 1 条）。
    ///
    /// 盘上因此只有完整的卷，下一趟幂等接着走。检查点在卷边界上，
    /// 见 `crate::run`。
    Finish,
    /// 立即停止：立刻停（ADR 0013 决定第 2 条）。
    ///
...
impl Instruction {
    ...
    /// 「按了立即停止却退回做完再停」在哪一份上悄悄发生都看不出来。三份闩因此各存各的，
...
///
/// 实现方**可以很久不返回**：会话要在确认点上把报告画出来并等用户拿主意
...
pub trait Progress: Send + Sync {
    ...
    /// 只是 `so_far` 那一格是 `None`。确认点这一问与那份报告是两件事。
...
/// 两条线程同时报到、一条答做完再停一条答继续，结果恒是做完再停。
///
...
/// 观察者在**确认点**上把人等掉的那一截，一次运行一份（停车场 Q41）。
///
...
impl LockSentinel {
    ...
    /// 「有没有观察者」那道判空**之前**，[确认点](Events::ask_before_the_second_pass)
    /// 自带的那道判空同理——它绕得过 `ask`，于是在它自己那一句上再问一次。
    ///
    /// 收的是**名字**而不是一条 [`Event`]：确认点那一处要在造出事件之前就问
    /// （造它要先拼一份报告）。
    #[track_caller]
...
pub(crate) struct Events<'a> {
    ...
    /// 上一趟按的停会跟着漏到下一趟去。
    standing: &'a Standing,
    /// 在确认点上等人等掉的那一截。与[闩](Standing)同一条寿命：活在 `run` 的栈上，
    /// 一次运行一份。
...
impl<'a> Events<'a> {
    ...
    ///
    /// 交回来的那个字只有确认点用得着（见 [`ask_before_the_second_pass`](Self::ask_before_the_second_pass)）；
    ...
    /// `crate::process_volume` 的《立即停止：回 `None`》那张清单上数得清**，本条不复述。
    ///
    ...
    /// **接着写出的确认点**：把「写出环节要开始了」报出去，回的是观察者**当场答的那个字**
    ...
    /// 闩答的是「这一趟还走不走」，这里问的是「这一卷的写出环节还做不做」。拿闩来答的话，
    /// 分析环节里按下的**做完再停**会顺手把当前卷的写出环节也吃掉，而做完再停的定义正是
    ...
    /// **页边界那个检查点**：这一趟按下立即停止了吗（ADR 0013 决定第 2 条）。
    ///
    /// 逐个成员往下走的每一个循环在自己的循环头上问它一次，**清单只有一处**，见
    /// `crate::process_volume` 的《立即停止：回 `None`》——那张表里有几处落在读取那一层
    /// （开工前摊开一整卷那两遍顺序扫，`p4-parking-lot/13`）：那一遍的「一个成员」与页边界
    /// 是同一道边界，因此**没有第三种停法**。
    /// 答是就当场停下，剩下的成员不做了。停下来之后调用方还要再问一次它，
    /// 决定这一卷算不算做完；**再问一次恒得同一个答案**，因为闩[只升不降](Standing)——
    /// 立即停止之上没有更强的指令了。各段因此不必把「我是被立即停止的」当成返回值往上传。
    ///
    /// 只认立即停止这一级：[做完再停](Instruction::Finish)停在卷边界上，页边界不该抢它的活
    /// （ADR 0013 决定第 1 条：做完再停要当前卷跑完，盘上只留完整的卷）。
...
mod tests {
    ...
    /// 确认点回的是观察者**当场答的那个字**，不是闩（ADR 0012 决定第 2 条）。
    ///
    /// 分开它们要一个特定的现场：闩已经落在做完再停上，而观察者当场答继续——那正是
    /// 「分析环节里按下做完再停」（ADR 0013 决定第 1 条：做完再停要**当前卷跑完**）。
    /// 确认点拿闩当答复的话，那一卷的写出环节会被顺手吃掉，盘上少一整卷。
    ...
    fn the_decision_point_answers_with_the_word_just_said_not_the_latch() {
        ...
        // 分析环节的页边界上按下做完再停：闩记住了它。
        tally.answers(Instruction::Finish);
        events.step();
        assert_eq!(events.standing(), Instruction::Finish, "按下的停没记住");

        // 手指离开键盘，确认点上答的是继续——这一卷的写出环节照走。
        tally.answers(Instruction::Continue);
        assert_eq!(
            events.ask_before_the_second_pass(nothing_yet),
            Instruction::Continue,
            "确认点拿闩当了答复，分析环节里按下的做完再停把当前卷的写出环节也吃掉了"
        );
        assert_eq!(
            events.standing(),
            Instruction::Finish,
            "确认点把闩降回去了：这一卷之后剩下的卷会接着做"
        );

        // 反过来：确认点上答的那个字照样进闩——那一卷停在这儿之后，剩下的卷也不必开工。
    ...
    /// 确认点上等人的那一截掐得出来，别处报到的不掐（停车场 Q41）。
    ///
    ...
    fn only_the_wait_at_the_decision_point_is_clocked_as_deliberation() {
        /// 观察者在确认点上等这么久。够长，调度抖动淹不掉它；够短，用例不因此变慢。
