...
pub(crate) enum SplitPart {
    ...
    ///
    /// 它也不是[非漫画文件](crate::report::NonVolumeFile)：那张表列的是**输出里一个字节都
...
///
/// **派读盘方式问的是这一个**（见 `crate::medium::IoPlan`）：「两遍那一路恒串行」的理由
...
impl Volume {
    ...
    /// 清点要的正是前者：这一卷的步数里有没有摊开那一段，得在开工之前算出来
...
/// 两级停止也是——立即停止让当前那一卷当场返回（ADR 0013 决定第 2 条），卷跟着析构。
///
...
///   处理完当场析构，而清点**一个都不攥**（见 `crate::survey`）——这一格因此是 1，
...
///   分析环节 `min(readers.count, 成员数)` 条（写出环节不读源，它写的是分析环节的结果）。
///   **读取端是一个归档句柄**时分析环节恒为一条顺序扫、用的就是卷自己那个读取端，
///   因此只有幂等那一道多开句柄，`Reads` 一析构就全收。**读取端是一个目录**时
///   （目录卷，以及摊开了的卷）两处都并发，但发下去的是几份卷根，只有**正在读**的
///   那个成员占一个句柄（本模块的 `read_file` 开完就关）。
/// - **写出那一侧**：缓存的溢写文件至多 1 个（`crate::cache`；分析环节起、整卷持有，
///   因此**与分析环节那几条读取重叠**），输出容器至多 1 个（`crate::sink`；归档卷是一个
///   打开的文件，目录卷逐个成员开完就关，写出环节才有）。
///
/// 三格相加，峰值封顶在 **`min(并发度, 这一卷的成员数) + 3`**，而并发度至多是核数
/// （[读取计划](crate::medium::IoPlan)按 `--io-mode` 与硬盘类型定它）。主项在两种读取端上同形：
...
pub enum Reader {
    ...
    ///
    /// 清点只列成员、不碰像素（见 `crate::survey`），它那一份卷因此停在这个形态上——
    /// 摊开一整卷只为数几个成员是白付一次全量写盘，而清点要把这一趟每个卷都数一遍。
    ///
...
impl Reader {
    ...
    /// 摊开之后它仍指着那个归档文件；这一个是**字节此刻住在哪儿**。硬盘类型按路径探测
...
///
/// 非分两条不可：摊开一整卷只为数几个成员是白付一次全量写盘，而清点要在开工之前把
...
///
/// 读取端取不出字节（见 [`Reader::Unextracted`]），而清点要的就是这个：
...
/// `.7z` **只列成员**那一条：解归档头，一个内容字节都不解。清点走它（见 [`enumerate`]）。
///
/// 归档头解完就把文件放掉——`Archive::open` 不留读取端，清点因此在这种卷上同样
...
/// `.rar` **只列成员**那一条：解归档头，一个内容字节都不解。清点走它（见 [`enumerate`]）。
...
/// 靠它把续的那几份摘掉，而摘掉之后清点连开都不开它们（从前每一份都要被整套列一遍）。
///
/// **开不开得了不在这一问的射程里**：开不了就当它不分卷，这一份照旧成一个候选，
/// 而那个错在[列成员](list_rar)那一遍原样报出去——点名的整趟拒绝、发现出来的进非漫画文件
...
/// 那是**卷转换失败**，其余卷照做（见 `crate::process_volume`）。半摊开的那些字节不必手动收——
...
/// 一个关掉了固实的 `.7z` 于是有 N 个块，立即停止之后还要空转 N−1 次
/// （不解内容、不落盘，停车场 Q286）。
///
...
/// 本来就自动跳过的目录：整个子树都不是卷的内容。
///
/// 这一类是什么、它与**无法访问的地方**分在哪里、为什么报告上一个字都没有——
/// 权威位置是 `CONTEXT.md` 的《自动跳过的目录 (IgnoredPlace)》，这里不复述。
...
///   因此**不是** [无法访问的地方](crate::UnreachablePlace)：分得开这两类的是词条里
///   那一句——换一趟会不会好。
///
/// 判定依据是**名字**，不是读不读得动：这几个名字在各自平台上固定、不随语言变，
/// 而按「读不动」认会把真正无法访问的那一半一起吞掉（`no-false-line/07` 收的停车场 Q203）。
///
...
/// 这个成员是不是不看的东西：躺在[自动跳过的目录](IGNORED_DIRECTORIES)里，
/// 或者本身就是打包环境留下的那几个文件。
///
...
///
/// 判定依据是**没有兄弟**，不是名字对不对得上卷名：一层目录若装下了整卷，它就不承担任何排序信息，
...
mod tests {
    ...
    /// 立即停止之后也不在，靠的是同一条寿命：立即停止让那一卷当场返回，卷跟着析构
    ...
    /// 不是它原本来自的那个 `.7z`。
    ///
    /// 硬盘类型按路径探测（ADR 0009 决定第 2 条），而**这个路径就是那道探测的输入**：
    ...
    fn enumerating_a_solid_archive_extracts_nothing() {
        ...
        assert!(listed.extraction_dir().is_none(), "清点那一遍摊开了");
