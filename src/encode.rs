//! 编码：把一张已按目标灰阶档位量化的图编成 PNG 字节。
//!
//! 编出字节而不是写文件——页要么落到目录里，要么进归档，去处由输出容器决定（见 `sink`）。
//!
//! 灰阶档位与抖动模式在编码器接口以外，**调色板模式在接口以内**（ADR 0004）：判定灰阶档位说的是
//! 量化格点，颜色类型、以及最终写进文件的那个灰阶档位由这里挑，挑的依据只有体积。
...
/// 调色板 PNG：色板只收这一页真正用到的取值，位宽因此可能低于判定灰阶档位。
///
...
/// 装得下这么多项色板的最小位宽。
///
/// 灰度与彩色两条调色板路径共用它：位宽由**色板项数**定，与判定灰阶档位无关。
...
mod tests {
    ...
    /// 每一档灰阶档位都要能编、能解回原样：写出去的像素与量化结果逐字节相同。
    #[test]
    ...
    /// 判定灰阶档位是 4bit，文件里却只用 1 位，而像素一个没动。
    #[test]
