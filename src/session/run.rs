...
impl Running {
    ...
    fn collect(&mut self) {
        ...
        if let Some(live) = &self.live {
            ...
            if live.undone().is_none() {
                self.earlier = Some(crate::render::plain::report(live.report(), live.mode()));
            }
        }
    }
    ...
    pub fn report(&self) -> Option<String> {
        let live = self.live()?;
        let attempt = crate::render::plain::report(live.report(), live.mode());
...
mod tests {
    ...
    fn a_refused_run_leaves_the_session_open_and_still_prints_what_was_worked_out() {
        ...
        let worked_out = crate::render::plain::report(live.report(), live.mode());
