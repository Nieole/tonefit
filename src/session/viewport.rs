...
/// | 报告区的**卷表**（`super::draw::report`） | 有：**恒停在末行** | 屏上的一行（表上一卷一行，成句的那几段折出来的每一行也算一行） | 画 |
/// | 补全候选（`super::draw::footer`） | 无 | 一条候选 | **不画**：它列而不选，一个键都不派，滚不动的东西画一条滚动条是在指一个按不动的地方；它说的是「还有 N 条」 |
///
/// 卷表那一处的光标**恒停在末行**：算出来的起点因此恰好是「滚到底」，
/// 与它从前自己算的那一个逐格相同——报告只增不减，而「一卷跑完当场看得见」说的正是
/// 刚添上去的那一行。让它翻得回去（连同**跟随**那个状态，本类型唯一的例外）
/// 归 `p3-session-legibility/10`。
///
/// 报告区**展开**那一副还没进来：那一副有自己的横竖两个滚动量
/// （`Session::clamp_report`），搬过来归 `p3-session-legibility/11` 的逐页表。
///
