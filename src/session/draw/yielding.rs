...
//! | 三 | 总览块那两条**横条**：先收窄，窄过 [`BAR_MIN_WIDTH`] 就整条让掉 | [`bar_width`] |
//!
//! 先让左栏：报告区挤到十几列就一个字都读不出来，而左栏那几行本来就折着行
//! （见 [`super::config::config`]），窄一点仍看得懂。
//!
//! 横条排在最后：它旁边就是走了几步 / 共几步那两个数，让掉它丢的是「一眼看得出」，
//! 不是内容——而让它之前先让的两样丢的都是字（停车场 Q169）。
//!
//! # 高度不够：总览不砍，让的是表
//!
//! | 第几步 | 让的是 | 谁说了算 |
...
/// 横条**收窄到头**那一档：再窄就整条让掉。
///
/// 八格是「一眼看得出走了多少」的下限：横条旁边就摆着走了几步 / 共几步那两个数，
/// 精确的一份那里已经有，横条只为一眼。窄到三四格，四分之一处与一半处画出来是同一格——
/// 那时它说的不比不画多，占的却是那两个数要的格。
pub(super) const BAR_MIN_WIDTH: u64 = 8;

/// 一条横条**在这么宽的一格里画多宽**：摆得下就照 `full` 那么宽，摆不下就一路收窄，
/// 窄过 [`BAR_MIN_WIDTH`] 就整条让掉（答 `None`）。
///
/// **横条本身有多宽不在这里**：那是调用方那一处的事，而总览块那两条与命令行那两条
/// 同一个出处（`crate::BAR_WIDTH`，总览块那一份在 `super::overview`）。这一条只答
/// 「这一格让给它几格」——**摆不下时谁让位**跨块，一块自己答不出来，
/// 与本模块别的几条因此同一个位置（停车场 Q169）。
///
/// `room` 是横条**连两个方括号一起**能占的格数。
pub(super) fn bar_width(full: u64, room: u16) -> Option<u64> {
    let inside = u64::from(room.saturating_sub(2));
    (inside >= BAR_MIN_WIDTH).then(|| inside.min(full))
}
...
mod tests {
    ...
    /// 停车场 Q169）。
    ///
    /// 让位的是**格数**，不是那个宽度本身：摆得开时答的恒是调用方给的那一个
    /// （总览块那两条与命令行那两条同一个出处，`the_bar_fills_from_empty_to_full`
    /// 钉着它）。
    #[test]
    fn a_bar_narrows_before_it_gives_way_altogether() {
        const FULL: u64 = 30;

        // 摆得开：一格不缩。方括号那两格算在 `room` 里，因此 32 是刚够。
        assert_eq!(bar_width(FULL, 100), Some(FULL));
        assert_eq!(bar_width(FULL, 32), Some(FULL), "刚够摆得开的那一档缩了");
        // 摆不开：让到剩几格就是几格。
        assert_eq!(bar_width(FULL, 31), Some(29));
        assert_eq!(
            bar_width(FULL, BAR_MIN_WIDTH as u16 + 2),
            Some(BAR_MIN_WIDTH)
        );
        // 再窄一格就整条让掉——半条横条比不画更坏。
        assert_eq!(bar_width(FULL, BAR_MIN_WIDTH as u16 + 1), None);
        assert_eq!(bar_width(FULL, 0), None, "一格都没有的格子也不恐慌");
    }

    /// **抬头摆不下时从中间省略**，两头都留得住（停车场 Q147）。
    #[test]
    fn a_title_that_does_not_fit_is_elided_in_the_middle() {
        let said = "转换 · 第 3/3 卷 · 还剩约 3m20s";
    ...
    /// 取值栏摊着的那一份会话：缩放方式那一行上按一下 `⏎`，取值就地摊在它下面。
    ...
    fn the_picker_and_the_two_overlays_still_say_their_piece_when_narrow() {
        ...
        assert!(shown.contains("缩放方式"), "{shown}");
    }
    ...
    ///
    /// **屏底那一行在这一档上折成两行**：`p3-session-legibility/12` 瘦身之后它一行摆得下，
    /// 而 `p4-parking-lot/07` 把键改成问按键表取——同义的一个不漏
    /// （`⏎ 空格`、`Esc Ctrl-C q`、`? F1`），64 列上因此又折了回去。
    /// **折行那一套本来就在**（`p2-loose-ends/07` 的目的，见下面那个循环与
    /// [`TOO_NARROW_FOR_ANYTHING`]），而 `q 退出` 一行不让那一条仍旧成立——
    /// 折出来的那几行一行不让（从前它是从行尾切掉的，每多一个键尾巴上那个键就少露一截，
    /// 停车场 Q75）。
    ///
    /// **左栏这一档上装不下，那条滚动条因此画在它右边那条框线上**（见 [`super::super::scrolling`]）：
    /// 十八行的屏留给左栏十三行，而三组设置一共二十二行。`▲`／`▼` 两头加中间那一截滑块
...
"┌配置────────────────────────────┐┌转换 · 第 3/3 ⋯ 还剩约 3m20s┐"
"│设备设置 · 判定的依据，绑面板， ▲│ 总体 3000/500⋯ · 已用 5m00s│"
"│改一次管很久                    █│ 本卷 卷三 · ⋯ 1000/3000 步 │"
...
"│  拆分跨页　　　　默认（拆）    ▼│                            │"
"└────────────────────────────────┘└────────────────────────────┘"
" ⏎ 空格 摊开取值 · t 预览 · x 转换 · Esc Ctrl-C q 退出 · ? F1 全"
" 部键                                                           "
"                                                                "
"#;
    ...
    /// 这一格就停在 [`FOOTER_HEIGHT`] 上，而 20 列上那一行此刻折出**四**行
    /// （`p4-parking-lot/07` 之后同义的键一个不漏）。**第四行连同末尾那一条
    /// 「全部键」一起裁在屏外**，屏上因此停在一个孤零零的 `· ?` 上——
    /// 「屏矮到这一格也长不动时裁的是底下」那一条在这一档上的样子，记在停车场 Q193。
    ///
...
"┌配置────────────────────────────────────────────┐┌总览────────────────────────┐"
"│设备设置 · 判定的依据，绑面板，改一次管很久     ▲│ 还没跑过。t 预览 · x 转换  │"
"│  型号　　　　　　未挑（跑起来之前必填）        █└────────────────────────────┘"
"│  可见灰阶数　　　默认（按屏幕规格）            █┌报告────────────────────────┐"
"│  画质门槛　　　　跟着型号走（先挑一个）        █│                            │"
"│                                                █│ 按 t 预览：只算不写，报告照│"
"│处理选项 · 这一趟的立场                         █│ 出。                       │"
"│  缩放方式　　　　默认（height）                █│              按 x 转换：写 │"
"│  裁白边　　　　　默认（裁）                    █│              到输出目录。  │"
"│  拆分跨页　　　　默认（拆）                    █│              跑起来之前必填│"
"│  跨页判定宽度　　默认（1.5）                   ║│              的两项是型号与│"
"│  阅读方向　　　　默认（rtl）                   ║│              输出目录。    │"
"│  缩放算法　　　　默认（lanczos3）              ║│                            │"
"│  提白上限　　　　默认（4）                     ║│                            │"
"│  灰阶档位　　　　自动（画质分说了算）          ║│                            │"
...
"│                                                ▼│                            │"
"└────────────────────────────────────────────────┘└────────────────────────────┘"
" ⏎ 空格 摊开取值 · t 预览 · x 转换 · Esc Ctrl-C q 退出 · ? F1 全部键            "
"                                                                                "
"                                                                                "
"#;
...
"┌配置────────────────────────────────────────────┐┌转换 · 第 3/3 ⋯ 还剩约 3m20s┐"
"│设备设置 · 判定的依据，绑面板，改一次管很久     ▲│ 总体 3000/500⋯ · 已用 5m00s│"
"│  型号　　　　　　未挑（跑起来之前必填）        █│ 本卷 卷三 · ⋯ 1000/3000 步 │"
"│  可见灰阶数　　　默认（按屏幕规格）            █│ 完成 1 卷 · 跳过 1 卷      │"
"│  画质门槛　　　　跟着型号走（先挑一个）        █│ 出事 隔离 1 ⋯ · 读不出 1 页│"
"│                                                █└────────────────────────────┘"
"│处理选项 · 这一趟的立场                         █┌报告────────────────────────┐"
"│  缩放方式　　　　默认（height）                █│ 记号  目录  卷数           │"
"│  裁白边　　　　　默认（裁）                    █│ !     库       2  隔离 1 卷│"
"│  拆分跨页　　　　默认（拆）                    █│坏页（出现的当场，逐页那几行│"
"│  跨页判定宽度　　默认（1.5）                   ║│在整卷跑完后才有）          │"
"│  阅读方向　　　　默认（rtl）                   ║│  库/卷二/017.jpg           │"
"│  缩放算法　　　　默认（lanczos3）              ║│    失败 解不出完整尺寸：   │"
"│  提白上限　　　　默认（4）                     ║│    JPEG 数据截断           │"
"│  灰阶档位　　　　自动（画质分说了算）          ║│                            │"
"│  抖动　　　　　　自动（画质分说了算）          ║│                            │"
"│  整卷统一灰阶　　默认（关）                    ║│                            │"
...
"│  抖动　　　　　　自动（画质分说了算）          ▼│                            │"
"└────────────────────────────────────────────────┘└────────────────────────────┘"
" 缩放方式 · ↑ ↓ j k 选 · → ⏎ 空格 定 · ← Esc 一格不改地退一步 · Ctrl-C q 退出 · "
" ? F1 全部键                                                                    "
...
"│ 记号  页名     尺寸       缩放                     判定  理由                │"
"│ *     001.jpg  1182x1680  缩放比 1.219 ⋅ 一次缩放  4bit  达标的最省空间档位  │"
"│ 代表页                                                                       │"
"│ ✗     017.jpg  1182x1680  读不出 ⋅ 用空白页占位                              │"
...
" · ? F1 全部键                                                                  "
" 只列需留意的页：差异大 · 坏页 · 残缺 · 尺寸未贴合屏幕 · 页面超宽 · 兜底上界，加"
" 上代表页                                                                       "
"#;
    ...
    /// 表砍到只剩记号 · 卷名 · 页数 · 统一档位 · 代表页。从前这一档上左栏还占着 10 列——
