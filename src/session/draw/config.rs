//! 屏上那一块：**左栏**——三组设置常驻的那一栏（`CONTEXT.md` 的《会话》：三组设置）。
//!
...
//!
//! **卷打得多了这一栏就装不下**（行数是 22 加卷数，再加摊开那一列，**再加折出来的那几行**）：
...
/// 左栏那一格的抬头：**三组设置此刻改不改得动**，改不动的话说清是哪一刻改不动。
///
/// **「只读」要看得出来**，不能是按了没反应（`CONTEXT.md` 的《会话》：
/// 一趟跑起来之后三组设置都只读）。而**只读是两个阶段**（[`Stage::read_only`]）——
/// 跑着与等待确认；一句写死「跑着」的话，停在确认点上等人拿主意的那一刻屏上说的是假话
/// （`p4-parking-lot/06` 收的停车场 Q160）。
///
/// **措辞只有这一处**：屏上说得出「三组设置此刻只读」的只有这一格抬头——
/// 压暗那一半（[`Tone::Muted`]）不上色的终端上一个字都不剩，
/// 而这一句在那样的终端上照旧在。
fn heading(stage: Stage) -> &'static str {
    match stage {
        Stage::Running(_) => "配置 · 跑着，三组设置都只读",
        Stage::Deciding(_) => "配置 · 等待确认，三组设置都只读",
...
/// 跑着与等待确认时按不动（上面那一条），**焦点切到报告区时按键全归那一块**
/// （`⇥`，ADR 0017），预设那一栏开着时同理——左栏此刻在屏上只为让人对照着看
/// （见 [`super::shell`]），反白它就是在指一个按不动的地方。
/// 压暗给的是**只读那两个阶段**（跑着与等待确认，[`Stage::read_only`]）：那两种是
...
/// 从前这一栏一点滚动都没有——行数是 22 加卷数，24 行的终端上打进三个卷，
...
pub(super) fn config(frame: &mut Frame, area: Rect, session: &Session) {
    ...
    // 只读是阶段那一维的事（跑着与等待确认两种，ADR 0017 决定第 3 条），
    // 反白还要问焦点此刻落在不落在这一栏上。
    let read_only = session.stage().read_only();
    // **反白只在焦点真落在这一栏上、而且三组设置此刻改得动时才给**：
    // 焦点在报告区（或展开着、预设栏开着）时它归那一块，跑着与等待确认时它一个键都不派
    ...
    // **折行而不是切掉**：画质门槛那一行要把**标定来源**原样带上来（spec 的 Further Notes），
...
mod tests {
    ...
    /// **跑起来之后：三组设置只读这件事在屏上看得出来**，而不是按了没反应（本票的验收）。
    ///
    ...
    fn a_run_in_progress_says_on_screen_that_the_three_layers_are_read_only() {
        ...
        // **取值栏那个键也不提**：跑起来之后摊不开，而三组设置只读那一条不因它松动
    ...
    ///
    /// 行数是 22 加卷数：24 行的终端上打进三个卷是 25 行，而那一格里面只有 22 行。
    ...
    /// **快照：等待确认时的左栏**（`p4-parking-lot/06` 的验收「快照：等待确认时的左栏」，
    /// 收的是停车场 Q160）。
    ///
    /// 钉的是**抬头那一句**：那一刻三组设置同样只读（[`Stage::read_only`]），而从前这一格
...
"#;
    ...
    ///
    /// 钉的是两件事：开头那三行让了出去（正文从「画质门槛」那一行画起——处理选项
    /// 十二项之后多让一行，滚动量是算出来的，不是记着的），
    ...
    fn the_config_column_that_does_not_fit() {
        ...
        // 一个卷都没打进来时是 22 行，22 行的格子恰好装得下——滚动条一格都不画。
...
"#;
    ...
    /// **每一行是一块面板**，带着分辨率 · PPI · 屏幕灰阶数 · 黑白／彩色——字面走
    ...
    fn the_unfolded_panels() {
        ...
        same_screen(&config_pane(&session, 52, 37), THE_UNFOLDED_PANELS);
...
"│  型号　　　　　　未挑（跑起来之前必填）          │"
"│    ● 未挑（跑起来之前必填）                      │"
"│    ○ 1072x1448 ⋅ 300 PPI ⋅ 16 级灰阶 ⋅ 黑白      │"
"│    ○ 1072x1448 ⋅ 300 PPI ⋅ 16 级灰阶 ⋅ 彩色      │"
"│    ○ 824x1648 ⋅ 300 PPI ⋅ 16 级灰阶 ⋅ 黑白       │"
"│    ○ 1236x1648 ⋅ 300 PPI ⋅ 16 级灰阶 ⋅ 黑白      │"
"│    ○ 1264x1680 ⋅ 300 PPI ⋅ 16 级灰阶 ⋅ 黑白      │"
"│    ○ 1264x1680 ⋅ 300 PPI ⋅ 16 级灰阶 ⋅ 彩色      │"
"│    ○ 1404x1872 ⋅ 227 PPI ⋅ 16 级灰阶 ⋅ 黑白      │"
"│    ○ 1404x1872 ⋅ 227 PPI ⋅ 16 级灰阶 ⋅ 彩色      │"
"│    ○ 1404x1872 ⋅ 300 PPI ⋅ 16 级灰阶 ⋅ 黑白      │"
"│    ○ 1440x1920 ⋅ 300 PPI ⋅ 16 级灰阶 ⋅ 黑白      │"
"│    ○ 1650x2200 ⋅ 207 PPI ⋅ 16 级灰阶 ⋅ 黑白      │"
"│    ○ 1860x2480 ⋅ 300 PPI ⋅ 16 级灰阶 ⋅ 黑白      │"
"│  可见灰阶数　　　默认（按屏幕规格）              │"
"│  画质门槛　　　　跟着型号走（先挑一个）          │"
"│                                                  │"
"│处理选项 · 这一趟的立场                           │"
...
"│  可见灰阶数　　　默认（按屏幕规格）              │"
"│  画质门槛　　　　画质门槛 5.123（在 boox-poke6 上│"
"│  实测，其他屏幕未验证）                          │"
"│                                                  │"
"│处理选项 · 这一趟的立场                           │"
"│  缩放方式　　　　默认（height）                  │"
