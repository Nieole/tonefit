...
//! 判据构成、掩蔽与聚合）从前摆在它上面、跟着表滚，`p3-session-legibility/12` 把那几行
...
/// | 钉住的抬头 | 无（这一趟的前提在[覆盖层](crate::session::state::Overlay::Premises)里，`i` 调得出） | 无 | **有**：这一卷的档位分布 · 定档页 · 列着几页 |
...
/// 适配方式、裁边、跨页拆分、判据构成、掩蔽与聚合那几行从前摆在表**上方**、跟着表滚
...
/// | 卷级失败 · 发现走不进去 | [出事](Tone::Trouble) | 卷根本没交出来；走不进去那一处连「那底下有没有卷」都没答上来，而两者共用一个退出码（`p4-parking-lot/11`） |
///
/// **颜色不是唯一载体**（见 [`super::paint`]）：七小结各自以自己那个词开头
/// （「非卷文件 3 个」「兜底上界 2 页」「隔离 4 卷」「卷级失败 1 卷」「发现走不进去 1 处」……），
...
fn tail_row(said: &Row) -> Painted {
    let tone = match said.kind {
        ...
        // 走不进去的地方与卷级失败同一档：两者共用一个退出码，而它比那一种更说不清
...
/// 决策点上攒着的那一份也展得开（`p3-session-legibility/10`），没做成的那几卷也停得住
/// （`p4-parking-lot/10`）；两处判据要是各问各的，就会有一个「按得动却不摆在屏上」的键。
///
...
mod tests {
    ...
    /// `007` 部分救回、`017` 是失败页因此整卷进隔离），另外三小结各补一笔：
    /// 一个非卷文件（连失败都不是）、一卷没做成（出事）、一处走不进去（同样出事，
    /// `p4-parking-lot/11`）。
    ///
...
"│设备层 · 判定的依据，绑面板，改一次管很久         ▲│ 总体 [====>   ] 3000/5000 步 · 已用 5m00s│"
"│  型号　　　　　　未挑（跑起来之前必填）          █│ 本卷 卷三 · 按档写出 1000/3000 步        │"
...
"│  抖动　　　　　　自动（判据说了算）              ▼│                                          │"
"└──────────────────────────────────────────────────┘└──────────────────────────────────────────┘"
" ⇥ 报告区 · 跑着…… · s 停（按一次收尾，再按一次中止） · Ctrl-C 退出会话（当前卷中止，盘上不留半 "
" 卷） · ? F1 全部键                                                                             "
"                                                                                                "
"#;
...
" 着…… · s 停（按一次收尾，再按一次中止） · Ctrl-C 退出会话（当前卷中止，盘上不留半卷） · ? F1 全"
" 部键                                                                                           "
"#;
...
"│  阈值　　　　　　跟着型号走（先挑一个）          █└──────────────────────────────────────────┘"
"│                                                  █┌报告 · 展开 库（第 1/1 个目录）───────────┐"
"│口味层 · 这一趟的立场                             █│ 记号  卷名  页数  档位分布  定档页       │"
"│  适配方式　　　　默认（height）                  ║│ -     卷一   180  跳过                   │"
"│  裁边　　　　　　默认（裁）                      ║│ ✓     卷二     1  4bit 1    001.jpg      │"
"│  跨页拆分　　　　默认（拆）                      ║│                                          │"
"│  拆分阈值　　　　默认（1.5）                     ║│                                          │"
"│  阅读方向　　　　默认（rtl）                     ║│                                          │"
"│  滤波器　　　　　默认（lanczos3）                ║│                                          │"
...
"│  抖动　　　　　　自动（判据说了算）              ▼│                                          │"
"└──────────────────────────────────────────────────┘└──────────────────────────────────────────┘"
" 卷表 · ↑ ↓ j k 选一卷 · ⏎ 空格 e 展开逐页 · g 回到跟随 · i 这一趟的前提 · Esc 回目录表 · ⇥ ⇧⇥  "
" 回配置 · 跑着…… · s 停（按一次收尾，再按一次中止） · Ctrl-C 退出会话（当前卷中止，盘上不留半   "
" 卷） · ? F1 全部键                                                                             "
"#;
    ...
    ///
    /// 逐条问的是那一行**长什么样**，不是屏上第几行：档位分布那一列写什么出自
    ...
    fn six_kinds_of_volume_each_get_their_own_row() {
        ...
        // 三、逐页：档位分布那一列数的是页上的判定（`two-pass-rework/02`）——这一卷一页、
        //     判成 4bit；从前这里每行都写着「逐页」。定档页那一格不在：默认那条路上没有它。
        assert!(
    ...
    /// **定档页那一列只在 `--envelope` 那条路上在场**（`two-pass-rework/02` 票面第三条：
    /// 「否则整格不在」）——默认逐页那一趟连列头都不占。
    ///
    /// 机制在 [`crate::session::columns::fit`] 的第零步（一整列都不在场就不占地方），
    /// 这一条问的是**屏上**：一趟里没有一卷走上包络时，卷表的列头一行没有「定档页」，
    /// 而档位分布那一列照旧在；有一卷走了上包络，那一列就回来。
    ...
    fn the_driver_column_is_only_there_when_a_volume_took_the_envelope_path() {
        ...
        let head = table(&live, 120, None, &only_branch(&live)).rows[0]
            .text
            .clone();
        assert!(head.contains("档位分布"), "{head}");
        assert!(head.contains("耗时"), "宽终端上别的列都在：{head}");
        assert!(
            !head.contains("定档页"),
            "默认逐页那一趟还摆着定档页那一列：{head}"
        );

        // 一卷走了上包络，那一列就回来——它是那条路在表上留下的分别。
    ...
    fn the_volume_names_come_from_the_same_place_as_the_progress_bar() {
        ...
        // 窄到一列都砍无可砍（档位分布也让掉了，`two-pass-rework/02`）：
...
"#;
    ...
    ///
    /// 左栏先让掉一截，报告区剩下二十几列——耗时、定档页与页数按
    /// [`crate::session::columns`] 那个次序砍掉，档位分布那一列还摆得下，
    /// **卷名与行首记号仍在**。
    ///
...
"│设备层 · 判定的依据，绑面板，改一次管很久       ▲│ 总体 5000/600⋯ · 已用 5m00s│"
"│  型号　　　　　　未挑（跑起来之前必填）        █│                            │"
"│  感知可分辨级数　默认（跟随面板）              █│ 完成 3 卷 · 跳过 1 卷      │"
"│  阈值　　　　　　跟着型号走（先挑一个）        █│ 出事 隔离 1 ⋯ 卷级失败 1 卷│"
"│                                                █└────────────────────────────┘"
"│口味层 · 这一趟的立场                           █┌报告 · 展开 库⋯ 1/1 个目录）┐"
"│  适配方式　　　　默认（height）                █│ 记号  卷名        档位分布 │"
"│  裁边　　　　　　默认（裁）                    █│ -     棋魂 07     跳过     │"
"│  跨页拆分　　　　默认（拆）                    █│ !     哆啦 03     4bit 1   │"
"│  拆分阈值　　　　默认（1.5）                   ║│ 隔离                       │"
"│  阅读方向　　　　默认（rtl）                   ║│ ✓     名侦探 05   4bit 1   │"
"│  滤波器　　　　　默认（lanczos3）              ║│ ✓     浪客行 12   2bit+FS 1│"
"│  纸白对齐上限　　默认（4）                     ║│ ✗     消失的那卷  没做成   │"
"│  位深　　　　　　自动（判据说了算）            ║│ 卷根不在了                 │"
"│  抖动　　　　　　自动（判据说了算）            ║│失败页（出现的当场，逐页那几│"
"│  上包络　　　　　默认（关）                    ║│行在整卷跑完后才有）        │"
"│  缓存预算　　　　默认（512.0 MiB）             ║│  库/哆啦 03/017.jpg        │"
"│  读取策略　　　　默认（auto）                  ║│    失败 解不出完整尺寸：   │"
"│                                                ▼│    JPEG 数据截断           │"
"└────────────────────────────────────────────────┘└────────────────────────────┘"
...
"#;
    ...
    ///
    /// - **裁边、跨页、彩页转灰三列连列头都不占**——这一卷一页都没裁过、没切过、
    ///   没有彩页转灰，一整列都不在场的列由 [`columns::fit`](crate::session::columns::fit)
    ///   的第零步先让掉；
    /// - **去处与判据两列让给了宽度**——砍列次序上它们中间隔着的那三列
    ///   （裁边、跨页、彩页转灰）已经由第零步让掉了，这一档上真让给宽度的就是这两列；
    /// - **缩放那一列在**，失败页那一行因此说得出「它的尺寸是卷内统一尺寸」
...
"│ 总体 [==============================] 2000/2000 步                                                                                   │"
"│ 判定 1 卷 4bit · 1 卷 跳过                                                                                                           │"
"│ 出事 特例页 1 张 · 宽溢出 1 页 · 几何门不成立 1 卷 · 失败 1 页                                                                       │"
"└──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘"
"┌报告 · 展开 卷二（第 2/2 卷）─────────────────────────────────────────────────────────────────────────────────────────────────────────┐"
"│ 档位分布 4bit 5 ⋅ 8bit 1 · 定档页 003.jpg · 要紧的页 6/8                                                                             │"
"│ 记号  页名     尺寸       缩放                       判定  理由                                                                      │"
"│ *     003.jpg  1182x1680  缩放比 1.219 ⋅ 未预缩      4bit  阈值内最低的一档          定档页                                          │"
"│ !     004.jpg  1600x1680  缩放比 1.219 ⋅ 未预缩      8bit  特例页单独定档            特例页  宽溢出                                  │"
"│ !     005.jpg  1182x1680  缩放比 1.219 ⋅ 未预缩      4bit  几何门不成立，本页不抖动  几何门不成立                                    │"
"│ !     006.jpg  1182x1680  缩放比 1.219 ⋅ 未预缩      4bit  卷级上包络                兜底上界                                        │"
"│ !     007.jpg  1182x1680  缩放比 1.219 ⋅ 未预缩      4bit  卷级上包络                救回 62.0%                                      │"
"│ ✗     017.jpg  1182x1680  失败页 ⋅ 卷内统一尺寸留白                                  失败 解不出完整尺寸：JPEG 数据截断              │"
"│                                                                                                                                      │"
"│                                                                                                                                      │"
"└──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘"
...
"└──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘"
"┌报告 · 展开 卷二（第 2/2 卷）─────────────────────────────────────────────────────────────────────────────────────────────────────────┐"
"│ 档位分布 4bit 5 ⋅ 8bit 1 · 定档页 003.jpg · 全部 8 页（要紧的 6 页）                                                                 │"
"│ 记号  页名     尺寸       缩放                       判定  理由                                                                      ▲"
"│ ✓     001.jpg  1182x1680  缩放比 1.219 ⋅ 未预缩      4bit  卷级上包络                                                                █"
"│ ✓     002.jpg  1182x1680  缩放比 1.219 ⋅ 未预缩                                      彩页 · 彩色分支：只缩放，不量化，不进灰度缓存也 █"
"│ 不进卷级上包络                                                                                                                       █"
"│ *     003.jpg  1182x1680  缩放比 1.219 ⋅ 未预缩      4bit  阈值内最低的一档          定档页                                          █"
"│ !     004.jpg  1600x1680  缩放比 1.219 ⋅ 未预缩      8bit  特例页单独定档            特例页  宽溢出                                  ║"
"│ !     005.jpg  1182x1680  缩放比 1.219 ⋅ 未预缩      4bit  几何门不成立，本页不抖动  几何门不成立                                    ║"
...
"│ !     007.jpg  1182x1680  缩放比 1.219 ⋅ 未预缩      4bit  卷级上包络                救回 62.0%                                      ▼"
"└──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘"
" ↑ ↓ j k 选一页 · a 只列要紧的页 · ⇥ 换下一卷 · Esc e 收起，左栏回来 · Ctrl-C q 退出 · ? F1 全部键                                      "
...
"└──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘"
"┌报告 · 展开 卷二（第 2/2 卷）─────────────────────────────────────────────────────────────────────────────────────────┐"
"│ 档位分布 4bit 1 · 定档页 001.jpg · 要紧的页 2/2                                                                      │"
"│ 记号  页名     尺寸       缩放                       判定  理由              判据        去处                        │"
"│ *     001.jpg  1182x1680  缩放比 1.219 ⋅ 未预缩      4bit  阈值内最低的一档  4bit 8.000  出/卷二/001.png       定档页│"
"│ ✗     017.jpg  1182x1680  失败页 ⋅ 卷内统一尺寸留白                                      出/隔离/卷二/017.png  失败  │"
"│ 解不出完整尺寸：JPEG 数据截断                                                                                        │"
"│                                                                                                                      │"
"│                                                                                                                      │"
"│                                                                                                                      │"
...
"│                                                                                                                      │"
"└──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘"
" ↑ ↓ j k 选一页 · a 列全部页 · ⇥ 换下一卷 · Esc e 收起，左栏回来 · 跑着…… · s 停（按一次收尾，再按一次中止） · Ctrl-C 退"
" 出会话（当前卷中止，盘上不留半卷） · ? F1 全部键                                                                       "
"                                                                                                                        "
"#;
...
"└──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘"
"┌报告 · 展开 棋魂 08（第 5/5 卷）──────────────────────────────────────────────────────────────────────────────────────┐"
"│ 档位分布 4bit 1 · 定档页 001.jpg · 要紧的页 1/1                                                                      │"
"│ 记号  页名     尺寸       缩放                   判定  理由              判据        去处                            │"
"│ *     001.jpg  1182x1680  缩放比 1.219 ⋅ 未预缩  4bit  阈值内最低的一档  4bit 8.000  出/棋魂 08/001.png  定档页      │"
"│                                                                                                                      │"
"│                                                                                                                      │"
"│                                                                                                                      │"
"│                                                                                                                      │"
...
"│                                                                                                                      │"
"└──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘"
" ↑ ↓ j k 选一页 · ⇥ 换下一卷 · Esc e 收起，左栏回来 · 等你拿主意…… · x 接着做第二遍（第一遍不重算） · a 剩下的卷都这样  "
" （往下不再问） · s 收尾（这一卷不写，剩下的卷也不开工） · Ctrl-C 退出会话（当前卷中止，盘上不留半卷） · ? F1 全部键    "
...
"#;
    ...
    fn the_pinned_heading_of_the_expanded_volume_never_scrolls_away() {
        ...
        assert!(top.contains("档位分布"), "抬头没钉在这一格顶上：{top}");
