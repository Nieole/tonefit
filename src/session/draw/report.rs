...
/// 判定、代表页、隔离、这一趟怎么读的、幂等命中说清哪四项依据没变，全在里面。
/// 坏页要在出现的当场看得见，那一段走 [`crate::render::failing_pages`]。
///
...
/// | 列的是 | 此刻摆得出的那几枝 | 那一枝底下那几卷 | 那一卷[需留意的页](crate::render::notable)，`a` 切全部页 |
/// | 钉住的抬头 | 无（这一趟的前提在[覆盖层](crate::session::state::Overlay::Premises)里，`i` 调得出） | 无 | **有**：这一卷的灰阶分布 · 代表页 · 列着几页 |
/// | 长过一格 | **自动滚动时钉在末行，自动滚动停了就跟着[光标那一卷](crate::session::state::Follow)走** | 同左 | 跟着[光标那一页](crate::session::state::Expansion::at)走 |
/// | 一行放不下 | 按固定次序**砍列**（[`crate::session::columns`]） | 同左，另一个次序 | 同左，第三个次序 |
/// | 成句的那几段 | 折行（按显示宽度，见 [`crate::wrap`]） | 折行 | 折行 |
/// | 左栏 | 在场 | **在场**（这一级摆得下） | 收起（见 [`super::shell`]） |
///
/// **表下面那两段（坏页、末尾那几小结）说的是整趟**，与表列的是哪一级无关：
/// 前两副都摆着它们（见 [`collapsed`]）。
///
/// 默认那一副跟着最新收摊的那一卷，是因为报告只增不减，而「一卷跑完当场看得见」说的正是
/// 刚添上去的那一行；**光标一挪自动滚动就停了**，往回翻因此不必另有一个滚动量
...
pub(super) fn report_pane(
    ...
    let Some(expansion) = session.expansion() else {
        ...
        // **视口跟着光标走**（[`Viewport`]），而[自动滚动](Follow)那一档跟的是**末行**：
        //
        // - **自动滚动着**：末行。表底下还有当场冒出来的坏页与结束之后那几小结，
        //   而它们说的正是最新那几卷的事——钉在最新那一卷那一行上会把它们推出格子。
        //   这一档与从前那一副（「滚到底」）**逐格相同**。
        // - **自动滚动停了**：光标那一行。「翻回去看第一卷」于是不必另有一个滚动量
    ...
    // 不解析这一道，下一个确认点一到，屏上就会拿另一卷的逐页冒充它
...
///
/// 到得了的只有一种：报告整个换了一趟（又按了一次预览或执行，而报告一趟一份）。
/// 确认点上那一卷收摊之后照旧指得着它自己——那一道由 [`Live::nearest`] 收在前面。
...
fn not_run_yet(starters: &Starters) -> String {
    ...
    if let Some(dry) = &starters.dry {
        said.push_str(&format!(" 按 {dry} 预览：只算不写，报告照出。\n"));
    }
    if let Some(run) = &starters.run {
        said.push_str(&format!("              按 {run} 转换：写到输出目录。\n"));
    }
    said.push_str("              跑起来之前必填的两项是型号与输出目录。");
    said
}

/// 默认那一副的正文：**卷表 · 坏页 · 末尾那几小结**。
///
/// **这一趟的前提不在这里了**（`p3-session-legibility/12` 票面第三条）：profile、
/// 缩放方式、裁白边、拆分跨页、画质分构成、细节放宽与聚合那几行从前摆在表**上方**、跟着表滚
...
fn collapsed(
    ...
    // **坏页是「出事」那一档**（spec 的《语义色》），而这一段头一行就叫「坏页」——
    ...
    // 末尾那几小结要看完整趟才给得出来（见 [`crate::render::tail`]），因此只在结束之后画。
    if live.ended() {
...
///
/// 七小结分属三档（`CONTEXT.md` 的《语义色》，收停车场 Q155）：
///
/// | 小结 | 语义 | 凭什么 |
/// |---|---|---|
/// | 非漫画文件 | [平常](Tone::Plain) | 它**连失败都不是**，退出码一格不动（`CONTEXT.md` 的《失败》） |
/// | 输出宽超过面板 · 兜底上界 · 残缺 · 隔离 | [注意](Tone::Caution) | 四样在《语义色》那张表的「注意」一档里逐条列着 |
/// | 卷转换失败 · 发现无法访问 | [出事](Tone::Trouble) | 卷根本没交出来；无法访问那一处连「那底下有没有卷」都没答上来，而两者共用一个退出码（`p4-parking-lot/11`） |
///
/// **颜色不是唯一载体**（见 [`super::paint`]）：七小结各自以自己那个词开头
/// （「非漫画文件 3 个」「兜底上界 2 页」「隔离 4 卷」「卷转换失败 1 卷」「发现无法访问 1 处」……），
/// 不上色的终端上一个字都不丢——**这一层一个空格都没添**，那几个词本来就在措辞里。
///
/// **隔离那一小结给的是「注意」不是「出事」**，虽然它那一句里也数着坏页：
/// 「隔离」在《语义色》里明写在「注意」一档，而卷表上隔离那一行给的也是它
/// （[`super::table::Mark::tone`]）——同一件事屏上两处不该是两种色。
/// 坏页自己那一档由表上方那一段（[`crate::render::failing_pages`]）与逐页表出
/// （停车场 Q197 记着这一笔）。
///
/// **一种行一条，不留 `_`**：末尾多一小结该挂哪一档是个要当场拿的主意，而一个 `_`
/// 会悄悄给它一个默认档——屏上少一种色，没人报错。**别的行逐个列在下面那一支里**：
/// 它们走不到这里（[`crate::render::tail`] 只出这七种），列出来是为了让第八小结
/// 变成**编译错误**而不是结束那一帧的恐慌——那一刻终端在备用屏、raw 模式里，
...
fn tail_row(said: &Row) -> Painted {
    let tone = match said.kind {
        ...
        // 无法访问的地方与卷转换失败同一档：两者共用一个退出码，而它比那一种更说不清
        // ——那一种点得出是哪几卷，这一种连少了几卷都答不出来（`p4-parking-lot/11`）。
        RowKind::FailedVolumeTail | RowKind::UnreachableTail => Tone::Trouble,
...
pub(super) fn report_title(session: &Session, live: Option<&Live>) -> String {
    ...
    let volumes = live
        ...
        .map_or_else(|| live.expandable(), |branch| branch.expandable());
    let at = volumes
        .iter()
...
/// 报告区那一格的抬头上说**自动滚动停了**的那句话（`CONTEXT.md` 的《会话》：自动滚动）。
///
/// **措辞只有这一处**：屏底那一行摆的是那个键（`g 回到自动滚动`，见
/// [`super::footer::report_prompt`]），不重说这件事——按键提示的家是屏底，
/// 状态的家是抬头，与展开那一副「第几卷在抬头上、键在屏底」同一条。
const FOLLOW_STOPPED: &str = "自动滚动停了";

/// **这一刻该不该在抬头上说「自动滚动停了」**。
///
/// 两个条件：光标真的[挪出去过](Follow::Stopped)，而且**报告还在长**（跑着或等待确认）。
/// 结束之后不说——那时报告不再长，跟着最新那一卷与停在某一卷上没有分别，
...
///
/// **与 `crate::session::terminal::expand` 挡在前面的那两条同一个画质分**：没跑过、或者表上一行都
/// 还没有，这几个键就无从谈起。
///
/// 问的是 [`Live::volumes`]（表上停得住的那几行），不是 `report().volumes`：
/// 确认点上攒着的那一份也展得开（`p3-session-legibility/10`），没做成的那几卷也停得住
/// （`p4-parking-lot/10`）；两处画质分要是各问各的，就会有一个「按得动却不摆在屏上」的键。
///
/// **一趟只做出没做成的卷时展开那个键照旧摆着**：按下去当场说清这一卷展不开
/// （`super::super::terminal` 的 `CANNOT_EXPAND`），与型号没挑时按 `t`／`x` 同一条待遇
/// ——那是「有第二步」的一种，不是「按不动的键」。
...
mod tests {
    ...
    /// **展开着头一枝**时报告区那一副的正文（卷表 · 坏页 · 末尾那几小结）。
    ///
    ...
    /// 一趟**末尾七小结全齐**的：末尾那几小结按小结上色要的正是它。
    ///
    /// [每种页各一张](fixture::a_page_of_every_kind)那一卷一次点起中间四小结
    /// （页面超宽 · 兜底上界 · 残缺 · 隔离——那一卷里 `004` 页面超宽、`006` 兜底上界、
    /// `007` 残缺、`017` 是坏页因此整卷进隔离），另外三小结各补一笔：
    /// 一个非漫画文件（连失败都不是）、一卷没做成（出事）、一处无法访问（同样出事，
    /// `p4-parking-lot/11`）。
    ///
    /// 七小结凑齐才问得出「分属三档」：少一小结，那一档就是拿另一小结冒充的。
    ...
    ///
    /// 从前它们是拼好的一段文字、整段给「平常」——各小结分属三档语义，而一段只上得了
    /// 一种色，末尾那几小结因此一个颜色都没有。
    ///
    /// 三件事：
    ///
    /// - **七小结一个不少，次序就是措辞那一层排的那个次序**（轻的在前、重的压尾）；
    ...
    fn every_tail_subsection_gets_its_own_tone_without_moving_a_glyph() {
        ...
        assert_eq!(rows.len(), expected.len(), "七小结没到齐");
    ...
    /// 展开着的一个会话，连同那一趟跑完的报告。
    ///
    /// 展开那一下**列的是需留意的页、光标停在头一页上**（[`Expansion::new`]）——
    ...
    fn the_volume_waiting_at_the_decision_point_expands_into_its_own_pages() {
        ...
        // 焦点切到报告区：自动滚动着的时候光标停的正是这一卷（它是表上最后一卷）。
        ...
        assert!(shown.contains(&tight("需留意的页 1/1")), "{shown}");
        // 上一卷（收摊了的那一卷）的逐页一行都不许冒充它：它那几页一页都不在。
        // 比的是**页名**：那几个词（差异大的页、页面超宽）总览块的出事行上也有一份，
        // 而这一格里说的是哪几页。
        ...
        assert!(after.contains(&tight("需留意的页 1/1")), "{after}");
    ...
    /// **快照：焦点在左栏、在目录表、在卷表、自动滚动停了，四张**（票面第八条）。
    ///
    /// 四张钉的是**屏上看得出焦点在哪**（票面第一条）与**自动滚动停了屏上说一句**
    ...
    /// 左栏三组设置因此照旧压暗、抬头照旧写着只读——四张里一张都没变。
    #[test]
    fn the_focus_and_the_stopped_follow_are_both_visible_on_screen() {
        ...
        // 一、焦点在左栏：反白落在配置那一行上……只是这一趟正跑着，三组设置只读，
        ...
        // 四、光标往回一卷：自动滚动停了，抬头说一句，屏底多出 `g 回到自动滚动`，
        // 反白跟着落到上一卷上。
...
"┌配置 · 跑着，三组设置都只读───────────────────────┐┌转换 · 第 3/3 卷 · 还剩约 3m20s───────────┐"
"│设备设置 · 判定的依据，绑面板，改一次管很久       ▲│ 总体 [====>   ] 3000/5000 步 · 已用 5m00s│"
"│  型号　　　　　　未挑（跑起来之前必填）          █│ 本卷 卷三 · 写出 [===>     ] 1000/3000 步│"
"│  可见灰阶数　　　默认（按屏幕规格）              █│ 完成 1 卷 · 跳过 1 卷                    │"
"│  画质门槛　　　　跟着型号走（先挑一个）          █└──────────────────────────────────────────┘"
"│                                                  █┌报告──────────────────────────────────────┐"
"│处理选项 · 这一趟的立场                           █│ 记号  目录  卷数  统一档位分布           │"
"│  缩放方式　　　　默认（height）                  ║│ ✓     库       2  跳过 1 ⋅ 4bit 1        │"
"│  裁白边　　　　　默认（裁）                      ║│                                          │"
"│  拆分跨页　　　　默认（拆）                      ║│                                          │"
"│  跨页判定宽度　　默认（1.5）                     ║│                                          │"
"│  阅读方向　　　　默认（rtl）                     ║│                                          │"
...
"│  抖动　　　　　　自动（画质分说了算）            ▼│                                          │"
"└──────────────────────────────────────────────────┘└──────────────────────────────────────────┘"
" ⇥ 报告区 · 跑着…… · s 停（按一次做完再停，再按一次立即停止） · Ctrl-C 退出会话（当前卷立即停   "
" 止，盘上不留半卷） · ? F1 全部键                                                               "
"                                                                                                "
"#;
...
"│  抖动　　　　　　自动（画质分说了算）            ▼│                                          │"
"└──────────────────────────────────────────────────┘└──────────────────────────────────────────┘"
" 报告区 · ↑ ↓ j k 选一枝 · ⏎ 空格 展开这一枝 · e 展开逐页 · i 这一趟的前提 · ⇥ ⇧⇥ 回配置 · 跑   "
" 着…… · s 停（按一次做完再停，再按一次立即停止） · Ctrl-C 退出会话（当前卷立即停止，盘上不留半  "
" 卷） · ? F1 全部键                                                                             "
"#;
...
"│  画质门槛　　　　跟着型号走（先挑一个）          █└──────────────────────────────────────────┘"
"│                                                  █┌报告 · 展开 库（第 1/1 个目录）───────────┐"
"│处理选项 · 这一趟的立场                           █│ 记号  卷名  页数  灰阶分布  代表页       │"
"│  缩放方式　　　　默认（height）                  ║│ -     卷一   180  跳过                   │"
"│  裁白边　　　　　默认（裁）                      ║│ ✓     卷二     1  4bit 1    001.jpg      │"
"│  拆分跨页　　　　默认（拆）                      ║│                                          │"
"│  跨页判定宽度　　默认（1.5）                     ║│                                          │"
"│  阅读方向　　　　默认（rtl）                     ║│                                          │"
"│  缩放算法　　　　默认（lanczos3）                ║│                                          │"
...
"│  抖动　　　　　　自动（画质分说了算）            ▼│                                          │"
"└──────────────────────────────────────────────────┘└──────────────────────────────────────────┘"
" 卷表 · ↑ ↓ j k 选一卷 · ⏎ 空格 e 展开逐页 · i 这一趟的前提 · Esc 回目录表 · ⇥ ⇧⇥ 回配置 · 跑   "
" 着…… · s 停（按一次做完再停，再按一次立即停止） · Ctrl-C 退出会话（当前卷立即停止，盘上不留半  "
" 卷） · ? F1 全部键                                                                             "
"#;
...
"│  抖动　　　　　　自动（画质分说了算）            ▼│                                          │"
"└──────────────────────────────────────────────────┘└──────────────────────────────────────────┘"
" 卷表 · ↑ ↓ j k 选一卷 · ⏎ 空格 e 展开逐页 · g 回到自动滚动 · i 这一趟的前提 · Esc 回目录表 · ⇥ "
" ⇧⇥ 回配置 · 跑着…… · s 停（按一次做完再停，再按一次立即停止） · Ctrl-C 退出会话（当前卷立即停  "
" 止，盘上不留半卷） · ? F1 全部键                                                               "
"#;
    ...
    fn a_report_taller_than_the_pane_keeps_its_last_lines() {
        // 没有坏页的那一趟：表就是这一格里最后那几行，问的正是「最新的那一卷还在不在」。
        let live = a_run_in_flight(false);
    ...
    /// **六种卷各有各的行**（票面第二条）：跳过、隔离、卷转换失败、逐页、覆盖、等待确认。
    ///
    /// 逐条问的是那一行**长什么样**，不是屏上第几行：灰阶分布那一列写什么出自
    /// [`crate::render::tally_column`]，行首记号与行尾那句话是这一层的事
    ...
    fn six_kinds_of_volume_each_get_their_own_row() {
        ...
        // 三、逐页：灰阶分布那一列数的是页上的判定（`two-pass-rework/02`）——这一卷一页、
        //     判成 4bit；从前这里每行都写着「逐页」。代表页那一格不在：默认那条路上没有它。
        assert!(
            body[2].contains("名侦探 05") && body[2].contains("4bit 1"),
            "{}",
            body[2]
        );
        assert!(
        ...
        // 四、覆盖：每一页都是被覆盖成的那一个候选，分布因此只有一档。
        assert!(
            body[3].contains("浪客行 12") && body[3].contains("2bit+FS 1"),
            "{}",
            body[3]
        );
        // 五、卷转换失败：记号 `✗`、灰阶分布写「没做成」、理由成句跟在后面，
        ...
        // 六、等待确认：那一卷**到此刻为止**的那一份照画，末尾标着那一句。
        assert!(
    ...
    /// **代表页那一列只在 `--envelope` 那条路上在场**（`two-pass-rework/02` 票面第三条：
    /// 「否则整格不在」）——默认逐页那一趟连列头都不占。
    ///
    /// 机制在 [`crate::session::columns::fit`] 的第零步（一整列都不在场就不占地方），
    /// 这一条问的是**屏上**：一趟里没有一卷走整卷统一灰阶时，卷表的列头一行没有「代表页」，
    /// 而灰阶分布那一列照旧在；有一卷走了整卷统一灰阶，那一列就回来。
    /// 六种卷那份夹具两条路混着，问不出这件事，因此另搭一趟全默认的。
    #[test]
    fn the_driver_column_is_only_there_when_a_volume_took_the_envelope_path() {
        let mut live = Live::new(&fixture::request(RunMode::Process), Resuming::GoesOn);
        ...
        let head = table(&live, 120, None, &only_branch(&live)).rows[0]
            .text
            .clone();
        assert!(head.contains("灰阶分布"), "{head}");
        assert!(head.contains("耗时"), "宽终端上别的列都在：{head}");
        assert!(
            !head.contains("代表页"),
            "默认逐页那一趟还摆着代表页那一列：{head}"
        );

        // 一卷走了整卷统一灰阶，那一列就回来——它是那条路在表上留下的分别。
        live.run_started(1, 1000);
        live.volume_started(Path::new("库/哆啦 03"), 1000);
        live.volume_finished(&fixture::processed_volume("哆啦 03", None));
        let head = table(&live, 120, None, &only_branch(&live)).rows[0]
            .text
            .clone();
        assert!(head.contains("代表页"), "{head}");
    }

    /// **卷按跑完的先后添上去，出事的靠行首记号跳出来、不靠位置**（票面第三条：不重排）。
    ///
    /// 比的是**整个卷名**（连卷号一起）：头一卷与末一卷同名不同号（`棋魂 07` / `棋魂 08`），
    /// 而它们恰是这条要钉的那一对——一个来自收摊了的那一列，一个是确认点上那一份。
    ...
    /// **光标停得上没做成的那一卷那一行**（`p4-parking-lot/10`，收停车场 Q159）。
    ///
    /// 它在表上占着一行，而**屏上看得见的一行就该选得中**：从前这一行
    /// [停不上去](crate::session::live::Volume)，`↑↓` 一路按下去会从它上面跨过去。
    /// **停得住不等于展得开**——那一列里仍旧没有它（[`Branch::expandable`]），
    /// 按下 `⏎` 得到的是一句话（`super::super::terminal` 的 `CANNOT_EXPAND`）。
    ///
    /// **一格不动排序**：它照旧排在收摊了的那几卷后面（停车场 Q151），
    /// 而那由 [`the_table_never_reorders_the_volumes_it_has_added`] 钉着。
    #[test]
    fn the_cursor_stops_on_the_row_of_a_volume_that_never_got_made() {
        let live = every_kind_of_volume(RunMode::DryRun, Resuming::Waits);
        let branch = only_branch(&live);

        let shown = table(&live, 120, Some(Volume::Failed(0)), &branch);

        // 列头占第零行，没做成的那一卷是表上第五行（见上面那条「不重排」）。
        assert_eq!(shown.cursor, Some(5), "光标没停在没做成的那一行上");
        assert!(
            shown.rows[5].text.contains("消失的那卷"),
            "{}",
            shown.rows[5].text
        );
        // 停得住的那几卷收着它，展得开的那几卷没有它——两件事分得开。
        assert!(branch.volumes.contains(&Volume::Failed(0)), "它停不住");
        assert!(
            !branch.expandable().contains(&Volume::Failed(0)),
            "它展得开"
        );
    }

    /// **表上没有列、别处也没有位置的那两句摆在那一卷底下**（过期副本、残缺）。
    ///
    /// 两种各有各的缺口：过期副本在末尾那几小结与总览块的出事行上**一个字都没有**，
    /// 少了这几行屏上就再也说不出「盘上还躺着上一趟那一份」；残缺末尾说得出，
    /// 但那一段结束之后才画，而跑着的那几十分钟里源文件不全这件事没人提。
    ///
    /// 它们是**句子**，因此不塞进格：缩进摆在那一行底下，整段折行。
    ///
    /// **两句都是「注意」那一档**（`p4-parking-lot/09`，收停车场 Q156）：残缺本来
    ...
    fn the_volume_names_come_from_the_same_place_as_the_progress_bar() {
        ...
        // 窄到一列都砍无可砍（灰阶分布也让掉了，`two-pass-rework/02`）：
        // 卷名从中间省略，书名与第几卷两头都还认得出。
        let narrow = table(&live, 12, None, &only_branch(&live)).rows;
        // 第五行是「消失的那卷」——十格的名字收进五格里。
    ...
    fn the_narrowest_screen_still_shows_every_mark_and_every_name() {
        ...
        // 耗时与代表页这一档上砍掉了——次序只有 `columns` 那一处出处。
    ...
    fn the_sentences_are_still_drawn_as_folded_prose() {
        ...
        //    而卷表从这一格的头一行起。默认那一套互锁一条都不咬，换一个咬得上的缩放方式。
        ...
        let said = opened_rows(&inside, 120)
            ...
            .join("\n");
        assert!(
            !said.contains("选项冲突 拆分开着，缩放方式却是 fit-inside"),
            "互锁那几行还在卷表上方：{said}"
        );
...
"└──────────────────────────────────────────────────────────────────────────────────────────────┘"
"┌报告 · 展开 库（第 1/1 个目录）───────────────────────────────────────────────────────────────┐"
"│ 记号  卷名  页数  灰阶分布  代表页   耗时                                                    │"
"│ -     卷一   180  跳过               3s                                                      │"
"│ ✓     卷二     1  4bit 1    001.jpg  1m12s                                                   │"
"│                                                                                              │"
"│                                                                                              │"
"│                                                                                              │"
"│                                                                                              │"
...
"#;
    ...
    /// **快照：同一趟，其中一卷有坏页。**
    ///
    /// 「坏页出现的当场就在主区可见，带原因」——那一段与表上那一行的隔离记号
...
"│ 记号  卷名  页数  灰阶分布  代表页   耗时                                                    │"
"│ -     卷一   180  跳过               3s                                                      │"
"│ !     卷二     2  4bit 1    001.jpg  1m12s  隔离                                             │"
"│坏页（出现的当场，逐页那几行在整卷跑完后才有）                                                │"
"│  库/卷二/017.jpg                                                                             │"
...
"#;
    ...
    /// 跳过、隔离、逐页、覆盖、卷转换失败五种各占一行——**不重排**，就是跑完的先后。
    /// 第六种（等待确认）只有接着写出那一趟到得了，它在
...
"┌转换 · 第 5/6 卷 · 还剩约 1m00s───────────────────────────────────────────────────────────────────────────────────────┐"
"│ 总体 [=========================>    ] 5000/6000 步 · 已用 5m00s                                                      │"
"│                                                                                                                      │"
"│ 完成 3 卷 · 跳过 1 卷                                                                                                │"
"│ 出事 隔离 1 卷 · 读不出 1 页 · 卷转换失败 1 卷                                                                       │"
"└──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘"
"┌报告 · 展开 库（第 1/1 个目录）───────────────────────────────────────────────────────────────────────────────────────┐"
"│ 记号  卷名        页数  灰阶分布   代表页   耗时                                                                     │"
"│ -     棋魂 07      184  跳过                3s                                                                       │"
"│ !     哆啦 03        2  4bit 1     001.jpg  1m12s  隔离                                                              │"
"│ ✓     名侦探 05      1  4bit 1              1m12s                                                                    │"
"│ ✓     浪客行 12      1  2bit+FS 1           1m12s                                                                    │"
"│ ✗     消失的那卷     -  没做成                     卷根不在了                                                        │"
"│坏页（出现的当场，逐页那几行在整卷跑完后才有）                                                                        │"
"│  库/哆啦 03/017.jpg                                                                                                  │"
"│    失败 解不出完整尺寸：JPEG 数据截断                                                                                │"
...
"#;
    ...
    ///
    /// 左栏先让掉一截，报告区剩下二十几列——耗时、代表页与页数按
    /// [`crate::session::columns`] 那个次序砍掉，灰阶分布那一列还摆得下，
    /// **卷名与行首记号仍在**。
    ///
    ...
    fn the_volume_table_on_the_narrowest_terminal() {
        let mut session = Session::new();
        let live = every_kind_of_volume(RunMode::Process, Resuming::GoesOn);
        // 那一趟跑完了：**卷表这一块只有跑过之后才进得去**，而屏底那一行照阶段
        // 摆键（一趟都没跑过时前提那一张根本不派，停车场 Q167）——不走这两步，
        // 这一张钉的就是一副真会话里到不了的屏。
        session.run_started();
...
"┌配置────────────────────────────────────────────┐┌转换 · 第 5/6 ⋯ 还剩约 1m00s┐"
"│设备设置 · 判定的依据，绑面板，改一次管很久     ▲│ 总体 5000/600⋯ · 已用 5m00s│"
"│  型号　　　　　　未挑（跑起来之前必填）        █│                            │"
"│  可见灰阶数　　　默认（按屏幕规格）            █│ 完成 3 卷 · 跳过 1 卷      │"
"│  画质门槛　　　　跟着型号走（先挑一个）        █│ 出事 隔离 1 ⋯转换失败 1 卷 │"
"│                                                █└────────────────────────────┘"
"│处理选项 · 这一趟的立场                         █┌报告 · 展开 库⋯ 1/1 个目录）┐"
"│  缩放方式　　　　默认（height）                █│ 记号  卷名        灰阶分布 │"
"│  裁白边　　　　　默认（裁）                    █│ -     棋魂 07     跳过     │"
"│  拆分跨页　　　　默认（拆）                    █│ !     哆啦 03     4bit 1   │"
"│  跨页判定宽度　　默认（1.5）                   ║│ 隔离                       │"
"│  阅读方向　　　　默认（rtl）                   ║│ ✓     名侦探 05   4bit 1   │"
"│  缩放算法　　　　默认（lanczos3）              ║│ ✓     浪客行 12   2bit+FS 1│"
"│  提白上限　　　　默认（4）                     ║│ ✗     消失的那卷  没做成   │"
"│  灰阶档位　　　　自动（画质分说了算）          ║│ 卷根不在了                 │"
"│  抖动　　　　　　自动（画质分说了算）          ║│坏页（出现的当场，逐页那几行│"
"│  整卷统一灰阶　　默认（关）                    ║│在整卷跑完后才有）          │"
"│  内存上限　　　　默认（512.0 MiB）             ║│  库/哆啦 03/017.jpg        │"
"│  读盘方式　　　　默认（auto）                  ║│    失败 解不出完整尺寸：   │"
"│                                                ▼│    JPEG 数据截断           │"
"└────────────────────────────────────────────────┘└────────────────────────────┘"
...
"#;
    ...
    /// **快照：预览那一副。**（票面第七条：预览那一张）
    ///
    /// 与执行那一副差的是抬头那一句 `dry-run` 与总览块那两行——表本身一列不变，
    /// 那正是「同一批行，两副排版」买到的东西。接着写出那一趟停在确认点上，
    /// **等待确认那一卷因此也在这张表上**（末一行）。
    #[test]
...
"┌预览 · 第 6/6 卷 · 还剩约 1m00s───────────────────────────────────────────────────────────────────────────────────────┐"
"│ 总体 [=========================>    ] 5000/6000 步 · 已用 5m00s                                                      │"
"│ 本卷 棋魂 08 · 写出 [>                             ] 0/1000 步                                                       │"
"│ 判定 1 卷 4bit · 1 卷 覆盖 2bit+FS · 1 卷 跳过 · 1 卷 逐页                                                           │"
"│ 出事 读不出 1 页 · 卷转换失败 1 卷                                                                                   │"
"└──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘"
"┌报告 · 展开 库（第 1/1 个目录）───────────────────────────────────────────────────────────────────────────────────────┐"
"│ 记号  卷名        页数  灰阶分布   代表页   耗时                                                                     │"
"│ -     棋魂 07      184  跳过                3s                                                                       │"
...
"│ ✓     浪客行 12      1  2bit+FS 1           1m12s                                                                    │"
"│ ✗     消失的那卷     -  没做成                     卷根不在了                                                        │"
"│ ✓     棋魂 08        1  4bit 1     001.jpg  1m12s  等你拿主意                                                        │"
"│坏页（出现的当场，逐页那几行在整卷跑完后才有）                                                                        │"
"│  库/哆啦 03/017.jpg                                                                                                  │"
"│    失败 解不出完整尺寸：JPEG 数据截断                                                                                │"
"│                                                                                                                      │"
...
"#;
    ...
    /// **快照：停在确认点上等待确认的那一卷在表上。**（票面第七条：等待确认那一张）
    ///
...
"#;
    ...
    ///
    /// 屏取 136 列：逐页那几行**轻松过 100 列**（票面原话）。**这一档上十一列摆不全**
    /// （`p4-parking-lot/10` 把几何那五格收进表之后，见 `crate::session::columns`）：
    ///
    /// - **裁白边、跨页、彩页转灰三列连列头都不占**——这一卷一页都没裁过、没切过、
    ///   没有彩页转灰，一整列都不在场的列由 [`columns::fit`](crate::session::columns::fit)
    ///   的第零步先让掉；
    /// - **去处与画质分两列让给了宽度**——砍列次序上它们中间隔着的那三列
    ///   （裁白边、跨页、彩页转灰）已经由第零步让掉了，这一档上真让给宽度的就是这两列；
    /// - **缩放那一列在**，坏页那一行因此说得出「它的尺寸是卷内统一尺寸」
    ///   （`p1-session/11` 的验收，停车场 Q162 记着它丢过一次）。
    ///
    /// 窄下来再砍成什么样另有一条
    /// （[`a_narrow_pane_cuts_columns_instead_of_scrolling_the_page_rows_sideways`]），
    /// 五格一格不少地摆出来在 `super::pages` 那一头
...
"│ 总体 [==============================] 2000/2000 步                                                                                   │"
"│ 判定 1 卷 4bit · 1 卷 跳过                                                                                                           │"
"│ 出事 差异大的页 1 张 · 页面超宽 1 页 · 尺寸未贴合屏幕 1 卷 · 读不出 1 页                                                             │"
"└──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘"
"┌报告 · 展开 卷二（第 2/2 卷）─────────────────────────────────────────────────────────────────────────────────────────────────────────┐"
"│ 灰阶分布 4bit 5 ⋅ 8bit 1 · 代表页 003.jpg · 需留意的页 6/8                                                                           │"
"│ 记号  页名     尺寸       缩放                     判定  理由                  画质分                                                ▲"
"│ *     003.jpg  1182x1680  缩放比 1.219 ⋅ 一次缩放  4bit  达标的最省空间档位    1bit+FS 32.000 ⋅ 2bit 20.000 ⋅ 4bit 8.000 ⋅ 8bit 2.000█"
"│ 代表页                                                                                                                               █"
"│ !     004.jpg  1600x1680  缩放比 1.219 ⋅ 一次缩放  8bit  差异大，单独判断      1bit+FS 32.000 ⋅ 2bit 20.000 ⋅ 4bit 8.000 ⋅ 8bit 2.000█"
"│ 差异大的页  页面超宽                                                                                                                 ║"
"│ !     005.jpg  1182x1680  缩放比 1.219 ⋅ 一次缩放  4bit  没贴合屏幕，不加抖动  1bit+FS 32.000 ⋅ 2bit 20.000 ⋅ 4bit 8.000 ⋅ 8bit 2.000║"
"│ 尺寸未贴合屏幕                                                                                                                       ║"
"│ !     006.jpg  1182x1680  缩放比 1.219 ⋅ 一次缩放  4bit  整卷统一灰阶          1bit+FS 32.000 ⋅ 2bit 20.000 ⋅ 4bit 8.000 ⋅ 8bit 2.000║"
"│ 兜底上界                                                                                                                             ▼"
"└──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘"
" ↑ ↓ j k 选一页 · a 列全部页 · ⇥ 换下一卷 · Esc e 收起，左栏回来 · Ctrl-C q 退出 · ? F1 全部键                                          "
...
"└──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘"
"┌报告 · 展开 卷二（第 2/2 卷）─────────────────────────────────────────────────────────────────────────────────────────────────────────┐"
"│ 灰阶分布 4bit 5 ⋅ 8bit 1 · 代表页 003.jpg · 全部 8 页（要紧的 6 页）                                                                 │"
"│ 记号  页名     尺寸       缩放                     判定  理由                  画质分                                                ▲"
"│ ✓     001.jpg  1182x1680  缩放比 1.219 ⋅ 一次缩放  4bit  整卷统一灰阶          1bit+FS 32.000 ⋅ 2bit 20.000 ⋅ 4bit 8.000 ⋅ 8bit 2.000█"
"│ ✓     002.jpg  1182x1680  缩放比 1.219 ⋅ 一次缩放                                                                                    █"
...
"│ !     005.jpg  1182x1680  缩放比 1.219 ⋅ 一次缩放  4bit  没贴合屏幕，不加抖动  1bit+FS 32.000 ⋅ 2bit 20.000 ⋅ 4bit 8.000 ⋅ 8bit 2.000▼"
"└──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘"
" ↑ ↓ j k 选一页 · a 只列需留意的页 · ⇥ 换下一卷 · Esc e 收起，左栏回来 · Ctrl-C q 退出 · ? F1 全部键                                    "
...
"#;
    ...
    /// 屏底摆着按停止那一副（按停止在这一块上照旧按得动，票面第六条），
    /// 而三组设置一个改动键都不派（左栏此刻连屏都不在）。
    #[test]
...
"└──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘"
"┌报告 · 展开 卷二（第 2/2 卷）─────────────────────────────────────────────────────────────────────────────────────────┐"
"│ 灰阶分布 4bit 1 · 代表页 001.jpg · 需留意的页 2/2                                                                    │"
"│ 记号  页名     尺寸       缩放                     判定  理由                画质分      去处                        │"
"│ *     001.jpg  1182x1680  缩放比 1.219 ⋅ 一次缩放  4bit  达标的最省空间档位  4bit 8.000  出/卷二/001.png       代表页│"
"│ ✗     017.jpg  1182x1680  读不出 ⋅ 用空白页占位                                          出/隔离/卷二/017.png  失败  │"
"│ 解不出完整尺寸：JPEG 数据截断                                                                                        │"
"│                                                                                                                      │"
"│                                                                                                                      │"
"│                                                                                                                      │"
...
"│                                                                                                                      │"
"└──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘"
" ↑ ↓ j k 选一页 · a 列全部页 · ⇥ 换下一卷 · Esc e 收起，左栏回来 · 跑着…… · s 停（按一次做完再停，再按一次立即停止） ·  "
" Ctrl-C 退出会话（当前卷立即停止，盘上不留半卷） · ? F1 全部键                                                          "
"                                                                                                                        "
"#;

    /// **快照：等待确认的时候展开的正是那一卷**（票面第五条，`p2-loose-ends/08`）。
    ///
...
"└──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘"
"┌报告 · 展开 棋魂 08（第 5/5 卷）──────────────────────────────────────────────────────────────────────────────────────┐"
"│ 灰阶分布 4bit 1 · 代表页 001.jpg · 需留意的页 1/1                                                                    │"
"│ 记号  页名     尺寸       缩放                     判定  理由                画质分      去处                        │"
"│ *     001.jpg  1182x1680  缩放比 1.219 ⋅ 一次缩放  4bit  达标的最省空间档位  4bit 8.000  出/棋魂 08/001.png  代表页  │"
"│                                                                                                                      │"
"│                                                                                                                      │"
"│                                                                                                                      │"
"│                                                                                                                      │"
...
"│                                                                                                                      │"
"└──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘"
" ↑ ↓ j k 选一页 · ⇥ 换下一卷 · Esc e 收起，左栏回来 · 等你拿主意…… · x 接着做写出环节（分析环节不重算） · a 后面的卷都写"
" 出（往下不再问） · s 做完再停（这一卷不写，剩下的卷也不开工） · Ctrl-C 退出会话（当前卷立即停止，盘上不留半卷） · ? F1 "
" 全部键                                                                                                                 "
...
"#;
    ...
    /// 表上那几格（尺寸、判定、理由、画质分）逐字能在 [`crate::render::pages`] 出的
    ...
    fn the_per_page_cells_come_from_render_and_only_for_the_volume_that_is_open() {
        ...
        // 坏页那一句原因、彩页那一句「不量化也不进整卷统一灰阶」，两句都在——
        // 它们是成句的那一格，跟在行尾（`p1-session/11` 的后两条验收）。
        // 「坏页 ⋅ 卷内统一尺寸留白」那一格此刻也在表上（缩放那一列，
        // `p4-parking-lot/10` 收停车场 Q162）——它由 `super::pages` 那一头逐格问。
        assert!(said.contains("失败 解不出完整尺寸"), "{said}");
        assert!(said.contains("用空白页占位"), "{said}");
    ...
    /// 窄了按 [`crate::session::columns`] 那个次序**砍列**——画质分一串先让掉，
    ...
    fn a_narrow_pane_cuts_columns_instead_of_scrolling_the_page_rows_sideways() {
        ...
        // 判定那一列在，画质分那一串让掉了——次序只有 `columns` 那一处出处。
        assert!(narrow.contains("判定"), "判定那一列被砍了：\n{narrow}");
        assert!(!narrow.contains("画质分"), "窄了还留着画质分：\n{narrow}");
    ...
    fn the_pinned_heading_of_the_expanded_volume_never_scrolls_away() {
        ...
        assert!(top.contains("灰阶分布"), "抬头没钉在这一格顶上：{top}");
        assert!(top.contains("代表页"), "{top}");
    ...
    /// **展开一卷没有需留意的页的：说一句话，不给一张空表**（票面第三条）。
    ///
    ...
    fn a_volume_with_no_pages_worth_listing_says_so_on_screen() {
        ...
        assert!(open.contains(&tight("这一卷没有需留意的页")), "{open}");
        assert!(
            open.contains(&tight("需留意的页 0/1")),
            "抬头没说这一副列着几页：{open}"
        );
