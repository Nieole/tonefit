...
//! ```
//!
//! **灰阶分布那一列**（`CONTEXT.md` 的《灰阶分布》，`two-pass-rework/02`）让灰阶档位齐不齐一眼看得出。
//! `--envelope` 那一趟多一列**代表页**（`087.png`），摆在分布与耗时之间；
...
//! 灰阶分布那一列写什么问 [`crate::render::tally_column`]，卷名与代表页走
...
//! 灰阶分布那一列与记号／行尾那个词说；表与报告别处都答不出的那两种（过期副本、残缺）
...
/// 一格不在场时那一列上写什么。
///
/// **只有页数用它**：它夹在卷名与灰阶分布中间，空着读起来像掉了一个数；
/// 而代表页与耗时排在末尾，空着就是「这一卷没有这件事」，不必再说一遍。
///
/// 取 `-` 而不是破折号 `—`：后者过不了 [`tonefit::width_is_stable`] 那一关
/// （停车场 Q154）。它夹在卷名与灰阶分布中间，右边还有三列。
const ABSENT: &str = "-";
...
///
/// **接住颜色的是这个记号，不是灰阶分布那一列上的那几个字。** 那几个字（「跳过」「没做成」）
...
/// （[`crate::session::columns`]），而灰阶分布那一列是砍得掉的。
...
impl Mark {
    /// 屏上那一个字符。
    ///
    /// 四个字形逐个过得了 [`tonefit::width_is_stable`] 那一关：跳过那一个从短横
...
struct Entry {
    ...
    /// 输出页数。
    pages: Option<String>,
    /// 灰阶分布：这一卷用了哪几档、各多少页；或者这一卷为什么一档都没有。
    tally: Option<String>,
    /// 代表页，只印最后那一段。**只在 `--envelope` 那条路上在场。**
    ...
    ///
    /// 它们各有各的语义，**不跟这一行走**：一卷可以正常跑完（平常）而其中几页是残缺的
    /// （要注意），两件事各说各的。
    under: Vec<Painted>,
}
...
impl Entry {
    ...
    /// 确认点上那一份是 [`Volume::Summarized`]——它不在收摊了的那几卷里
...
/// - [残缺](RowKind::Salvaged)——末尾那一小结说得出它，但那一段**结束之后才画**
///   （见 [`super::report`]），而跑着的那几十分钟里源文件不全这件事就没人说。
///
/// 别的成句的那几行表上都答得出：跳过在灰阶分布那一列上（[`crate::render::tally_column`]），
/// 逐页与覆盖那两句说的「候选从哪来」由同一列上的分布显示出来，隔离在行首记号与行尾那个词上。
/// 尺寸贴合检查那几句注解不在这里——表**有意**不带几何，追下去要展开那一卷
/// （`p3-session-legibility/11` 的逐页表），而把它们摆回来就是把这一票要消灭的
/// 那四五行长句原样搬回屏上。
///
/// **两种都是[注意](Tone::Caution)那一档。** 残缺本来就明写在《语义色》里
/// （源文件不全，这一卷交出来的页不全是完整解出来的）；过期副本从前**四档里一档都不占**，
/// `p4-parking-lot/09` 把它收进这一档（收停车场 Q156）。
///
/// 收进「注意」而不是留在「平常」：盘上躺着的那一份是**上一趟**写下的，它可能整卷都是白页
/// ——摆在文件管理器里与一本正经的书没有分别，而这一趟没有覆盖它、也没有删它。
/// 那正是「要留神的一件事」：既不是失败（进不了「出事」），也不是「不必特别看它」
/// （那才是「平常」）。**不为它编第五档**：编出来的那一档在 [`Tone`] 上没有对应的语义。
///
/// **颜色不是唯一载体**：那一句以「过期副本」四个字开头（措辞出自 [`crate::render`]，
/// 这一层一个字都没添），不上色的终端上一个字都不丢。
...
/// 代表页那一列：[那一行](RowKind::Driver)上的路径，只印最后那一段。
///
...
/// 确认点上攒着的那一份）——那一列由[分组](crate::render::grouped)算好摆在
...
fn entries(live: &Live, only: &Branch) -> Vec<Entry> {
    ...
    // 确认点上那一卷**到此刻为止**的那一份（停车场 Q52）：它还没收摊，不在报告那一列里，
...
mod tests {
    ...
    /// 画质分在 [`tonefit::width_is_stable`]。这张表画成什么样在 [`super::report`] 那一头问，
    /// 本模块只钉这一条自己说了算的事。
    #[test]
