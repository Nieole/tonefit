...
/// 判据构成、掩蔽与聚合（`p3-session-legibility/12` 票面第三条）。
///
...
"┌这一趟的前提 · Esc 关─────────────────────────────────────────────────────────┐"
"│profile kobo-libra-2：1264x1680 ⋅ 300 PPI ⋅ 16 级灰阶 ⋅ 黑白 ⋅ 阈值 5.123（盲 │"
"│测标定于 boox-poke6，其余面板未复核）                                         │"
"│适配方式 以高为准（宽随源比例，允许超出面板宽）                               │"
"│裁边 按行列墨量占比 ⋅ 墨阈 200 ⋅ 行列占比 0.5%                                │"
"│跨页拆分 跨页候选阈值 1.50 x 面板宽高比 ⋅ 装订沟定切点 ⋅ 右开（右半在先）     │"
"│判据构成 低通后的局部均值误差 ＋ 颗粒超出格点间距 21.6% 的那一部分（地板盲测标│"
"│定于 boox-poke6，其余面板未复核）                                             │"
"│判据掩蔽 平坦块不打折 ⋅ 结构越密折得越狠，压不到 0.50 以下 ⋅ 活动度 8.0 时落在│"
"│两者中点（地板与拐点均未标定占位值）                                          │"
