...
/// 表上的一行：一枝。
struct Entry {
    /// 这一枝底下**停得住的那几卷**（`CONTEXT.md` 的《会话》：自动滚动——光标恒是一卷）。
    ///
    /// **光标停在其中任何一卷上，反白的都是这一行**：这一副只是把那个光标归到一行上，
    /// 它自己不另记一个。
    ///
    /// **卷表上有几行，这里就有几卷**：一枝底下的卷全没做成时它照旧收得住那几条
    /// （[`Volume::Failed`]，`p4-parking-lot/10` 收停车场 Q159）——
    /// 与卷表上没做成那几行停得上去是同一条规矩。
    inside: Vec<Volume>,
    /// 行首记号。
    mark: Mark,
    /// 这一枝是哪个目录，**印全路径**。
...
/// （`CONTEXT.md` 的《会话》：自动滚动——屏上那个光标恒是一卷，这一副只是把它归到一行上）。
