...
//! | 语义 | 屏上 | 用在哪 |
//! |---|---|---|
//! | [平常](Tone::Plain) | 终端默认色 | 正常跑完的卷、普通的页、非漫画文件那一小结、屏底那一句报的「做成了」 |
//! | [注意](Tone::Caution) | 黄 | 隔离、残缺、页面超宽、兜底上界、尺寸未贴合屏幕、差异大的页、过期副本、屏底那一句问的「再按一次」 |
//! | [出事](Tone::Trouble) | 红 | 坏页、卷转换失败、拒绝开始、屏底那一句报的「没做成」 |
...
//! [记号](super::table)说了算）、摆在一卷那一行底下的那两句（同上，过期副本与残缺）、
//! 逐页表那几行（[`super::pages`]）、总览块的抬头与出事行（[`super::overview`]）、
//! 报告区当场冒出来的坏页那一段（[`super::report`]）、报告末尾那几小结（同上，
...
//! 出事行行首是「出事」两个字，没做成那一趟的抬头里有「没做成」，坏页那一段头一行
//! 就叫「坏页」，只读的左栏抬头上写着「跑着，三组设置都只读」，报告末尾那几小结各自
//! 以自己那个词开头（「非漫画文件」「兜底上界」「隔离」「卷转换失败」……），
//! 摆在一卷底下那两句同样如此（「过期副本」「残缺」），
...
mod tests {
    ...
    /// 一屏高到**连末尾那几小结一起摆得下**：卷表那几行下面还有坏页那一段
    /// 与那几小结，报告区一格都不必滚。
    ...
    /// 四处一次问齐，各是一种语义配一个载体：
    ///
    /// - **卷转换失败**那一行是**红**的，行首记号是 `✗`，档位那一列写着「没做成」；
    /// - **隔离**那一行是**黄**的，行首记号是 `!`，行尾那个词是「隔离」；
    /// - **跳过**那一行是**暗**的，行首记号是 `-`，档位那一列写着「跳过」；
    /// - **坏页那一段**整段**红**，而它头一行就叫「坏页」、原因那一行上还有「失败」。
    ///
    ...
    fn the_row_that_went_wrong_is_red_and_says_so() {
        ...
        // 坏页那一段：抬头与原因两行各带一个载体，两行都是红的。
        // 这一份夹具画得出这一段，靠的是那一卷的坏页**两半都喂**
        // （`super::super::probe::volume_finished`）——只喂后一半时这一段不在屏上，
        // 底下这四句一句都问不到。
        for said in ["坏页（出现的当场", "失败 解不出完整尺寸"] {
    ...
    /// 说的正是它：坏页那一段整段红，中间那几行是页的路径，而头一行就叫「坏页」）。
    ///
    ...
    ///
    /// 坏页那一段整段红（见 [`super::super::report`]），而它头一行就叫「坏页」、
    /// 逐条的原因行上还有「失败」两个字——去掉颜色一个字都不丢。
    /// 中间那几行是页的路径：**载体是那一段，不是那一段里的每一行**——一段话是一起读的。
    ///
    /// 同一屏上顺带问出事行那一条：它同时数着隔离（注意）与坏页（出事），
    ...
    ///
    /// 从前那一段整段给「平常」——那几小结分属三档语义，而一段只上得了一种色，
    /// 末尾那几小结因此一个颜色都没有。
    ///
    /// 这一条走 `TestBackend`（spec 的《Testing Decisions》：屏上那几处走快照那一路），
    /// 问的是**屏上那几行真是什么色**：非漫画文件那一小结一格都没上色（它连失败都不是）、
    /// 隔离那一小结黄、卷转换失败那一小结红。三行各自带着自己那个词——
    ...
    fn the_tail_is_painted_one_subsection_at_a_time() {
        ...
        // 非漫画文件那一小结这份夹具里没有，补一笔：三档要在同一屏上都出现过。
        ...
        // 末尾那几小结**结束之后才画**（见 `super::super::report`）。
        ...
        // **从下往上找**：总览块的出事行说的是同一批数（「隔离 1 卷 · 读不出 1 页 ·
        // 卷转换失败 1 卷」）——这一趟结束了，两处此刻相等（`p4/08` 立的分工是
        ...
        for (said, colour) in [
            ("非漫画文件 1 个", None),
            ("隔离 1 卷 · 读不出 1 页", Some(Color::Yellow)),
            ("卷转换失败 1 卷", Some(Color::Red)),
        ] {
    ...
    /// 跑着与**等待确认**——后者从前不压暗、抬头还写着「跑着」，而那一刻三组设置同样改不动
    ...
    fn the_read_only_left_column_is_muted_and_the_title_says_why() {
        ...
        for said in ["跑着，三组设置都只读", "等待确认，三组设置都只读"] {
