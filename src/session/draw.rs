...
//! # 屏上那几块各住在哪儿
//!
//! **本模块只把屏分成格子**，而**摆不下时谁让位**在 [`yielding`]——那一份次序跨着屏上
...
//! | 屏底那几行 | [`footer`] |
//!
//! **屏底那一行与 `?` 那张表共用一份**：一个键怎么写、它派的那件事怎么说（长短两份）
//! 在 [`keys`]——那两处摆的是同一批键，措辞不许各写一套（`p4-parking-lot/07`）。
//!
...
//! 屏底那一句有多重）。上面那几块各按**语义**要色（「注意」「出事」「不要紧」），
//! 一处画法都不自己挑颜色。
//!
//! # 摆不下的时候
//!
//! **谁让位、按什么次序，只有 [`yielding`] 一处**（宽度先让左栏再砍列，高度总览不砍、
...
/// （[`Tone`](crate::session::tone::Tone)——这一行讲的这件事怎么样，上不上色由 `paint` 一处定），这一份带的是终端库的
