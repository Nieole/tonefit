...
//! `NO_COLOR` 在场时怎么退，都在画法那一层的 `super::shell::paint` 一处定
//! （本仓库唯一写得出颜色名的地方）。这里答的是**画法要的是哪一种**。
//!
//! # 它一个终端都不碰
//!
//! 因此摆在 `tui` 特性**外面**（见 `super` 的《终端库在哪一半》），与 [`super::tone`] 同一侧。
//! 要它在这一侧的是会话的状态机：屏底那一句回话由状态机说出口，而说出口的那一刻就定了
...
///
/// 每一种在屏上是哪一个具名色，只在 `super::shell::paint` 那张表里。
...
pub enum Kind {
    ...
    Progress,
    /// **说明**：一整段说给人读的字。眼下**只有说明卡的全文**读它；设计稿在详情栏的
    /// 长说明与预设栏那一段上用的是同一色（那两块随 13／14 接进来）。
    /// 与[卷名](Self::Volume)同一个灰：屏上要一眼扫到的那几格用终端默认色，
    /// 一整段读物比它们收一点。
    Prose,
}
