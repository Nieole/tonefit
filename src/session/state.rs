//! 会话的状态机：三组设置、光标停在哪、哪个键在这个状态下做什么。
//!
...
//! # 此刻在做什么由两维说（ADR 0017）
//!
//! [`Stage`]（这一趟走到哪个阶段了：没跑过 / 跑着 / 等待确认 / 结束了）与
//! [`Focus`]（眼下在看什么：左栏 / 编辑一行 / 取值栏 / 报告区 / 展开着 / 预设栏）。
//! **两维各答各的**：三组设置只读、按停止与答话那几个键归第一维（[`stage_action`] 一处答完），
...
/// **展开那一副列的是哪几页**（`CONTEXT.md` 的《会话》：需留意的页）。
///
...
pub enum Action {
    ...
    Cancel,
    /// 起一趟：[预览](RunMode::DryRun)或[执行](RunMode::Process)。
    ///
    /// 两者**是同一件事的两半**，区别只在做到哪一步（`CONTEXT.md` 的《会话》：
    /// 预览就是会话里按下去的那一次 dry-run），因此是一个带参数的动作、不是两个变体——
    /// 分成两个，「预览与转换走同一条回路」这句话就得靠自觉。
    ///
    /// 真把线程起起来的那件事**不在状态机里**（见 [`Session::apply`]）：
    /// 本模块一个终端都不碰，也不该起线程。
    Start(RunMode),
    /// **按停止**：把[闩](Session::stopping)往上升一级——没按过就是做完再停，按过一次就是立即停止
    /// （ADR 0013）。
    ///
    /// 一个动作，不是两个：两级停止是**同一个键按两次**（`CONTEXT.md` 的《会话》：
    /// 「立即停止是**再按一次**」）。分成 `Finish` 与 `Abort` 两个动作，
    ...
    Stop,
    /// **在确认点上答一个字**，连同这个字[管几卷](Reach)（`CONTEXT.md` 的《会话》：
    /// 确认点、都这样）。
    ///
    /// 只在 [`Stage::Deciding`] 那个状态下派得出来，而**每一卷的预览都到得了**那个状态。
    /// 三个字各有一个键：`x` 答[继续](Instruction::Continue)——分析环节不重算，直接进写出环节；
    /// `s` 答[做完再停](Instruction::Finish)——这一卷一个字节都不写，等价于一次 dry-run；
    /// `a` 答的是**同一个继续，外加后面的卷都写出**（[`Reach::ForTheRest`]）——
    /// 往下的确认点不再停，几十卷的一趟因此按一下就挂得住。
    ///
    /// **它带着那个字，而不是像[按停止](Self::Stop)那样让状态机自己升一级**：
    /// 确认点回的是**当场那个字**，不是闩（ADR 0012 决定第 2 条）。三个键因此是三个方向，
    /// 不是同一个键按几次。
    ///
    /// **「后面的卷都写出」同样不进闩**：闩只升不降，而它是个可以是「继续」的粘性答案。
    /// 它记在观察者那一侧的「确认点的默认答案」上（`super::run::Gate`），
    /// 与库那一侧记「至今收到过最强指令」的那一格分开放——按停止时按到的那一级一格不动。
    ///
    /// [立即停止](Instruction::Abort)不从这条路出去：等待确认时按 `Ctrl-C` 是[退出会话](Self::Quit)，
    /// 而退出会话本来就走立即停止（`super::run::Running::leave`，停车场 Q63）——
    /// 那一卷等于没做，`partial` 也没留下。
    ///
    /// 把那个字送到计算线程上在 `crate::session::terminal::press` 那一层（`Running::decide`），
    /// 与[按停止](Self::Stop)同一条规矩：本模块不碰线程。
    ...
    /// （逐页那几行轻松过 100 列），而卷表那几列摆得下，左栏留着才对得上三组设置。
    ///
    ...
    /// （票面：**选中一卷**可展开逐页）。方向用 [`Step`]，与三组设置那几个取值环
    /// 同一个取值——两处都是「在一圈上挪一格」。
    ///
    ...
    Collapse,
    /// **换一副列法**：只列[需留意的页](Listing::Notable) ⇄ 列[全部页](Listing::All)
    /// （`a`，票面第二条）。
    ///
    /// **带着去哪一档**，与[切焦点](Self::Focus)同一个形状、同一条理由：
    /// 屏底那一行要说出这一下按过去是什么样，而一个 toggle 说不出。
    ///
    /// **等待确认时这个键不派它**（见 [`expanded_action`]）：那一刻 `a` 是
    /// [「后面的卷都写出」](Self::Answer)，而答话那三个键在哪一块上都按得动
    /// （ADR 0017 决定第 4 条）。两个意思撞在同一个字母上，让的是这一个——
    /// 换一副列法等得起，一条停在确认点上的线程等不起。停车场 Q161 记着这一笔。
    List(Listing),
    ...
    Pick,
    /// **套用**光标停着的那一份预设：两层整个换成它，路径与输出一格不动。
    ///
    ...
    ///
    /// **要按两下**（与两级停止、与覆盖同一个形状，ADR 0013）：第一下只说一句
    ...
    Erase,
    /// **出灰阶测试图**：按当前设备设置那块面板画一张，写到盘上（`CONTEXT.md` 的《会话》：
    /// 设备设置里还挂着灰阶测试图）。
    ///
    /// **只在光标停在设备设置上时派得出来**（见 [`Session::browsing_action`]）：
    /// 它出的那个数——可见灰阶数——正是设备设置唯一填不出来的一格，
...
/// 三组设置。分界线画在**生命周期**上，不画在「哪几个 flag 长得像」上。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Layer {
    Device,
    Taste,
...
impl Layer {
    ...
    pub fn title(self) -> &'static str {
        match self {
            Layer::Device => "设备设置 · 判定的依据，绑面板，改一次管很久",
            Layer::Taste => "处理选项 · 这一趟的立场",
            Layer::Scope => "路径与输出 · 每趟都不同，不进预设",
        }
    }
}
...
impl Field {
    ...
    /// 而 `_` 会替它默默拿成处理选项（与本模块那几个取值环同一条规矩）。
    ...
    /// 每一行是那块面板自己的 `Display`：分辨率 · PPI · 屏幕灰阶数 · 黑白／彩色），
...
impl Stage {
    ...
    /// **这是「只读」在本仓库唯一的画质分**——画法照它压暗（[`super::draw::config`]），
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum Focus {
    /// **左栏**：光标在三组设置上走（[`Session::field`] 说得出停在哪一行）。
    Config,
    /// 某一行正在被打字改。
    Editing(Edit),
    ...
    ///
    /// **跑着与等待确认时一样进得来**：几十分钟的一趟里想回头看第一卷，不必等它跑完。
    /// 那两个阶段按得动的键在这里照旧按得动（按停止、答话那三个），
    /// 而三组设置照旧一个改动键都不派——那一条归[阶段](Stage)那一维。
    ///
    ...
    /// 换回[报告区](Self::Report)，三组设置一格没动，光标也还停在原处）。
    ///
    ...
    /// 三组设置一格没动（**套用**才动，而那是用户在这一栏上按下去的那一下）。
    ///
    /// **跑着的时候开不了**：套用一份预设就是把两层整个换掉，而跑起来之后三组设置只读
    ...
    /// 一个键退得回来、退回来时三组设置一格没动的状态。
    ///
    /// **跑着的时候摊不开**：一趟跑起来之后三组设置都只读（`CONTEXT.md` 的《会话》），
    ...
    ///
    /// **进去之后除了按停止与答话，别的键一律不派**：那三个键在焦点落在哪一块上都按得动
...
pub enum Overlay {
    ...
    /// ——用户只能试。这一张把那件事摆上屏。
    Keys,
    /// **这一趟的前提**：profile · 缩放方式 · 裁白边 · 拆分跨页 · 互锁 ·
    /// 画质分构成 · 细节放宽 · 聚合（措辞出自 [`crate::render::header`]，会话不另编一份）。
    ///
...
///
/// **「任何时候」那一组是[阶段那一维](stage_action)**：按停止、答话那三个、退出会话，
...
impl KeyGroup {
    ...
    ///   ——它们与跑着、等待确认**结构上碰不到面**（决定第 8 条《两维不是笛卡儿积》）。
...
/// **自动滚动**：卷表上那个光标停在哪儿（`CONTEXT.md` 的《会话》：自动滚动）。
///
/// **自动滚动是卷表独有的**：别处的视口仍是「算出来的」（`CONTEXT.md` 的《视口》——
...
pub enum Follow {
    ...
    #[default]
    Latest,
    /// **自动滚动停了**：光标停在这一卷上，报告再长它也不动。
    ///
    /// 屏上说得出这件事（报告区那一格的抬头，见 `super::draw::report::report_title`），
    /// `g` 扳回[自动滚动](Self::Latest)——不说的话，「它怎么不跟着最新那一卷了」
    /// 在屏上没有一处答得出。
    Stopped(Volume),
}
...
pub struct Naming {
    ...
    /// （见 [`Session::name_is_taken`]）。
    ///
    /// 与两级停止同一个形状（同一个键按两次，ADR 0013），但多一条：
...
pub struct Expansion {
    ...
    /// 展开的是**哪一卷**（[`Volume`]）。
    ///
    /// **不再是「报告上第几卷」**（`p3-session-legibility/10`）：确认点上那一卷也展得开，
...
impl Expansion {
    /// 摊开 `volume` 那一卷：**默认只列需留意的页**，光标停在头一页上。
    ///
    ...
    /// 从 `at` 那一卷往一边挪一格，**两头都转一圈**——与三组设置那几个取值环同一条
    /// （见 [`around`]）。
    ///
...
/// 一个会话。
///
/// 三组设置可改、按 `t` 预览、按 `x` 执行、跑起来之后按 `s` 停（一次做完再停、再一次立即停止）、
/// 按 `⇥` 把焦点切到报告区（`↑↓` 选一卷，`⏎` 展开它的逐页，左栏跟着收起）、
/// 按 `p` 开预设那一栏（存下当前两层，或套用一份）、
/// 停在设备设置上按 `c` 出灰阶测试图、按键退出。
///
/// # 此刻在做什么由**两维**说（ADR 0017）
///
/// [这一趟走到哪个阶段了](Stage)与[眼下在看什么](Focus)——**两维各答各的**：
/// 三组设置只读、按停止与答话那几个键归第一维，`↑↓`／`⏎` 归哪一块管归第二维。
/// 「哪些键在哪个状态下有效」仍旧一处答完（[`Self::action`]），只是那张表按两维查。
/// **预览每走完一卷的分析环节都会停下来等人**（[`Stage::Deciding`]），那时按 `x` 接着做
/// 写出环节、按 `a` 后面的卷都写出、按 `s` 做完再停。
///
/// **出灰阶测试图不往这里加状态**：它一按就完，屏底说一句就是全部结果——
...
pub struct Session {
    ...
    /// 眼下在看什么（[两维](Focus)之二）。
    focus: Focus,
    /// **卷表上那个光标**停在哪儿（`CONTEXT.md` 的《会话》：自动滚动）。
    ///
...
impl Notice {
    ...
    /// **没做成**（[出事](Tone::Trouble)）：跑不起来（型号没挑、输出目录没填）、
    /// 灰阶测试图写不出去、打进去的值解析不过、这一步此刻按不动（还没跑过就想展开）。
    ...
    /// **做成了**（[平常](Tone::Plain)）：存好了、删掉了、套上了、灰阶测试图出完了。
...
impl Session {
    ...
    /// **卷表上那个光标停在哪儿**：自动滚动，还是停在某一卷上
    /// （`CONTEXT.md` 的《会话》：自动滚动）。
    ...
    ///
    /// **[自动滚动](Follow)跟着扳回去**：新的一趟从第一卷起长，而上一趟停在哪一卷上
    ...
    /// **报告区那个光标此刻停在哪一卷上**（`CONTEXT.md` 的《会话》：自动滚动）。
    ///
    /// [自动滚动](Follow::Latest)着的时候是**最新的那一卷**——它是算出来的，不是记着的：
    /// 报告长一卷它就跟着走一卷。停了的时候是停着的那一卷，而那一卷若已经不在
    /// （确认点上那一份收摊了）就落到最后一卷上（[`Live::nearest`]）。
    ///
    /// **[展开着一枝](Focus::Opened)时它收在这一枝底下**（`volume-discovery/08`）：
    /// 自动滚动着的时候最新那一卷随时可能落到**另一枝**上，而屏上摆的是这一枝的卷表——
    /// 不收的话，那一格一行都不反白，而 `⏎` 展开的会是屏上根本没有的那一卷。
    /// 收法是落到**这一枝里最新收摊的那一卷**上（[`Branch::latest`](super::live::Branch::latest)）：
    /// 自动滚动说的是「跟着最新的走」，而这一枝里最新的就是它。**不是这一枝的末一行**——
    /// 末一行可能是没做成的那一卷，而那几条排在收摊卷后面（停车场 Q151），
    /// 落上去之后这一枝再收摊几卷自动滚动都还钉在原地，与 [`Live::latest`] 治的是同一件事。
    ///
    ...
    pub fn standing(&self, live: &Live) -> Option<Volume> {
        let at = match self.follow {
            // **自动滚动跟的是最新收摊的那一卷**（[`Live::latest`]），不是表上末一行：
            // 末一行可能是没做成的那一卷，而它排在收摊卷后面——落上去之后自动滚动就再也
            // 走不动了（停车场 Q159 收进来的那一条只让光标**停得上**，不改自动滚动跟谁）。
    ...
    /// 见 [`around`]）。挪完[自动滚动就停了](Follow::Stopped)——票面第三条：光标一动就停。
    ///
    ...
    ///
    /// **只有一枝时一格不动，自动滚动也不停**（点名一个目录跑就是这一档）：转一圈回到
    /// 原地，屏上分毫不变，而把自动滚动停掉是**看不见的后果**——从此新卷收摊光标不再跟着走。
    /// 「按了没反应」比「按了偷偷改了个状态」好（`CONTEXT.md` 的《自动滚动》：
    /// 光标一挪自动滚动就停了——这一下压根没挪）。
    ...
    /// **回到自动滚动**：光标交回给最新的那一卷（`g`，票面第三条）。
    ///
    /// 已经在自动滚动时按它一格不变——它不是一个开关：自动滚动停了是**光标挪出去**的后果，
    /// 而不是一个按得回去的状态（票面：`g` 回到自动滚动，不是 `g` 切换自动滚动）。
    ...
    /// **此刻停在确认点上等人拿主意吗**（`CONTEXT.md` 的《会话》：确认点）。
    ///
    ...
    /// 当前的设备设置与处理选项，装成一份**预设**。**路径与输出不进去**——
    /// 它每趟都不同，混进预设会让人套用时误写到上一次的输出目录（`crate::preset` 的抬头）。
    ///
    /// 「不含路径与输出」在这里不必靠自觉：这一层拼得出来的只有那两个字段，
    ...
    /// 灰阶测试图写出去了：屏底说清**图在哪儿**，以及**此刻**要做对的那一件事。
    ///
    ...
    /// [预设栏](Focus::Picking)）：跑起来之后三组设置只读，那三个状态从浏览才进得去，
    ...
    /// **三组设置只读由阶段那一维说了算**，焦点落到报告区上不解锁任何一个改动键；
    /// **按停止与答话那三个键反过来在这几块上都按得动**（ADR 0017 决定第 4 条）。
    ...
    fn action_in(&self, focus: &Focus, key: Key) -> Action {
        // **掀开覆盖层那两个键归这一处**，与按停止、答话那几个归 [`stage_action`] 一处
    ...
    ///
    /// 一个键都不剩的那一组整组不出（没跑过与结束了这两个阶段上「任何时候」照旧有
    /// 退出与覆盖层那几个，因此不会整张空掉）。
    ...
    /// 几个，挑的是**动作**（「就在这一行上动手」「预览」「退出」），键与措辞一律出自这里。
    #[cfg_attr(
        not(feature = "tui"),
        allow(dead_code, reason = "只有画法与那条循环读得到，而它们在 tui 特性后面")
    )]
    ...
    ///
    /// 跑着与等待确认时一个改动键都不派（三组设置只读，`CONTEXT.md` 的《会话》），
    ...
    fn config_action(&self, key: Key) -> Action {
        ...
        match key {
            // **`⇥` 跑着与等待确认时照样切得过去**（票面：跑着的时候一样能用）——
            // 它不改三组设置里的任何一格，而几十分钟的一趟里回头看第一卷正是这一下。
            Key::Tab => Action::Focus(Pane::Report),
            // 剩下的全交给阶段那一维：那一维一个改动键都没有，
            // 「跑着与等待确认时左栏一个改动键都不派」因此是结构上成立的。
    ...
    fn browsing_action(&self, key: Key) -> Action {
        ...
        match key {
            ...
            // 出灰阶测试图。**这是唯一一个认层的键**：它出的那个数是设备设置唯一填不出来的
            // 一格，停在处理选项或路径与输出上按它没有意义（见 [`Action::Chart`]）。
    ...
    fn apply(&mut self, action: Action) -> Exit {
        match action {
            ...
            // 状态转回「跑着」就在这里；把那个字交给停在确认点上的那条线程
            // 在 `crate::session::terminal::press`（`Running::decide`）——与按停止同一条分工。
            ...
            // 切焦点与回到自动滚动两支反过来：一格报告都不必读，因此就在本模块做掉。
            ...
            // 出灰阶测试图同样要碰盘（真画图与落盘的是 `tonefit::write_calibration_chart`），
    ...
    /// 套用一份预设：**两层整个换成它，路径与输出一格不动**（票面第三条）。
    ///
    ...
    pub(super) fn took(&mut self, name: &str, preset: Preset) {
        ...
        self.says(Some(Notice::done(format!(
            "套上了「{name}」：设备设置与处理选项换成了它，路径与输出一格没动"
        ))));
    }
    ...
    ///
    /// 两下而不是一下，理由与两级停止同一条（ADR 0013：立即停止是「再按一次」）：
    ...
    ///
    /// 两下而不是一下，与覆盖同一条（ADR 0013：立即停止是「再按一次」）——
    ...
    /// 它只把状态换回浏览，三组设置一个字节都不碰——「看一眼有哪些值」不该付出改掉它的代价。
    ...
    /// 认不认大小写**是运行期探出来的**，不按平台常量猜——一个进程探一次，
    /// 那一格是这条路上唯一记得住的东西（见 `complete` 的模块文档）。
    ///
...
fn revealing(key: Key, stage: Stage) -> Option<Action> {
    match opens(key)? {
        ...
        // 而屏上不摆按不动的键——判定依据是「此刻按下去有没有第二步」，不是「这个键存不存在」。
...
/// 它因此是这一块**自己认下的一个字母**，不往下交：交下去的话，没跑过与结束了
...
fn overlay_action(covered: &Covered, key: Key, stage: Stage) -> Action {
    match key {
        ...
        Key::Down | Key::Char('j') => Action::Move(Step::Next),
        Key::Esc => Action::Cancel,
        // `q` 归这一块自己（见上）：交给阶段那一维的话，没跑过与结束了那两个阶段上
        ...
        // 另一张那个键换过去，剩下的交给阶段那一维：按停止（`s`）与答话那三个
...
/// 而跑起来之后左栏一个改动键都不派——它们与跑着、等待确认**结构上碰不到面**
/// （ADR 0017 的《两维不是笛卡儿积》）。
///
/// **`Ctrl-C` 在每一个状态下都是退出**，跑到一半也是（见 [`Key::Interrupt`]）；
/// `q`／`Esc` 只在没跑过与结束了这两个阶段上退得出去（停车场 Q63：退出会话走立即停止，
...
/// 预告确认点上按什么（`super::draw::footer` 的「接着写出」那一句，`p1-session/14`），
...
/// 跑起来之后阶段那一维派得出的键：**一个改动键都不派，只留按停止**。
///
/// 「跑起来之后三组设置只读」（`CONTEXT.md` 的《会话》）因此是结构上成立的，
/// 不是画法上把它们涂灰：改一行的那几个动作在这个阶段根本不存在。
///
/// **按停止只有 `s` 一个键，按两次**（ADR 0013：立即停止是「再按一次」）：
/// 第一次升到做完再停，第二次升到立即停止。升到立即停止之后它**不再有意义**——闩到了顶，
/// 再按一次没有更强的一级可去，因此派 [`Action::Ignored`] 而不是一个什么都不改的动作。
/// 「按了立即停止之后退不回做完再停」于是不必靠任何一处代码守着：键盘上没有那个键。
///
/// **它在焦点落在哪一块上都按得动**（[`stage_action`]，ADR 0017 决定第 4 条）：
/// 报告区上、展开着的时候、连同掀着一张覆盖层的时候，两级语义一格不变——
/// 按停止问的是「这一趟还走不走」，与眼下在看什么无关。
///
...
/// **停在确认点上等人拿主意时，阶段那一维派得出的键**：`x` 接着做写出环节，
/// `a` 后面的卷都写出，`s` 做完再停（`p1-session/14`、`volume-discovery/07`，ADR 0012）。
///
/// 那两个方向各拿一个**已经有主的键**，因为它们在这里做的正是那个键一直在做的事：
/// `x` 是执行——「接着做写出环节」就是把这一趟做完；`s` 是停——「做完再停」是它的第一级，
/// 而确认点上答做完再停之后停出来的现场恰好也是「盘上不留半卷」（这一卷一个字节都不写）。
/// 另取两个新键的话，屏上就要多记两个只在这一刻有效的记号，而它们与已有的那两个
/// 说的是同一件事。
///
/// **`a` 是这个阶段自己的键**（`all`：后面的卷都写出）。它没有一个「一直在做这件事」
/// 的旧主可借——它答的字与 `x` 逐字相同，差别只在[管几卷](Reach)，而那件事别处不存在。
/// 借 `x` 按两下也说不出它：两级停止那个形状说的是「再按一次更重一级」，
/// 而这一下不比 `x` 更重，只是更远。
///
/// **`s` 在这里不是两级停止。**跑着时 `s` 升的是[闩](Session::stopping)，一次做完再停、
/// 再一次立即停止；这里 `s` 答的是**当场那个字**，答完那条线程就接着走，没有第二次可按
/// （`CONTEXT.md` 的《会话》：确认点不是第三个检查点）。
///
/// **`x` 在这里不是「起一趟」。**浏览时 `x` 起的是新的一趟，这里它答的是眼前这一趟的
/// 那一问——两者都是「把它做出来」，而在这个阶段根本没有第二趟可起：三组设置此刻只读。
///
/// **三个键在焦点落在哪一块上都按得动**（[`stage_action`]，ADR 0017 决定第 4 条）：
/// 报告区上翻着旧卷的时候、掀着一张覆盖层的时候，这一问照旧答得出——
/// 答话问的是这一卷的写出环节做不做，与眼下在看哪一卷无关。
///
...
/// `e` 直接展开逐页，`g` 回到自动滚动，`⇥` 切回左栏。
///
/// **`↑↓` 挪的仍旧是那一个光标，而它停的恒是一卷**（`CONTEXT.md` 的《会话》：自动滚动）：
...
///
/// **`g` 回到自动滚动**（`CONTEXT.md` 的《会话》：自动滚动）：光标一动自动滚动就停了，
...
/// 不横着滚。
///
/// **`t`／`x`／`p`／`c` 归左栏**：起一趟之前要看的是三组设置，而焦点此刻不在它上面
/// （与[展开着](expanded_action)同一条）。它们一个 `⇥` 之外。
///
/// 剩下的交给[阶段那一维](stage_action)：按停止与答话那三个在这一块上照样按得动
/// （票面第五条），而 `q`／`Esc` 照旧只在没跑过与结束了那两个阶段上退得出去
...
/// **展开着一枝时的按键表**：`↑↓` 选一卷，`⏎`／`e` 展开它的逐页，`g` 回到自动滚动，
...
///
/// **`a` 切到全部页、再按一次切回来**（票面第二条）。**等待确认时它不派**：
/// 那一刻 `a` 是「后面的卷都写出」，而答话那三个键在焦点落在哪一块上都按得动
...
/// 同一个意思。两级停止那个 `s` 不给第二个键，因为**立即停止退不回做完再停**；
/// 收起退得回去，因此不必守着只有一个入口。
///
/// **跑着与等待确认时也展得开**（`p3-session-legibility/10`，推翻停车场 Q72）：
/// 按停止与答话那三个键交给[阶段那一维](stage_action)，它们与这里的选页键
/// 一个都不冲突——`p1-session/11` 记着的那第二重代价（「按停止那个键在一屏滚动键里
...
fn expanded_action(expansion: &Expansion, key: Key, stage: Stage) -> Action {
    match key {
        Key::Up | Key::Char('k') => Action::Move(Step::Back),
        Key::Down | Key::Char('j') => Action::Move(Step::Next),
        // 等待确认时这个字母归答话（见上）：交给阶段那一维，它在那儿答的是
        // 「后面的卷都写出」。
...
impl Session {
    ...
    ///
    /// 每一行印的是那块面板自己的 `Display`——**分辨率 · PPI · 屏幕灰阶数 · 黑白／彩色**，
    ...
    ///
    /// **不叫 `device_level` 一类的名字**：`CONTEXT.md` 的《会话》里**设备设置**是三组设置的
...
mod tests {
    ...
    /// 六段起换阶段——跑着、等待确认，各问一遍**焦点在左栏**与**焦点在报告区**；
    /// 七段是展开着，连同它在跑着与等待确认时的那一副；八到十段是预设那一栏；
    /// 十一、十二两段是覆盖层那两个键与覆盖层自己那一支（问的是**没跑过**那个阶段）。
    ///
    /// 每一段都问到「这个键在这里没有意义」那几个——[`Action::Ignored`] 是一个取值，
    /// 不是遗漏；而**跨两维的那几条**另在两处再问一遍：三组设置只读、按停止在报告区上照样
    ...
    fn which_keys_do_what_in_which_state() {
        ...
        // 出灰阶测试图那个键在**处理选项**上没有意义：它出的数是设备设置那一格的。
        ...
        // 起一趟、展开、预设、灰阶测试图都要先退回左栏，而那是一个键的事。
        for key in [
            Key::Tab,
            Key::BackTab,
            Key::Backspace,
        ...
        assert!(!inside.at_a_panel(), "下钻那一层上没有第三组设置可进");
        assert_eq!(session.action(Key::Enter), Action::Choose);
        assert_eq!(session.action(Key::Space), Action::Choose);
        assert_eq!(session.action(Key::Right), Action::Choose);
        assert_eq!(session.action(Key::Down), Action::Move(Step::Next));
        ...
        // 六、跑起来之后，**焦点还在左栏**：三组设置只读，预览与转换也按不动
        // （一趟里跑不了第二趟）。按得动的只剩三个：`s`（按停止）、`⇥`（切焦点）
        // 与 Ctrl-C（退出）。
        session.press(Key::Esc);
        session.run_started();
        for key in [
            ...
            Key::Left,
            Key::Right,
            // 回车与空格按不动，**取值栏因此摊不开**：三组设置只读那一条不因取值栏松动
            // （`CONTEXT.md` 的《会话》：一趟跑起来之后三组设置都只读）。
            Key::Enter,
            Key::Space,
            Key::Backspace,
            Key::Esc,
            ...
            Key::Char('e'),
            // 预设那一栏也开不了：套用一份预设就是把两层整个换掉，而这时三组设置只读。
            Key::Char('p'),
            // 出灰阶测试图也按不动：光标此刻不在设备设置上——跑着时它根本没停在任何一行上
        ...
        // **`⇥` 跑着时切得动焦点**（ADR 0017）：它不改三组设置里的任何一格，
        // 而几十分钟的一趟里回头看第一卷正是这一下。
        assert_eq!(session.action(Key::Tab), Action::Focus(Pane::Report));
        // Ctrl-C 仍旧退得出去：它在**每一个**状态下都是退出。
        assert_eq!(session.action(Key::Interrupt), Action::Quit);

        // 六之二、**停在确认点上等人拿主意**（`p1-session/14`、`volume-discovery/07`）：
        // 三组设置照旧只读，而按得动的换成了答话那三个——`x` 接着做写出环节，
        // `a` 后面的卷都写出，`s` 做完再停。
        session.at_the_decision_point(true);
        assert!(session.deciding(), "没进等待确认那个状态");
        for key in [
            Key::Up,
            Key::Down,
            Key::Left,
            ...
            Key::BackTab,
            Key::Backspace,
            // `q`／`Esc` 与跑着时同一条（停车场 Q63）：退出会话走立即停止，
            // 而那一卷此刻正停在确认点上——最容易手滑的两个键不该挂这个后果。
            Key::Esc,
            Key::Char('q'),
            // 起一趟那两个键里 `t` 仍旧按不动：这一趟还没结束。
            Key::Char('t'),
            // 别的那几个照旧：三组设置只读，报告展不开，预设栏开不了，灰阶测试图出不了。
            Key::Char('d'),
            Key::Char('e'),
            Key::Char('p'),
            Key::Char('c'),
        ...
        // 答话那三个：一个字都没变（`x` 接着做、`a` 后面的卷都写出、`s` 做完再停）。
        assert_eq!(
            session.action(Key::Char('x')),
            Action::Answer(Instruction::Continue, Reach::ThisVolume)
        );
        ...
        assert_eq!(session.action(Key::Interrupt), Action::Quit);
        // 三组设置照旧改不动，`q`／`Esc` 照旧退不出去：**两条都归阶段那一维**，
        // 焦点落在哪一块与它们无关。
        for key in [
            Key::Left,
            Key::Right,
        ...
        // 答完话回「跑着」那一副：这一块上按停止又是升闩了，两级语义一格不变，
        // 而这一块自己那四个键**跑着的时候一个不少**（票面第二条：三个阶段都用得动）。
        session.press(Key::Char('x'));
        assert!(!session.deciding(), "答完话还停在确认点上");
        ...
        assert_eq!(session.action(Key::Char('g')), Action::Follow);
        assert_eq!(session.action(Key::Tab), Action::Focus(Pane::Config));
        // 回左栏：按停止那个键在那一块上照旧是升闩——两块上是同一件事。
        session.press(Key::Tab);
        assert_eq!(session.focus(), &Focus::Config);
        assert_eq!(session.action(Key::Char('s')), Action::Stop);
        assert_eq!(session.action(Key::Char('x')), Action::Ignored);

        // 结束之后配置又改得动，而按停止那个键在浏览时没有意义——还没有东西可停。
        ...
        session.go_to(Field::Out);
        assert_eq!(session.action(Key::Char('e')), Action::Expand);
        // **`⇥` 结束之后照旧切得过去**：报告一行不少地摆在那儿。
        assert_eq!(session.action(Key::Tab), Action::Focus(Pane::Report));

        // 六之四、**结束了、焦点落在报告区**：`↑↓`／`⏎`／`g`／`⇥` 与跑着时逐字相同，
        ...
        assert_eq!(session.action(Key::Esc), Action::Quit);
        assert_eq!(session.action(Key::Interrupt), Action::Quit);
        // 起一趟那两个键归左栏：起一趟之前要看的是三组设置，而焦点此刻不在它上面。
        for key in [
            Key::Char('t'),
            Key::Char('x'),
            Key::Char('s'),
        ...
        // 选页那几个键一格不变，而阶段那一维那几个键跟着进来——按停止按得动，
        ...
        // **等待确认时答话那三个都在**：与焦点落在哪一块无关（票面第五条）。
        // `a` 这一刻归答话——换一副列法等得起，一条停在确认点上的线程等不起
        ...
        for key in [
            ...
            Key::Char('x'),
            Key::Char('e'),
            // 出灰阶测试图也一样：这一栏开着时左栏虽在屏上，光标却归这一栏管
            // （见 `move_cursor`），设备设置那三行此刻停不上去。
            Key::Char('c'),
        ] {
            assert_eq!(
                session.action(key),
        ...
        assert_eq!(session.action(Key::Interrupt), Action::Quit);
        // **除了按停止与答话，别的键一律不派**：`q` 也在内——这一块上的「退一步」是 `Esc`，
        // 而它是这一块自己认下的一个字母，不交给阶段那一维（`overlay_action`）。
        // **按停止与答话那几个这一刻同样不派**，但那是**阶段**那一维说的：一趟都没跑过时
    ...
    /// 要按几下不必用户自己数。
    ///
    /// 先跑一趟再结束：[前提那一张](Overlay::Premises)**一趟都没跑过时根本不派**
    ...
    fn an_overlay_covers_one_block_and_gives_it_back() {
        ...
        // 「只列需留意的页」。问 `self.focus` 的话这里会答成「列全部页」——
    ...
    /// **覆盖层掀着时按停止与答话那三个照样按得动**（ADR 0017 决定第 4 条，
    /// `p4-parking-lot/06` 收的停车场 Q164）：这一块与左栏、报告区、展开着一样，
    /// 把自己不认的键交给 [`stage_action`]。
    ///
    /// 三件事：
    ///
    /// - **跑着时 `s` 是按停止，两级语义一格不变**——一次做完再停、再一次立即停止，到顶不再有
    ///   （闩升的是同一格，不因为屏上盖着一张纸而另算一份）；
    /// - **等待确认时 `x`／`a`／`s` 三个各答各的**，与它们在别的每一块上逐字同一件事；
    /// - **按停止与答话都不关掉这一张**：它盖住的那一块原样在底下，`Esc` 照旧还得回来。
    ///
    ...
    fn the_overlay_hands_the_stage_keys_back() {
        ...
        assert_eq!(session.action(Key::Char('s')), Action::Ignored);
        assert!(session.overlay().is_some(), "按停止把这一张关掉了");

        // 等待确认：那三个各答各的，答的是当场那个字，不是闩。
        let mut session = Session::new();
        session.run_started();
        session.at_the_decision_point(true);
        session.press(Key::Char('?'));
    ...
    /// **这一块自己认下的那几个字母不往下交**——「除了按停止与答话，别的键一律不派」
    /// 的另一半（`p4-parking-lot/06`）。
    ///
    /// 三个键，四个阶段各问一遍：
    ///
    /// - **`Esc` 恒是关掉这一张**，不是退出会话——没跑过与结束了那两个阶段上
    ...
    ///   会在四组里各出现一次；
    /// - **只列此刻这个阶段派得出的键**：跑起来之后按停止在这张表上，没跑过时它不在。
    #[test]
    ...
    ///
    /// 画质分不是「这个键存不存在」，是「**此刻按下去有没有第二步**」。两处来源：
    ///
    ...
    fn what_was_never_said_is_not_written_and_a_default_that_was_said_is() {
        ...
        // 把缩放方式转到**恰好等于默认值**的那一档上：屏上从「默认（…）」变成那个值本身。
        let mut session = Session::new();
    ...
    /// 而不是它与上一次配置合出来的某种东西。路径与输出那一半在这里就地验：
    ...
    ///
    /// 收起不是删掉：三组设置一格没动，光标还停在原处——那正是「一键回到配置」的意思。
    ///
    ...
    /// **报告区那个光标选得动一卷，而自动滚动一动就停**（票面第二、三条）。
    ///
    /// 四件事一条问齐：自动滚动着的时候光标是**算出来的**（一卷收摊它就跟着走一卷）；
    /// 挪一下自动滚动就停了，报告再长它也不动；两头都绕得回去（与左栏那一列同一条）；
    /// `g` 把它交回给自动滚动。
    ///
    ...
    fn the_report_cursor_picks_a_volume_and_stops_following_the_moment_it_moves() {
        ...
        assert_eq!(session.follow(), Follow::Latest, "一卷都没有却停了自动滚动");
    }

    /// **目录表上挪的是一枝，展开一枝之后挪的才是一卷**（`volume-discovery/08`
    /// 票面第二条）。
    ///
    /// 屏上那个光标**恒是一卷**（`CONTEXT.md` 的《会话》：自动滚动）：目录那一级只是把它
    ...
    fn the_cursor_moves_by_branch_on_the_directory_table_and_by_volume_inside_one() {
        ...
        // 自动滚动着：光标停在最新**收摊**的那一卷上，也就是乙那一枝——丙那一卷没做成，
        // 自动滚动不落到它身上（[`Live::latest`]）。
        assert_eq!(session.standing(&live), Some(Volume::Settled(2)));
        ...
        assert_eq!(
            ...
            "自动滚动着的那一卷不在这一枝上时该落到这一枝的末一卷再往下转一格"
        );
        session.select(&live, Step::Next);
    ...
    /// **自动滚动收进一枝底下时落的是这一枝里最新收摊的那一卷，不是它的末一行**
    /// （`p4-parking-lot/10`，收停车场 Q159 之后要守住的那一半）。
    ///
    /// 展开着一枝而自动滚动着的那一卷落在**另一枝**上时，光标要收到这一枝里来
    /// （`volume-discovery/08`）。收到**末一行**的话它会落在没做成的那一卷上——
    /// 那几条排在收摊卷后面（停车场 Q151），从此这一枝再收摊几卷光标都还钉在原地，
    /// 而屏上写着「自动滚动着」。收法因此是 [`Branch::latest`](super::live::Branch::latest)，
    /// 与 [`Live::latest`] 逐条同形。
    #[test]
    ...
    /// **只有一枝时，目录表上按 `↑↓` 一格不动，自动滚动也不停**（`volume-discovery/08`）。
    ///
    /// 点名一个目录跑就是这一档，仓库里的夹具也全是它。转一圈回到原地，屏上分毫不变，
    /// 而把自动滚动停掉是**看不见的后果**——从此新卷收摊光标不再跟着走
    /// （`CONTEXT.md` 的《自动滚动》：光标一挪自动滚动就停了，而这一下压根没挪）。
    #[test]
    fn one_branch_alone_neither_moves_the_cursor_nor_stops_the_follow() {
        ...
        session.select(&live, Step::Next);
        assert_eq!(session.follow(), Follow::Latest, "一枝独走却停了自动滚动");
        session.select(&live, Step::Back);
        assert_eq!(session.follow(), Follow::Latest);
        // 自动滚动照旧跟着走：又收摊一卷，光标落到它上面。
    ...
    fn the_cursor_stays_inside_the_branch_that_is_open() {
        ...
        // 下一卷收摊在**另一枝**上：自动滚动着的那一卷跑到了乙，而屏上摆的是甲的卷表。
    ...
    /// **确认点上那一卷停得住、也展得开**（spec 的《焦点与两维模式》第五条）。
    ///
    ...
    fn the_volume_waiting_at_the_decision_point_is_one_the_cursor_can_stand_on() {
        ...
        assert_eq!(live.volumes().len(), 3, "新的确认点没进表");
        assert_eq!(
            session.standing(&live),
            Some(Volume::Settled(1)),
    ...
    /// **两维各改各的**（ADR 0017）：切焦点不动阶段，结束了不动焦点。
    ///
    /// 票面第四条与第五条一条问齐：**三组设置只读由阶段那一维说了算**——焦点落到报告区上
    /// 一个改动键都不解锁；**按停止在报告区上照样按得动，两级语义不变**——
    ...
    fn the_two_dimensions_move_one_at_a_time() {
        ...
        // 三组设置照旧改不动：改动键归左栏，而左栏此刻一个改动键都不派。
        assert_eq!(session.action(Key::Left), Action::Ignored);

        // 按停止在这一块上照样按得动，而且照旧是两级（ADR 0013：立即停止是再按一次）。
    ...
    /// 「翻到底了」在屏上没有落点（与三组设置那几个取值环不同，见 [`Session::move_cursor`]）。
    #[test]
    ...
    /// **跑着 ⇄ 等待确认是同一趟的两副样子**（`p1-session/14`）：转过去、转回来，
    /// 中间那个闩一格没动。
    ///
    /// 闩非钉不可：确认点上答的字**不是闩**（ADR 0012 决定第 2 条）。分析环节里按下的做完再停
    /// 要在答完话之后照旧作数——被确认点抹掉的话，那一趟会一直跑到最后一卷。
    /// 反过来，答话也不该把闩往上推：`s` 在确认点上答的是「这一卷的写出环节不做了」，
    /// 而不是「这一趟不走了」。
    #[test]
    fn the_decision_point_is_a_second_face_of_the_same_run_and_leaves_the_latch_alone() {
        let mut session = Session::new();
        // 没跑着的时候这一问不作数：确认点是一趟跑起来之后才有的事。
        session.at_the_decision_point(true);
        assert_eq!(session.focus(), &Focus::Config, "没跑着也进了等待确认");
        ...
        // 答一个做完再停：状态当场转回「跑着」，而闩仍旧是那一级——答话不是升闩。
        ...
        // **「后面的卷都写出」同样不动那个闩**（`volume-discovery/07` 票面第四条）：
        ...
        // 那一趟停在确认点上被立即停止时收不到「跑完」以外的东西，结束那一下照样回浏览。
        session.at_the_decision_point(true);
        assert!(session.deciding());
        session.run_finished();
        assert_eq!(session.focus(), &Focus::Config, "等待确认时结束没回浏览");
        assert_eq!(session.stopping(), Instruction::Continue);
    }

    /// **两级停止：同一个键按两次。** 一次做完再停，再一次立即停止，第三次没有意义。
    ///
    /// 两级的定义在 ADR 0013：做完再停是当前卷跑完才停，立即停止是当场停。
    ...
    fn one_key_pressed_twice_is_the_two_stage_stop() {
        ...
        // 按停止之后会话仍开着——停下来不是退出（本票的验收）。
        assert_eq!(session.press(Key::Char('s')), Exit::Stay);
        assert!(matches!(session.stage(), Stage::Running(_)));

        // 那一趟结束：回到浏览，配置又改得动，闩跟着这一趟一起走。
        session.run_finished();
        assert_eq!(session.focus(), &Focus::Config);
        assert_eq!(session.stopping(), Instruction::Continue);
    ...
    /// 左栏就是三块，次序是设备设置 → 处理选项 → 路径与输出，各项都在。
    #[test]
    ...
    /// 十行里八行印成`默认（…）`，灰阶档位与抖动那两行印的是「自动（画质分说了算）」——
    ...
    /// 才谈得上「一格不改」。
    ///
    /// 比的是三组设置的全部内容，不是那一行印出来的那句话：`Esc` 不该改动**任何**一格
    ...
    fn escaping_out_of_the_unfolded_values_changes_not_one_cell() {
        ...
        for field in unfoldable() {
            ...
            assert_eq!(session.device, before.device, "{field:?} Esc 改了设备设置");
            assert_eq!(session.taste, before.taste, "{field:?} Esc 改了处理选项");
            assert_eq!(session.scope, before.scope, "{field:?} Esc 改了路径与输出");
        }
    }
    ...
    fn unfolding_and_turning_in_place_write_the_same_cell() {
        for field in unfoldable() {
            ...
            for (at, cell) in cells.iter().enumerate() {
                ...
                assert_eq!(
                    picked.device, turned.device,
                    "{field:?} 第 {at} 格：设备设置"
                );
                assert_eq!(picked.taste, turned.taste, "{field:?} 第 {at} 格：处理选项");
            }
        }
    }

    /// 型号那一行摊开的**第一层是面板**，每一行带着那块面板的
    /// 分辨率、PPI、屏幕灰阶数、黑白／彩色（`p3-session-legibility/06` 票面第一条）。
    ///
    ...
    /// **两条路走完之后设备设置逐格相同**。写型号只有 [`Session::set_device`] 一条路，
    /// 两条路各写一份的话，这里当场红。
    #[test]
    fn picking_a_model_and_turning_in_place_write_the_same_cell() {
        for (at, (_, devices)) in Profile::devices_by_panel().into_iter().enumerate() {
            for (step, device) in devices.iter().enumerate() {
                ...
                assert_eq!(
                    picked.device.gray_levels, None,
                    "{device}：屏幕灰阶数没清空"
                );
                assert_eq!(picked.device.threshold, None, "{device}：画质门槛没清空");
            }
        }
    }
    ...
    /// 才谈得上「一格不改」。比的是三组设置的全部内容：换掉型号会连带清掉标定出来的两个数，
    /// 那种连带同样落在这个断言里。
    #[test]
    ...
    ///
    /// 与跨页判定宽度同型：走到那一行、`⏎` 进编辑、打一个数、`⏎` 收下。**打 0 是「关」，
    ...
    fn the_white_align_limit_is_edited_in_the_session_and_takes_effect_next_run() {
        ...
        // 五、跑起来之后这一行与别的行一样只读——跑着与等待确认两个阶段都是
        // （[`Stage::read_only`]）：回车不进编辑，打的字一个都不落；
        // 结束之后又改得动——趟与趟之间改，正是会话的定义。
        session.run_started();
        session.go_to(Field::WhiteAlignLimit);
        ...
        assert_eq!(
            session.taste.white_align_limit, None,
            "跑着时改动了处理选项"
        );
        session.at_the_decision_point(true);
        assert_eq!(
        ...
        session.run_finished();
        assert_eq!(session.action(Key::Enter), Action::Edit, "结束之后该改得动");
    }
