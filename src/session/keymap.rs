...
impl Deed {
    ...
    /// （`CONTEXT.md` 的《会话》：确认点）。
    ///
    /// **「哪一件答哪个字」只有这一处**：终端层按 [`Deed`] 查它
    /// （`super::terminal::input` 把答出来的字交给停在确认点上的那条线程）。
    ///
    /// 那两个方向各拿一个**已经有主的键**，因为它们在这里做的正是那个键一直在做的事：
    /// `x` 是转换——「接着做写出环节」就是把这一趟做完；`s` 是停——「做完再停」是它的第一级，
    /// 而确认点上答做完再停之后停出来的现场恰好也是「盘上不留半卷」。**`a` 是这个阶段自己的键**
    /// （`all`）：它答的字与 `x` 逐字相同，差别只在[管几卷](Reach)，而那件事别处不存在。
    /// **`s` 在这里不是两级停止**：它答的是当场那个字，答完那条线程就接着走、闩一格不动
    /// （`CONTEXT.md` 的《等待确认》）。
    ///
    /// - `x`：接着做写出环节，**只管这一卷**；
    ...
    #[cfg_attr(
        not(feature = "tui"),
        allow(dead_code, reason = "只有终端层读得到，而它在 tui 特性后面")
    )]
...
pub const TABLE: &[Row] = &[
    ...
    // 同一件事在预设栏上**两句**，调用方点名要哪一句（与 `s` 的两句停止、`⏎` 的
    // 「确定 / 跳到结果」同一副）：屏底那一行摆「返回」（上面那一行），而**顶上一条预设**
    // 照旧摆「预设」——那一条写的是这个键把人带到哪一栏，掀着的时候也是这一句
    // （设计稿 `drawConfig` 的那两件与 `S.cfg.presets` 无关）。
    row(
        Group::Config,
        Deed::Presets,
        key('p'),
