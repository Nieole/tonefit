...
/// 卷内统一的那个尺寸：坏页按它留白占位（12 号票：卷内尺寸保持一致）。
///
/// 取处理成了的那些页里**出现次数最多**的那个尺寸，并列时取先出现的。漫画卷内绝大多数页
/// 同一个尺寸，众数因此就是「这一卷看上去的样子」；取最大值会让一张跨页把整卷的空白占位页撑宽，
/// 取第一页则会被卷首的封面或彩页带偏。
///
...
/// 一张纸白的页：坏页留在输出里的那个**空白占位页**。
///
/// 白而不是别的什么——空白占位页顶住页序与尺寸，但不冒充内容，也不该往页上添一笔本来没有的墨。
...
/// 汇总：先逐页定档，再把它们收成卷级的一个统一档位（ADR 0006：位深按卷取上包络并加迟滞）。
///
/// 夹在两遍之间——要看完整卷才做得了，而写出环节此刻已经不必回头碰源页（ADR 0005）。
/// 返回的逐页判定与 `pages` 等长同序，写出环节读的就是它。
///
/// **只有灰度路径上的页进来。**另外两种页没有画质分曲线：彩色分支上的页不该有——ADR 0006
/// 决定第 5 条说彩页在彩色 profile 下「根本不进灰度整卷统一灰阶」；坏页则是没有可求画质分的像素
/// （12 号票）。两者在返回的判定里都占位为 `None`，位置留着——写出环节与报告都按页序取。
///
/// **第一刀按尺寸贴合检查切**（ADR 0007 决定第 2 条）。门成立的页与未贴合屏幕的页候选集不是同一套：
/// 后者少了抖动那一维，而整卷统一灰阶取的是 (灰阶档位, 抖动模式) 这个组合——候选集不同的页
/// 排不进同一条序列。卷级那一层因此只在其中**一组**上做，其余页取门成立的那一组；
/// 那一组一页都没有时才轮到另一组，摘一页是为了护着别人，而那时没有别人可护
/// （ADR 0007 决定第 5 条）。
///
/// 摘出去的那一组**不单独定档**：它们跟着卷级统一档位的灰阶档位走、不低于它，抖动关掉
/// （ADR 0007 决定第 3 条）。门只拿走抖动，不拿走档次——让它们各按自己那条曲线定，
/// 灰阶档位那一维也会跟着逐页变，而它们并没有偏离卷内分布，摘它们的理由是几何。
/// 反过来只给统一档位也不行：抖动被拿走之后同一档灰阶档位保真更差，那一页可能真的还要高一档。
///
/// **第二刀按页残缺切**（04 号票）。残缺页有画质分曲线，那条曲线却是在一页大半留白的图上
/// 求出来的，代表不了这一卷。它因此不进整卷统一灰阶，按自己那条曲线单独定档——与差异大的页同一个待遇
/// （ADR 0006 决定第 5 条），只是摘它的理由是页残缺，不是画质分偏离。
...
/// 拿的是「统一档位的灰阶档位，不低于它，抖动关掉」，不是 04 号票那条「按自己那条曲线单独定档」。
/// 摘残缺页的理由是它那条曲线不具代表性——一页大半留白，误差恒为零，判出来必偏低——
/// 而不具代表性的曲线更没有资格把这一页压到统一档位以下。
///
/// 逐页定档也落在这里，而不在分析环节：摘出去的那两组都要拿卷级统一档位当参照，
/// 而那一档要看完整卷才定得下来。
///
/// 两条出口走不到整卷统一灰阶，各有各的道理：默认那条路上灰阶档位逐页各判各的，整卷统一灰阶要显式
...
fn summarize_volume(
    ...
    //
    // 序号非带不可：卷级每一步都只在其中一部分页上做——彩页与坏页根本不在场，
    // 未贴合屏幕的页与残缺页各自摘出去——手上那个序列与卷内页序早就不重合了。
    ...
    if !request.envelope {
        // 默认那条路：每一页拿到画质分说它要的那一档，卷级一层都没有（ADR 0018 决定第 2 条）。
        // 上面那一刀分的两组、摘出去的残缺页，在这里都不必再问——没有统一档位要定，
    ...
    // 代表页的序号在整卷统一灰阶那一侧指进**其余页**的序列，报告里那个序号指进整卷的页。
    // 卷内混着彩页、未贴合屏幕的页或残缺页时两者不重合，这一步把它换回去——不换，
    // 报告会指着另一页说「就是它定的档」。
...
/// 覆盖项裁到只剩一个候选时的那一个：判定被顶掉，画质分说什么都不改变结果（spec 的 story 23）。
///
/// 「裁到只剩一个」与「有覆盖项」两条都要：`--gray-levels 2` 撞上尺寸未贴合屏幕同样只剩一个候选，
...
/// 分析环节产出的一张**输出页**：它从哪一个源页来、写到哪儿去，有没有处理成，
/// 以及处理成了的话留下了什么。
///
/// 一个源页产出**一到多张**（见 [`split`]）。因此 `source` 会重复
/// ——同一源页切出来的几张都指着它——而 `target` 一定不重复
/// （见 [`ensure_one_member_per_output`]）。分析环节之后管线上处处按输出页数事：
/// 汇总、整卷统一灰阶的序号、写出环节写页、报告里的逐页结果，一律以它为单位。
struct OutputPage {
...
/// 一页在分析环节的结局。
///
/// 与报告那一侧的 [`PageOutcome`] 同形而不同物：这里装的是**写出环节要用的东西**
...
enum Outcome {
    ...
    Processed {
        size: Size,
        /// 这一页裁掉了多少白边（页几何批 02 号票）。裁白边在适配之前，`size` 由裁完的尺寸算出。
        crop: Crop,
        /// `size` 是不是**兜底上界**改出来的（07 号票，见 [`FitMode::target`]）。
        ///
        /// 它跟着页走，不由报告那一侧按尺寸倒推：算目标尺寸的地方只有一处，
        /// 倒推一遍就是第二处——与尺寸贴合检查那一条同一个理由（见 [`Branch::Gray`] 的 `gate`）。
        backstopped: bool,
        /// 这一张是那一刀的产物：切在哪条中缝上、是哪一侧。整页出的是 `None`（04 号票）。
    ...
    /// 坏页：字节读不出来，或者连完整尺寸都解不出来（12 号票）。
    ///
    /// 它在这里仍然占着自己那一格——页序不因为一页坏了就错位，
    /// 写出环节照样给它写一张卷内统一尺寸的白页。
    Failed { reason: String },
}

/// 一页在分析环节里走的那条分支，连同它留给写出环节的东西。
///
/// 两条分支留下的不是同一套：灰度路径留画质分曲线与缓存序号，彩色分支留编好的字节
/// （ADR 0005 决定第 4 条）。
enum Branch {
    /// 灰度路径。
    Gray {
        scores: Vec<CandidateScore>,
        /// 这一页的尺寸贴合检查判定（ADR 0007 决定第 1 条：门逐页判）。
        ///
        /// 它跟着页走，不由汇总那一步按尺寸重算：判定尺寸贴合检查的地方只有一处
        /// （[`GeometryGate::of`]，ADR 0003 要求灰阶硬上界与抖动判定同源），
        /// 而重算一遍就是第二处。
        gate: GeometryGate,
        /// 这一页在缓存里的序号。
        ///
        /// 序号跟着页走，不由写出环节数数补出来：彩页在彩色 profile 下不进灰度缓存
        /// （ADR 0005 决定第 4 条）、坏页也不进，页序与缓存序因此不重合，
        /// 而重新数出来的序号会静默地把另一页的像素写到这一页的位置上。
        slot: usize,
        /// **纸色提白对这一页做了什么**（纸色提白批 02 号票）。
        ///
...
impl OutputPage {
    /// 这一页的画质分曲线。彩色分支与坏页上都没有——一条不量化，一条没解出来。
    ...
    /// 这一页写出的尺寸。坏页没有自己的尺寸——它按卷内统一尺寸出，而那个数
    ...
    /// 这一页的尺寸贴合检查判定。判定范围之外的页没有——彩色分支上的页不在范围内
    /// （ADR 0010 决定第 4 条），坏页连几何都没有。残缺页在范围内（ADR 0007 决定第 1 条）。
    ...
    /// `uniform` 只对坏页说话：它写出去用的就是这个尺寸。
    /// **借着算，不吃掉这一页**：同一批页要拼两次报告——一次在[确认点](progress::Events::ask_before_the_second_pass)
    /// 上（那一份交给观察者拿主意，停车场 Q52），一次在这一卷收摊时——而写出环节夹在两者中间，
    /// 它读的是这同一批页。拿走所有权的话，第一次拼完写出环节就没得读了。
    ///
    /// 复制掉的只有报告要的那几格（源路径、画质分曲线、失败那句话）：编好的字节与缓存序号
...
struct Candidates {
    ...
    holds: Vec<Candidate>,
    /// 未贴合屏幕时的候选集。覆盖项把它裁空时是 `None`：`--dither fs` 撞上一页贴不住面板
...
impl Candidates {
    ...
    /// 画质分只有一问：**换成以高为准之后，这一页的门成不成立**。成立就指得出那条出路；
    /// 不成立的只有一种页——以高为准算出的目标尺寸越过[兜底上界](max_target_pixels)、
    /// 被退回 fit-inside 的那种（07 号票），对它劝换缩放方式是假话。
    ///
...
/// 分析环节：读 → 解码 → **切开** → 逐张彩页识别 → 分流。
///
/// 灰度路径：转灰 → 几何与尺寸贴合检查 → 缩放 → 画质分曲线 → 进缓存。
/// 彩色分支：几何 → 缩放 → 编码，不进缓存、不求画质分（ADR 0005 决定第 4 条）。
///
/// **灰度页在这一遍就编完的有两条路**（那一格装的因此是字节，不是参照，见 [`Settles`]）：
/// 默认那条路上灰阶档位逐页各判各的，画质分一出来这一页的档就定了，当场量化、编码
...
/// 因为分流决定了哪些页进得了整卷统一灰阶（ADR 0006 决定第 5 条）。
/// 走哪条分支由**面板与页**共同决定：只有彩色面板上的彩页走彩色分支，
/// 黑白面板上的彩页转灰、和其它页走同一条路。
///
/// 画质分两种模式都求值，dry-run 预告的就是照做时的那一档（spec 的 story 6）。
/// 覆盖了判定也照求：`--dry-run --bit-depth 2` 要说得清「你点的这一档画质分是多少」。
/// 彩色分支上没有这回事——那条路径不量化，dry-run 因此连编码都省了。
///
/// **尺寸贴合检查在这一遍上逐页收口。**门是几何的、一页看得出来，而它只决定这一页
/// （ADR 0007 决定第 1 条）：算到哪一页就判哪一页的门，候选集随之定下，画质分只在那一套上求。
...
///
/// **残缺页在范围内**：它的尺寸是文件头里的真尺寸，答得出「这一页会不会被下游再缩一次」，
/// 而它答的只是自己那一页。04 号票把它摘出去，是因为那时门对整卷只有一个结果；
/// 门改成逐页判之后那条理由不在了，口径把它收了回来（ADR 0007 决定第 1 条）。
///
/// **读不出、解不出的页在这里变成坏页**（12 号票），而不是让整卷的调用返回 `Err`。
/// 它同样不在判定范围内，理由比彩页还直白：它连尺寸都没有。画质分与缓存也一样绕开——
/// 没有像素可求画质分，也没有像素可缓存。它留下的只有一条原因，等写出环节给它留一张白页。
///
/// **读取与计算在这里分成两层**（13 号票，见 `read` 与 `medium`）：读取按硬盘类型定并发度，
/// 计算走 rayon 满核，两层之间是一道按在途字节背压的有界通道。页因此**乱序算完**，
/// 页序在收尾处按序号归位——除此之外，这一遍的产物与一页一页顺着做完全相同。
///
/// **立即停止让它回一份不全的清单**（ADR 0013 决定第 2 条）：发页那一侧的循环头上问一次闩，
/// 答立即停止就不再往下发。回来的因此可能短于源页数——[`process_volume`] 紧接着问一次闩，
/// 是立即停止就把整卷丢掉，那半份清单谁也看不见。
fn first_pass(
    volume: &mut Volume,
    redo: &[usize],
    compute: &Compute,
    ...
    // 它们的字节写出环节从上一趟的输出里搬。整卷重做时 `redo` 就是全部源页。
    ...
    // 换一次调度就换一句错误的报告等于没有报告。`--dither fs` 撞上尺寸贴合检查那一支尤其吃这一条,
    ...
    let pages: Vec<OutputPage> = scored
        ...
        .flatten()
        .collect();
    // 分析环节就编了字节的那两条路各自定过一份档，交出去对账（见 [`first_pass_verdicts`]）。
    // 立即停止那一支不留——手上这半份结果连同整卷一起丢掉。
...
/// 分析环节交出来的东西：这一卷的输出页，加上**分析环节就照它编好了字节**的那份逐页判定。
///
/// 它只在字节提前编好的那两条路上有——默认那条路（逐页各判各的）与顶死那一条
/// （见 [`first_pass_verdicts`]）；整卷统一灰阶那条路上字节要等写出环节才编，这一格因此是 `None`。
///
...
/// 这一卷的档**什么时候**定得下来。分析环节走完那一格缓存里装参照还是装编好的字节，
/// 由它一处说了算。
///
...
enum Settles {
    /// **等整卷**：整卷统一灰阶的统一档位要看完整卷才定得下（ADR 0005、ADR 0006）。
    /// 那一格装参照，写出环节取回来量化编码。
    AfterTheVolume,
...
impl Settles {
    /// 这一趟走哪一条。
    ///
    /// **预览不在字节提前编好的那两条之内**：那一趟没有写出环节，编出来的字节一个读者都
    ...
    /// 判定还有得判时才分两条路：整卷统一灰阶开着等整卷，不开就这一页自己。
    ...
    /// （two-pass-rework/14、15）：预览要预告的是照做时会发生的事（spec 的 story 6），
    ...
    /// 灰度页的字节是不是分析环节就编好：这一页自己定得下、或碰卷之前就定死，都是；
    /// 等整卷的那条路不是——那一格装参照，字节写出环节才编。
    ...
    /// 两条路上「候选从哪来」不取决于这一趟判了几页；等整卷的那条路答不出来（统一档位要判过才有）。
...
impl Compute<'_> {
    ...
    ///
    /// 报到一步，不是几步：步按**源页**数（`CONTEXT.md` 的《进度》：分析环节走每一页），
    /// 而一张源页读一次、解一次，切成几张不改变这一遍的工作量。
    ///
    /// 坏页在这里就报出去，不等整卷跑完（09 号票：坏页出现的当场就在主区可见）。
    ...
    fn color_page(
        ...
        //
        // 灰度路径上没有这一条：那边缩放结果还有画质分这个消费者，而预览存在的理由
        // 正是预告那个判定。
    ...
    fn gray_page(
        ...
        let slot = match self.settles {
            ...
            // 画质分曲线照旧求——上面那一格一步没少，预览说得出你点的那一档画质分是多少。
...
/// 加上 [`Reason::Override`]，分析环节当场量化编码即可。
///
...
/// 分析环节就照它编好了字节的那份逐页判定：默认那条路上逐页各判各的（`pinned` 是 `None`），
...
///
/// 三项绑成一个类型而不是各占一个参数：它们由同一段（裁白边 → 判跨页 → 拆分 → 每半再裁）
/// 一起算出，一起传下去，一起落进报告。摊成三个参数之后，两个可空的同型参数换了位置
/// 编译器一句话都不会说，而报告里会静默地把一张页说成另一张的形状。
#[derive(Debug, Clone, Copy)]
struct Piece {
    /// 这一张在源页上留下的那一块——裁白边、切开、每半再裁三段窗口叠起来的结果
    /// （见 [`Crop::then`]）。
    crop: Crop,
    /// 这一张是那一刀的产物吗；是的话，切在哪条中缝上、是哪一侧。整页出的是 `None`。
    cut: Option<spread::Cut>,
...
impl Piece {
    ...
    /// 切出来的一半：外层裁白边、这一刀、每半再裁，三段窗口叠成源页上的一块。
    /// 切得开的必然是候选。
...
impl Placement {
    ...
    /// 配上这一张的结局，就是分析环节产出的一张输出页。源页序号到此为止，理由见
    /// [`Placement::page`]。
...
/// 写出环节按阅读顺序要写的一格：这一趟**重做**的一张，或上一趟写的、**留下**的一张
/// （two-pass-rework/14；`CONTEXT.md` 的《留下的页》）。
///
/// 两种绑成一个枚举而不是两张表：归档卷的成员按写入顺序排，留下的与重做的**交错**着
/// 才是阅读顺序（理由与彩页为什么不在分析环节写出是同一条，见 [`second_pass`]）。
/// 卷内统一尺寸、撞名校验也走这一串——两件事问的都是整本书，不是这一趟碰过的那几张。
enum Slot<'a> {
    /// 这一趟重做的一张，连同汇总给它的判定（彩页与坏页没有）。
    Redone {
        page: &'a OutputPage,
        verdict: Option<Verdict>,
    },
    /// 上一趟写的、留下的一张，写出环节从上一趟的输出里搬过来（见 [`second_pass`]）。
...
impl<'a> Slot<'a> {
    ...
    /// 它写出去的像素尺寸。这一趟重做的坏页没有——它的尺寸正要由别的页定出来。
    fn size(&self) -> Option<Size> {
        match self {
...
///
/// 走的是源页序：留下的那一族按记录里的次序摆，重做的那几张从分析环节的产出里按序取——
/// 分析环节产出本来就按源页序摆、同一源页切出的几张挨着（见 [`first_pass`]），
...
fn in_reading_order<'a>(
    ...
    for (page, kept) in volume.pages.iter().zip(retained.families()) {
        ...
        next == scored.len(),
        "分析环节产出 {} 张，只有 {next} 张排得进阅读顺序：{} 这一张来路对不上任何一个源页",
        scored.len(),
...
///
/// **立即停止让它半路回来**（ADR 0013 决定第 2 条）：写出那一层的循环头上问一次闩，
/// 答立即停止就当场 `Ok(())`。它不必把这件事写进返回值——闩只升不降，调用方再问一次
...
impl Encode<'_> {
    ...
    ///
    /// 出的是 [`Cow`]：彩页的字节分析环节就编好了，这里**借**它而不是复制一份。
    ...
    fn page<'p>(&self, page: &'p OutputPage, verdict: Option<Verdict>) -> Result<Cow<'p, [u8]>> {
        ...
        match &page.outcome {
            Outcome::Failed { .. } => {
                // 空白占位页按 1bit 编，不跟卷级统一档位走。它不是一个**判定**——它没进过候选集、
                // 没求过画质分，卷级那一档说的是「这一卷的内容要几档灰」，而这一页没有内容。
                // 灰阶档位是编码属性（`CONTEXT.md`），而整页只有一个取值时 1bit 恰好装得下它；
                ...
                .map(Cow::Owned)
                .with_context(|| format!("编 {source} 这一页的空白占位页"))
            }
            Outcome::Processed {
                branch: Branch::Color { encoded },
                ..
            } => Ok(Cow::Borrowed(
                encoded.as_deref().expect("照做的那一趟分析环节就编过彩页"),
            )),
            Outcome::Processed {
                ...
                // 默认那条路与顶死的那一趟这一页分析环节就编好了（12、06 号票），这一遍只把它写出去。
...
fn gray_bytes(
    ...
    // 源页序号不在这个 `zip` 里：它是真的可空——分析环节那一处有，写出环节那一处没有
    // （见 [`Placement::page`]）。
    let record = recorder
        .zip(origin)
...
///
/// 这一遍**把源字节多读一遍**——它在分析环节解码之前，而分析环节还要再读一次。这笔成本换不掉：
/// 彩页在分析环节就编好并写进 tEXt（ADR 0010），那一刻这一页记录里源那一项必须已经齐了。
/// 换来的是命中时一趟都不用做——多读一遍字节，省掉的是整卷的解码、缩放、画质分与编码。
/// `--no-metadata` 连这一遍都不读：那时既没有记录可写，也没有依据可比。
///
/// 它走的是与分析环节同一个[读取层](read)，因此在没有寻道惩罚的盘上这一遍也是并发读的。
/// 喂哈希那一端仍**严格按成员次序**——卷级源哈希是有序的，乱一位整卷的指纹就变了，
/// 页级那一份按成员序号落位，而读取层交付本来就有序（见 `read` 的模块头）。
///
/// **[读取端是一个归档句柄](source::ReadingEnd::Archive)的卷上它也并发**，与分析环节不同：
...
fn volume_fingerprint(
    ...
    for read in read::reads(reader, &members, io.fingerprint.count, read::BUDGET) {
        ...
        // 也不让它走出那一卷（见那里的《立即停止：回 `None`》）。
        if events.aborting() {
            break;
        }
        let relative = &members[read.index].relative;
        match &mut feeding {
            // 读不出字节的成员在这一遍不算失败：它在分析环节里才变成坏页（12 号票），
            // 而这一遍排在分析环节之前。这里把它记成「读不出来」照样喂进哈希——
...
enum Reuse {
    ...
    /// 归档那一支上它握着最终位置上那个文件的句柄，从这里一直握到写出环节搬完——
    /// 中间隔着分析环节与确认点（ADR 0012）；那段时间里 Windows 上动不了上一趟的输出，认下。
    ByPage {
        retained: Retained,
        output: sink::Written,
    },
...
/// 之后**够不着了**——一页都没有的东西不是卷，清点就把它丢掉了；留着这一句，
/// 是因为「记录随页走」这条不变量要写在它成立的地方。
///
/// # 名单从哪儿来（页几何批 04 号票）
///
/// 从前它由 `page_targets` 在碰像素之前预告出来，逐个去比。拆分跨页落地之后那条预告
/// 不成立了——一个源页产出几张由内容决定（有没有中缝），而幂等的全部意义是在解码之前答完。
///
...
/// 整卷统一灰阶的代表页序号，写进 tEXt 那句 `volume-p95, driven by page 087` 用它。
///
/// 另外三种卷级判定没有代表页可指：覆盖项顶掉了判定，默认路径上卷级那一层没开，
/// 而跳过的卷根本走不到写出这一步。
...
/// 把这一页的参照与每个候选各比一遍。
///
/// 候选先裁再求值，顺序是 ADR 0003 定的：被裁掉的候选不进入画质分。
/// 这里只出量，拿量去和画质门槛比在 `decide`。
...
///
/// 四道裁剪，全部发生在画质分求值之前：灰阶档位按屏幕灰阶数裁（ADR 0003），抖动模式按尺寸贴合检查裁
...
fn dither_outside_the_gate_error(by_height_holds: bool) -> String {
    let way_out = if by_height_holds {
        ...
        format!(
            "缩放方式这一侧没有出路：以高为准让每一页都贴住面板高，而这一页算出的目标尺寸\
             越过 {} 像素、被兜底上界退回 fit-inside 出（07 号票），门是在那张退回来的页上判的",
            max_target_pixels()
        )
    };
    format!(
        "{}。{way_out}。剩下两条路——不点 --dither{HARD_SPACE}fs（画质分自己会替这一页把抖动关掉），\
         或换一张宽高比没这么极端的源页",
        Interlock::DitherOutsideTheGate
    )
}

/// 一个源页**最多**产出几张输出页：跨页从中缝上切一刀，因此是两张（页几何批 04 号票）。
///
/// 真产出几张由内容决定，解完像素才知道（见 `crate::spread`）。这个数是碰像素之前
/// 给得出来的那个**上界**：进度步数照它预告（[`volume_steps`]），分析环节走完拿它对一次区间
/// （[`process_volume`]）。
///
/// 它不是「切几刀」的配置项：切点只有一个（中缝），两半就是两张。
...
/// 一张坏页产出几张空白占位页。**恒 1，而且与 [`MAX_OUTPUTS_PER_SOURCE_PAGE`] 无关**：
/// 它没有像素可切，切不出第二张来（12 号票：读不出的页用空白页占位，页码顺序不变）。
///
...
mod tests {
    ...
    //!
    //! 这几条直接喂一份 N=2 的分析环节产物，问的是从成员命名到进度步数、从汇总到写出，
    ...
    /// 一份最小的请求。各用例只改自己那一处。
    ///
    /// `pub(crate)` 是给 `survey` 那几条用例的：它们要的也是这一份，只改点名的卷与输出目录。
    ...
    /// 一张走灰度路径的输出页。`values` 是三个候选各自的画质分值。
    ...
    ///
    /// 头一张源页的两半走彩色分支、不进整卷统一灰阶，灰度页因此排在第 2、3 位上——
    /// 按源页数的话代表页会指到 0 或 1，而那是另一张页。
    #[test]
    fn the_volume_level_summary_indexes_output_pages_not_source_pages() {
        ...
        let Some(VolumeVerdict::Envelope(envelope)) = verdict else {
            panic!("这一卷该由整卷统一灰阶定档，实际是 {verdict:?}");
        };
        assert!(
    ...
    /// 写出环节按输出页写出：一个源页的两半各落一个成员，各带各的字节，各报到一步（页几何批 03 号票）。
    ///
    /// 成员名跟着页走。由写出环节按源页序数出来的话，两半会写到同一个名字上——
    /// 一张覆盖另一张，而报告里两页仍各说各的。
    #[test]
