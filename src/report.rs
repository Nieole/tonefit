...
/// `--per-page` 一开，卷级根本没有候选，也就没有定档页可指。
...
pub enum VolumeVerdict {
    ...
    Override(Candidate),
    /// `--per-page` 关掉了上包络：候选逐页最优，卷内没有基准档（ADR 0006 决定第 6 条）。
    /// 翻页跳变随之回来，抖动模式也跟着逐页可变——**孤立地高出邻居的那些页除外**，
    /// 它们由段式迟滞压回邻居那一档（`crate::hysteresis`）。迟滞不跟着上包络一起关，
    /// 只是换了一种没有基准档也问得出的形态。
    PerPage,
