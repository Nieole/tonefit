...
pub struct Report {
    ...
    /// **这一趟做了多久**：`run` 从进到出的墙钟，扣掉在决策点上等人的那几分钟
    /// （加固批 11 号票，停车场 Q41）。
    ///
    /// 它因此不等于「`run` 这个调用在墙上占了多久」，而两者只在有人于**续做的决策点**上
    ...
    /// （**拒绝执行**那几种各是一道检查，**预扫那一种除外**——它在下一段单说；
    /// 单子在 `CONTEXT.md` 的《失败》）与逐卷的介质探测都在卷外，而这两样都要摸文件系统
...
/// 一个**非卷文件**（`CONTEXT.md` 的《处理对象》）：发现走完之后没被任何卷收下的文件。
///
...
///
/// 逐个变体都列出来、不留 `_`：与 [`RunOutcome`] 同一条规矩——多一类非卷文件该怎么说，
...
pub enum RunOutcome {
    /// 点名的卷都走过一遍了。其中可能有卷被隔离、有卷没做成、有卷在**决策点**上
    /// 停在了第二遍之前（ADR 0012：续做）——那都是**卷**的结局，不是这一趟的。
    ///
    /// 决策点上停下的那一卷照样进 [`Report::volumes`]（它做过事，只是没写出去），
...
pub struct VolumeReport {
    ...
    /// 而试算不编码，那一趟的彩页因此一次都不缩（05 号票）——除了这个数，
    /// 没有别的外部信号说得出它真的没跑。
    pub resizes: usize,
    ...
    ///
    /// 它在报告里，只为让「某一趟确实没攒参照」量得出来：位深与抖动被裁到只剩一个候选时，
    /// 第二遍要用的那一档在开卷之前就定死了，参照没有必要活到第二遍
    /// （两遍管线批 06 号票）。
...
pub struct VolumeTiming {
    ...
    /// 观察者在**决策点**上答了收尾（ADR 0012 决定第 2 条：那一卷停在第二遍之前，
    /// 等于走了一次试算）。三种都是「这一遍压根没走」，因此是零而不是一个很小的数。
    pub second_pass: Duration,
    /// **这一卷做了多久**：从打开卷到这份卷报告成型的墙钟，扣掉在决策点上等人的那一截
    /// （停车场 Q41）。
    ///
    /// 「打开卷」是**两遍**：预扫那一遍由预扫交过来，重开那一遍就发生在这段墙钟之内
...
impl VolumeTiming {
    ...
    ///
    /// 打开与枚举在这里**各两遍**：预扫数一遍算步数，轮到这一卷时按路径再开一遍
...
/// 卷级判定：这一卷的候选从哪来（spec 的卷报告形状：判定位深、抖动模式、判定理由、定档页）。
///
/// 几项绑成一个枚举而不是各占一个字段，因为它们不是每一种情形下都同时成立：
/// 默认那条路（逐页）上卷级根本没有候选，也就没有定档页可指。
///
...
impl VolumeVerdict {
    ...
    ///
    /// 默认那条路（逐页）上没有卷级的那一个：抖动跟着位深一起逐页可变（ADR 0018）。
...
impl PageReport {
    ...
    /// 这一页的缩放，含义见 [`Scaling`]。失败页没有——它没解出来，也就没有目标尺寸可谈。
