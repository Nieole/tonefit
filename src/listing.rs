...
//! 报告末尾那三小结（非漫画文件 · 卷转换失败 · 发现无法访问）、清点那条拒绝、
//! 开工前撞名那条拒绝，加上尺寸贴合检查与纸白那两句里点名头几页的那一小截。
//!
//! # 共用的是哪两件，不共用的是哪两件
//!
...
//! 报告末尾那三小结（bin）、清点那条拒绝与撞名那条拒绝（**库自己写下的、
...
//! **三副都在本模块里**，添第四副也添在这里——「另有」那两个字只有一处出处，
...
impl<'a, T> FirstFew<'a, T> {
    ...
    /// 上限自己给。
    ///
    /// **给别的数要有别的理由**：尺寸贴合检查与纸白那两句里点名头几页取三，理由是那一小截
    ...
    /// **一条都不列、只说个数的那一副**：`另有 3 个`。
    ///
    /// 会话清点之后那棵树的**末行**走的是它（`session::shell::list`）：没勾的处理路径
    /// 一条都不进树，末行暗淡地说一句另有几条（`CONTEXT.md` 的《卷列表》）。
    /// 「列零条」是一副排版，不是一个特例——[摞成一块](Self::stacked)与
    /// [串在一句话里](Self::strung)那两副在这一档上各自会多出一个省略号或一个逗号。
    ///
    /// 前面挂什么、后面接什么归说它的那一处（那一行接的是「路径未勾选，本次不处理」）。
    /// **一条都不剩就一个字都不说。**
    pub fn only_the_rest(&self, unit: &str) -> Option<String> {
        self.says_the_rest(unit)
    }
