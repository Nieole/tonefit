...
//!
//! 分层的理由在硬盘类型上（ADR 0009，见 [`crate::medium`]）：读取的最优并发度由盘决定，
...
//! 读取端是一个目录就抄一份卷根（目录卷，以及摊开了的卷），是一个归档句柄就另开一个
//! 文件句柄。因此「一个 ZipArchive 就是一个游标」
...
pub struct Read {
    ...
    /// 读出来的字节，或者读不出来的那句话。
    ///
    /// 读不出来在这里**不是错误，是一个结果**：它在分析环节里变成坏页（12 号票），
...
/// 按计划读一批成员。
///
/// `readers` 是[读取计划](crate::medium::IoPlan)定下的并发度，1 即串行。
/// [读取端是一个归档句柄](crate::source::ReadingEnd::Archive)的卷上两遍那一路仍恒为 1，
/// 幂等那一道不是——两个数各由计划里的一格给出，这一层只照它做。
/// **摊开了的卷不落在那一格上**：它的读取端是一个目录，两路拿同一个数。
///
/// **开不出那么多读取端就用开出来的那几条。**读取端是一个归档句柄时每条线程要一个自己的文件句柄
...
/// （[`Independent::Replaced`]，判定依据与代价见 `source::ArchiveStamp`），
...
mod tests {
    ...
    ///
    /// 12 号票的界线在这里保住：一页读不出来变成坏页，不是让读取层把整卷掀了。
    #[test]
    ...
    ///
    /// 这一层**不判该不该并发**——那是 `crate::medium` 那一层的事（读取端是一个归档句柄时
    /// 两遍恒为一条，幂等那一道按硬盘类型走）。这里钉的是「点名了就派得动」：从前它在这里被兜底按回串行，
    ...
    ///
    /// 读取端是一个归档句柄时它就是一个打开的文件句柄，这一条因此钉的是那笔句柄账里带乘数的
